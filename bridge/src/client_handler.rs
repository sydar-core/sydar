...
impl ClientHandler {
    ...
    pub async fn send_immediate_job_to_client<T: sydarApiTrait + Send + Sync + ?Sized + 'static>(
        &self,
        ...
        tokio::spawn(async move {
            ...
            // Get block template
            let template_result = sydar_api_clone.get_block_template(&wallet_addr, &remote_app, &canxium_addr).await;
            ...
            if !state.is_initialized() {
                ...
                use crate::hasher::sydarDiff;
                let mut stratum_diff = sydarDiff::new();
    ...
    pub async fn new_block_available<T: sydarApiTrait + Send + Sync + 'static>(&self, sydar_api: Arc<T>) {
            ...
            tokio::spawn(async move {
                ...
                let block = match template_result {
                    ...
                    Err(e) => {
                        if e.to_string().contains("Could not decode address") {
                            record_worker_error(&instance_id, &wallet_addr, crate::errors::ErrorShortCode::InvalidAddressFmt.as_str());
                            error!("failed fetching new block template from sydar, malformed address: {}", e);
                            client_clone.disconnect();
                        } else {
                            record_worker_error(&instance_id, &wallet_addr, crate::errors::ErrorShortCode::FailedBlockFetch.as_str());
                            error!("failed fetching new block template from sydar: {}", e);
                        }
                        return;
                    }
                };
        ...
        // Check balances periodically
        {
            ...
            if last_check.elapsed() > BALANCE_DELAY && !addresses.is_empty() {
                *last_check = Instant::now();
                ...
                // Fetch balances via sydar_api
                let addresses_clone = addresses.clone();
                let sydar_api_clone = Arc::clone(&sydar_api);
                let instance_id = self.instance_id.clone();
                tokio::spawn(async move {
                    match sydar_api_clone.get_balances_by_addresses(&addresses_clone).await {
                        Ok(balances) => {
                            // Record balances
                            crate::prom::record_balances(&instance_id, &balances);
                        }
                        Err(e) => {
                            warn!("failed to get balances from sydar, prom stats will be out of date: {}", e);
                        }
                    }
                });
            }
