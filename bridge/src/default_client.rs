...
static WALLET_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"sydar(test|dev)?:([a-z0-9]{61}|[a-z0-9]{63})").unwrap());
...
pub fn default_handlers() -> HashMap<String, crate::stratum_listener::EventHandler> {
    ...
    handlers.insert(
        ...
        Arc::new(|ctx: Arc<StratumContext>, event: JsonRpcEvent| {
            ...
            let event = event.clone();
            Box::pin(async move {
                // Default handler - no client_handler/sydar_api (will use polling fallback)
...
/// If client_handler and sydar_api are provided, sends immediate job after authorization
pub async fn handle_authorize(
    ctx: Arc<StratumContext>,
    event: JsonRpcEvent,
    client_handler: Option<Arc<crate::client_handler::ClientHandler>>,
    sydar_api: Option<Arc<dyn crate::share_handler::sydarApiTrait + Send + Sync>>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    *ctx.worker_name.lock() = worker_name.clone();
    ...
    if let (Some(client_handler), Some(sydar_api)) = (client_handler, sydar_api) {
        tracing::debug!(
            "[AUTHORIZE] Step 3-4: Triggering immediate job send for client {} (extranonce already sent)",
            ctx.remote_addr
        );
        client_handler.send_immediate_job_to_client(ctx.clone(), sydar_api).await;
    } else {
        // Fallback: let polling loop handle it (may cause disconnects for IceRiver)
        tracing::warn!(
            "[AUTHORIZE] WARNING: No client_handler/sydar_api available - job will be sent by polling loop (may cause IceRiver disconnect)"
        );
    }
...
fn clean_wallet(input: &str) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    // Try to decode as sydar address (supports sydar:, sydartest:, sydardev:)
    if Address::try_from(input).is_ok() {
        return Ok(input.to_string());
    }

    // Try with sydar: prefix if no recognized prefix
    if !input.starts_with("sydar:") && !input.starts_with("sydartest:") && !input.starts_with("sydardev:") {
        return clean_wallet(&format!("sydar:{}", input));
    }
    ...
    Err("unable to coerce wallet to valid sydar address".into())
}
