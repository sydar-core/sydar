...
use sydar_hashes::{BlockHash, HasherBase};
...
/// sydar difficulty representation
#[derive(Debug, Clone)]
pub struct sydarDiff {
    pub hash_value: f64,
    pub diff_value: f64,
    pub target_value: BigUint,
}

impl Default for sydarDiff {
    fn default() -> Self {
...
pub fn stratum_difficulty_to_target_sydar(stratum_diff: u64) -> BigUint {
...
/// Based on: https://github.com/tmrlvi/sydar-miner/blob/bf361d02a46c580f55f46b5dfa773477634a5753/src/client/stratum.rs#L375
pub fn diff_to_target(diff: f64) -> BigUint {
    ...
    if use_stratum_alt {
        ...
        let stratum_target = stratum_difficulty_to_target_sydar(diff_u64);
...
/// Uses sydar_hashes::BlockHash to match the working stratum implementation
/// Returns the Hash type directly (not bytes) to match working stratum code
pub fn serialize_block_header(
    block: &sydar_consensus_core::block::Block,
) -> Result<sydar_hashes::Hash, Box<dyn std::error::Error>> {
...
///
/// NOTE: Timestamp is in MILLISECONDS (as per sydar block header format)
/// Pool uses: timestampLE.writeBigUInt64LE(timestamp) which writes u64 in LE
pub fn generate_iceriver_job_params(pre_pow_hash: &sydar_hashes::Hash, timestamp: u64) -> String {
...
/// This matches sydar's compact difficulty format
