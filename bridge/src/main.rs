...
use sydar_stratum_bridge::log_colors::LogColors;
use sydar_stratum_bridge::{BridgeConfig as StratumBridgeConfig, sydarApi, listen_and_serve, prom};
use std::ffi::OsString;
use std::path::{Path, PathBuf};
use std::sync::{Arc, OnceLock};
use tracing_subscriber::EnvFilter;

use sydard_lib::args as sydard_args;
...
struct Cli {
    ...
    #[arg(last = true, help = "sydard arguments (use '--' separator if sydard args start with hyphens)")]
    sydard_args: Vec<String>,
}
...
fn log_bridge_configuration(config: &app_config::BridgeConfig) {
    ...
    tracing::info!("\tsydard:          {} (shared)", config.global.sydard_address);
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        ...
        if node_mode == NodeMode::Inprocess {
            EnvFilter::new("warn,sydar_stratum_bridge=info,sydar=info,sydard=info,sydard_lib=info,log=info")
        } else {
            EnvFilter::new("warn,sydar_stratum_bridge=info")
        }
    });

    // Start in-process node after tracing is initialized so bridge logs (including the stats table)
    // are not filtered out by a tracing subscriber installed by sydard.
    let mut inprocess_node: Option<InProcessNode> = None;
    if node_mode == NodeMode::Inprocess {
        let mut node_args: Vec<String> = cli.sydard_args;

        // Add appdir if not provided in sydard_args
        if !node_args.iter().any(|arg| arg.starts_with("--appdir")) {
            let default_appdir = app_dirs::default_inprocess_sydard_appdir();
            ...
            assert!(cli.appdir.is_none(), "appdir should not be specified both in bridge args and sydard args");
        }

        let mut argv: Vec<OsString> = Vec::with_capacity(node_args.len() + 1);
        argv.push(OsString::from("sydard"));
        argv.extend(node_args.iter().map(OsString::from));
        let args = sydard_args::Args::parse(argv).map_err(|e| anyhow::anyhow!("{}", e))?;
    ...
    // Create shared sydar API client (all instances use the same node)
    let sydar_api = sydarApi::new(
        config.global.sydard_address.clone(),
        config.global.block_wait_time,
        config.global.coinbase_tag_suffix.clone(),
    )
    .await
    .map_err(|e| anyhow::anyhow!("Failed to create sydar API client: {}", e))?;
    ...
    for (idx, instance_config) in config.instances.iter().enumerate() {
        ...
        let sydar_api_clone = Arc::clone(&sydar_api);
        ...
        let handle = tokio::spawn(async move {
            ...
            let bridge_config = StratumBridgeConfig {
                ...
                sydard_address: global.sydard_address.clone(),
            ...
            listen_and_serve(
                bridge_config,
                Arc::clone(&sydar_api_clone),
                if is_first_instance { Some(sydar_api_clone) } else { None },
            )
            .await
