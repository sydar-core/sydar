...
pub struct MiningState {
    ...
    stratum_diff: Arc<Mutex<Option<sydarDiff>>>,
    max_jobs: u16,
    last_header: Arc<Mutex<Option<sydar_consensus_core::header::Header>>>, // Track previous header for change logging
}
...
impl MiningState {
    ...
    pub fn stratum_diff(&self) -> Option<sydarDiff> {
        self.stratum_diff.lock().clone()
    }

    /// Set stratum difficulty
    pub fn set_stratum_diff(&self, diff: sydarDiff) {
    ...
    /// Get last header
    pub fn get_last_header(&self) -> Option<sydar_consensus_core::header::Header> {
        self.last_header.lock().clone()
    }

    /// Set last header
    pub fn set_last_header(&self, header: sydar_consensus_core::header::Header) {
