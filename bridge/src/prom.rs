...
pub fn init_metrics() {
    ...
    BLOCK_ACCEPTED_COUNTER.get_or_init(|| {
        register_counter_vec!(
            ...
            "Number of blocks accepted by the connected sydar node (may later be red)",
            WORKER_LABELS
        )
        .unwrap()
    });
...
async fn get_config_json() -> String {
        ...
        && let Some(doc) = docs.first()
    {
        ...
        if let Some(addr) = doc["sydard_address"].as_str() {
            config.insert("sydard_address".to_string(), serde_json::Value::String(addr.to_string()));
        }
...
async fn update_config_from_json(json_body: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    out_str.push_str("# This file configures the stratum bridge that connects miners to the sydar node\n\n");
    ...
    if let Some(addr) = config.get("sydard_address").and_then(|v| v.as_str()) {
        out_str.push_str("# sydar node gRPC address (format: \"HOST:PORT\" or \"grpc://HOST:PORT\")\n");
        out_str.push_str(&format!("sydard_address: {}\n\n", addr));
    }
