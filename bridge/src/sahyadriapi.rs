...
use crate::share_handler::sydarApiTrait;
...
use sydar_notify::{listener::ListenerId, scope::NewBlockTemplateScope};
use sydar_rpc_core::notify::mode::NotificationMode;
use sydar_rpc_core::{
    GetBlockTemplateRequest, GetConnectedPeerInfoRequest, GetCurrentBlockColorRequest, GetInfoRequest, GetsydarDagInfoRequest,
...
/// sydar API client wrapper using RPC client
...
impl sydarApi {
    ...
    pub async fn new(address: String, _block_wait_time: Duration, coinbase_tag_suffix: Option<String>) -> Result<Arc<Self>> {
        ...
        debug!("{} {}", LogColors::api("[API]"), LogColors::label("Establishing RPC connection to sydar node:"));
        ...
        // Connect to sydar node with grpc:// prefix, using extended request timeout and reconnection support
        let client = Arc::new(
            ...
            .await
            .context("Failed to connect to sydar node")?,
        );
    ...
    async fn start_stats_thread(self: Arc<Self>) {
        ...
        use sydar_rpc_core::{EstimateNetworkHashesPerSecondRequest, GetsydarDagInfoRequest};
        ...
        loop {
            ...
            // Get block DAG info
            // GetsydarDagInfoRequest is a unit struct, construct directly
            let dag_response = match self.client.get_sydar_dag_info_call(None, GetsydarDagInfoRequest {}).await {
                Ok(r) => r,
                Err(e) => {
                    warn!("failed to get network hashrate from sydar, prom stats will be out of date: {}", e);
                    continue;
                }
            };
    ...
    async fn start_node_status_thread(self: Arc<Self>) {
        ...
        loop {
            ...
            let dag_info_fut = self.client.get_sydar_dag_info_call(None, GetsydarDagInfoRequest {});
    ...
    pub async fn submit_block(&self, block: Block) -> Result<SubmitBlockResponse> {
        // Use sydar_consensus_core::hashing::header::hash() for block hash calculation
        // In sydar, the block hash is the header hash (transactions are represented by hash_merkle_root in header)
        use sydar_consensus_core::hashing::header;
        ...
        match &result {
            Ok(response) => {
                ...
                tokio::spawn(async move {
                    ...
                    if let Ok(dag_response) = client_clone.get_sydar_dag_info_call(None, GetsydarDagInfoRequest {}).await {
    ...
    async fn wait_for_sync(&self, verbose: bool) -> Result<()> {
        if verbose {
            debug!("checking sydard sync state");
        }
        ...
        loop {
            match self.client.get_sync_status().await {
                Ok(is_synced) => {
                    if is_synced {
                        if verbose {
                            debug!("sydard synced, starting server");
                        }
                        break;
                    }
                }
            ...
            if verbose {
                warn!("sydar is not synced, waiting for sync before starting bridge");
            }
                ...
                Err(e) => {
                    ...
                    if error_str.contains("Odd number of digits") {
                        return Err(anyhow::anyhow!(
                            "Failed to convert RPC block to Block after {} attempts: {} - This usually indicates a malformed hash field in the block template from the sydar node. The block may have a hash with an odd-length hex string.",
                            max_retries,
                            error_str
                        ));
                    } else {
    ...
    pub async fn get_balances_by_addresses(&self, addresses: &[String]) -> Result<Vec<(String, u64)>> {
        ...
        let utxos = self
            .client
            .get_utxos_by_addresses_call(None, sydar_rpc_core::GetUtxosByAddressesRequest::new(addresses))
            .await
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if let Err(e) = api_clone.wait_for_sync(false).await {
                    error!("error checking sydard sync state, attempting reconnect: {}", e);
...
impl sydarApiTrait for sydarApi {
    async fn get_block_template(
        ...
        sydarApi::get_block_template(self, wallet_addr, "", "").await.map_err(|e| {
    ...
    async fn submit_block(
        &self,
        block: Block,
    ) -> Result<sydar_rpc_core::SubmitBlockResponse, Box<dyn std::error::Error + Send + Sync>> {
        sydarApi::submit_block(self, block)
            .await
