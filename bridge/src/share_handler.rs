...
impl ShareHandler {
    ...
    pub async fn handle_submit(
        ...
        event: JsonRpcEvent,
        sydar_api: Arc<dyn sydarApiTrait + Send + Sync>,
        ...
        if let Some(Value::String(submitted_identity)) = event.params.first() {
            ...
            let submitted_clean = submitted_address.trim_start_matches("sydar:").trim_start_matches("sydartest:");
            let authorized_clean = wallet_addr.trim_start_matches("sydar:").trim_start_matches("sydartest:");
        ...
        loop {
            ...
            // Use sydar_pow::State for proper PoW validation
            use sydar_pow::State as PowState;
            ...
            if meets_network_target {
                ...
                // The header comes directly from the sydar node via get_block_template_call()
                // We preserve: version, bits, timestamp, all hash fields, parents, scores, etc.
                header_clone.nonce = nonce_val;

                // Verify timestamp is still valid (not too old)
                // sydar typically accepts blocks with timestamps within a reasonable window
                ...
                debug!(
                    ...
                    "Using REAL header from sydar node block template"
                );
                ...
                debug!("{} {}", LogColors::block("[BLOCK]"), "Calling sydar_api.submit_block()...");

                // Submit block to node
                let block_submit_result = sydar_api.submit_block(block.clone()).await;
                ...
                match block_submit_result {
                    Ok(_response) => {
                        ...
                        let sydar_api = Arc::clone(&sydar_api);
                        let block_hash_for_confirm = block_hash.clone();

                        tokio::spawn(async move {
                            for _ in 0..BLOCK_CONFIRM_MAX_ATTEMPTS {
                                match sydar_api.get_current_block_color(&block_hash_for_confirm).await {
                                    Ok(true) => {
...
// Trait for sydar API operations
#[async_trait::async_trait]
pub trait sydarApiTrait: Send + Sync {
