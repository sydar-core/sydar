use crate::{
    ...
    sydarapi::sydarApi,
    share_handler::{sydarApiTrait, ShareHandler},
...
pub async fn listen_and_serve<T: sydarApiTrait + Send + Sync + 'static>(
    config: BridgeConfig,
    sydar_api: Arc<T>,
    // Optional: if concrete sydarApi is provided, use notification-based listener
    concrete_sydar_api: Option<Arc<sydarApi>>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let authorize_handler = {
        ...
        Arc::new(move |ctx: Arc<StratumContext>, event: JsonRpcEvent| {
            ...
            let event_clone = event.clone();
            Box::pin(async move {
                handle_authorize(ctx_clone, event_clone, Some(client_handler), Some(sydar_api))
                    .await
                    .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
            })
                as std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), Box<dyn std::error::Error + Send + Sync>>> + Send>>
    ...
    let submit_handler = {
        ...
        Arc::new(move |ctx: Arc<StratumContext>, event: JsonRpcEvent| {
            ...
            Box::pin(async move {
                share_handler
                    .handle_submit(ctx_clone, event, sydar_api)
                    .await
                    .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
            })
                as std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), Box<dyn std::error::Error + Send + Sync>>> + Send>>
    ...
    // If concrete sydarApi is provided, use notification-based listener
    // Otherwise, use polling only (fallback for trait objects)
    if let Some(concrete_api) = concrete_sydar_api {
        // We have concrete sydarApi - use notification-based listener
        let client_handler_cb = Arc::clone(&client_handler);
        let sydar_api_cb = Arc::clone(&sydar_api);
        ...
        // Call the method directly on Arc<sydarApi> (it's an instance method taking Arc<Self>)
        ...
        // No concrete sydarApi provided - use polling only
        warn!("Using polling-based block template listener (concrete sydarApi not provided, notifications not available)");

        let client_handler_poll = Arc::clone(&client_handler);
        let sydar_api_poll = Arc::clone(&sydar_api);
        tokio::spawn(async move {
            ...
            loop {
                interval.tick().await;
                // Poll for new blocks
                client_handler_poll.new_block_available(Arc::clone(&sydar_api_poll)).await;
            }
        });
    }
