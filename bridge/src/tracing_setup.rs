    ...
    N: for<'a> FormatFields<'a> + 'static,
{
    fn format_event(
        ...
        // Special-case forwarded node logs (from `tracing_log::LogTracer`) to match sydard style:
        ...
        if self.apply_colors {
            ...
            if message.contains("[ASIC->BRIDGE]") {
                ...
                // Configuration lines - color the label part (e.g., "\tsydard:          value")
...
pub(crate) fn init_tracing(
    ...
    // In inprocess mode, the embedded node primarily uses the `log` crate (via sydar_core::* macros).
