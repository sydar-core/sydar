...
use sydar_daemon::{DaemonEvent, DaemonKind, Daemons};
use sydar_wallet_core::account::Account;
use sydar_wallet_core::rpc::ConnectOptions;
use sydar_wallet_core::rpc::DynRpcApi;
use sydar_wallet_core::storage::{IdT, PrvKeyDataInfo};
use sydar_wrpc_client::{Resolver, sydarRpcClient};
...
impl From<&sydarCli> for Arc<Terminal> {
    fn from(ctx: &sydarCli) -> Arc<Terminal> {
        ctx.term()
    }
}

impl AsRef<sydarCli> for sydarCli {
    fn as_ref(&self) -> &Self {
        self
    }
}

impl workflow_log::Sink for sydarCli {
...
impl sydarCli {
    pub fn init() {
        cfg_if! {
            if #[cfg(not(target_arch = "wasm32"))] {
                ...
                sydar_core::log::init_logger(None, "info");
            } else {
                sydar_core::log::set_log_level(LevelFilter::Info);
            }
        }
    ...
    pub async fn try_new_arc(options: Options) -> Result<Arc<Self>> {
        ...
        let sydar_cli = Arc::new(sydarCli {
        ...
        let term = Arc::new(Terminal::try_new_with_options(sydar_cli.clone(), options.terminal)?);
        term.init().await?;

        cfg_if! {
            if #[cfg(target_arch = "wasm32")] {
                sydar_cli.init_panic_hook();
            }
        }
    ...
    pub fn try_rpc_client(&self) -> Option<Arc<sydarRpcClient>> {
                    ...
                    msg = multiplexer.receiver.recv().fuse() => {
                        ...
                        if let Ok(msg) = msg {
                            match *msg {
                                Events::WalletList { .. } => {},
                                Events::WalletPing => {
                                    // log_info!("sydar NG - received wallet ping");
                                },
                                Events::Metrics { network_id : _, metrics : _ } => {
                                    // log_info!("sydar NG - received metrics event {metrics:?}")
                                }
                                Events::FeeRate { .. } => {},
                                ...
                                Events::UtxoIndexNotEnabled { .. } => {
                                    tprintln!(this, "Error: sydar node UTXO index is not enabled...")
                                },
                                ...
                                Events::ServerStatus {
                                    ...
                                    tprintln!(this, "Connected to sydar node version {server_version} at {}", url.unwrap_or("N/A".to_string()));

                                    let _is_open = this.wallet.is_open();

                                  //  if !is_synced {
                                  //     if is_open {
                                  //          terrorln!(this, "Unable to update the wallet state - sydar node is currently syncing with the network...");

                                  //      } else {
                                  //          terrorln!(this, "sydar node is currently syncing with the network, please wait for the sync to complete...");
                                  //      }
                                  //  }
    ...
    pub async fn shutdown(&self) -> Result<()> {
        if !self.shutdown.load(Ordering::SeqCst) {
            ...
            let sydard = self.daemons().try_sydard();
            ...
            if let Some(sydard) = sydard.as_ref() {
                sydard.mute(false).await?;
                sydard.stop().await?;
            }
...
pub async fn sydar_cli(terminal_options: TerminalOptions, banner: Option<String>) -> Result<()> {
    sydarCli::init();

    let options = Options::new(terminal_options, None);
    let cli = sydarCli::try_new_arc(options).await?;

    let banner =
        banner.unwrap_or_else(|| format!("sydar Cli Wallet v{} (type 'help' for list of commands)", env!("CARGO_PKG_VERSION")));
    ...
    // --- sydar Global Auto-Connect Start ---
    ...
    if let Some(client) = cli.wallet().try_wrpc_client().as_ref() {
        tprintln!(cli, "sydar Auto-Pilot: Handshaking with node...");
        ...
        match client.connect(Some(connect_options)).await {
            Ok(_) => {
                ...
                let _ = cli.wallet().settings().set(sydar_wallet_core::settings::WalletSettings::Server, url);
                tprintln!(cli, "sydar Auto-Pilot: Connection Successful!");
            }
            Err(e) => {
                tprintln!(cli, "sydar Auto-Pilot Error: Node busy or offline. Details: {}", e);
            }
        }
    }
    // --- sydar Global Auto-Connect End ---
