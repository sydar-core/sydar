use crate::imports::*;
use sydar_consensus_core::tx::{TransactionInput, TransactionOutpoint};
use sydar_wallet_core::storage::Binding;
use sydar_wallet_core::storage::{TransactionData, TransactionKind, TransactionRecord};
use sydar_wallet_core::wallet::WalletGuard;
...
impl TransactionExtension for TransactionRecord {
    ...
    async fn format_transaction_with_args(
        ...
        let suffix = sydar_suffix(&self.network_id.network_type);
        ...
        match transaction_data {
            ...
            | TransactionData::Change { utxo_entries, aggregate_input_value, .. } => {
                let aggregate_input_value =
                    transaction_type.style_with_sign(kana_to_sydar_string(*aggregate_input_value).as_str(), history);
                ...
                if include_utxos {
                    for utxo_entry in utxo_entries {
                        ...
                        let amount = transaction_type.style_with_sign(kana_to_sydar_string(utxo_entry.amount).as_str(), history);
            ...
            | TransactionData::TransferOutgoing { fees, aggregate_input_value, transaction, payment_value, change_value, .. } => {
                if let Some(payment_value) = payment_value {
                    lines.push(format!(
                        ...
                        "",
                        style(kana_to_sydar_string(*payment_value)).red(),
                        style(kana_to_sydar_string(*aggregate_input_value)).blue(),
                        style(kana_to_sydar_string(*fees)).red(),
                        style(kana_to_sydar_string(*change_value)).green(),
