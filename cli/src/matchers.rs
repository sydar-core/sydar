...
pub fn register_link_matchers(cli: &Arc<sydarCli>) -> Result<()> {
    if !is_wasm() {
        return Ok(());
    }
    ...
    // addresses (open,copy) https://explorer.sydar.org/addresses/
    let cli_ = cli.clone();
    cli.term().register_link_matcher(
        &js_sys::RegExp::new(r"(sydar|sydartest):\S+", "i"),
        Arc::new(Box::new(move |modifiers, uri| {
            if modifiers.ctrl || modifiers.meta {
                if uri.starts_with("csmtest") {
                    ...
                    let url = format!("https://explorer.sydar.org/addresses/{uri}");
                    if is_nw() {
                        nw_sys::shell::open_external(&url);
                    } else {
                        link::open(&url);
    ...
    // blocks (open,copy) https://explorer.sydar.org/blocks/
    let cli_ = cli.clone();
    cli.term().register_link_matcher(
        ...
        Arc::new(Box::new(move |modifiers, text| {
            ...
            if modifiers.ctrl || modifiers.meta {
                nw_sys::shell::open_external(&format!("https://explorer.sydar.org/blocks/{uri}"));
            } else {
    ...
    cli.term().register_link_matcher(
        ...
        Arc::new(Box::new(move |modifiers, text| {
            ...
            if modifiers.ctrl || modifiers.meta {
                nw_sys::shell::open_external(&format!("https://explorer.sydar.org/txs/{uri}"));
            } else {
                write_to_clipboard(&cli_, uri.to_string().as_str());
            }
        })),
...
fn write_to_clipboard(cli: &Arc<sydarCli>, text: &str) {
    if is_nw() {
