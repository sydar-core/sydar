use sydar_wallet_core::account::BIP32_ACCOUNT_KIND;
use sydar_wallet_core::account::LEGACY_ACCOUNT_KIND;
use sydar_wallet_core::account::MULTISIG_ACCOUNT_KIND;
                    ...
                    ctx.term().help(
                        &[
                            (
                                ...
                                "Import KDX keydata file or sydarnet web wallet data on the same domain",
                            ),
                            (
                                "account import mnemonic bip32",
                                "Import Bip32 (12 or 24 word mnemonics used by sydarwallet, kaspium, onekey, tangem etc.)",
                            ),
                            (
                                "account import mnemonic legacy",
                                "Import accounts 12 word mnemonic used by legacy applications (KDX and sydarnet web wallet)",
                            ),
                            (
                ...
                match import_kind.as_ref() {
                    "legacy-data" => {
                        ...
                        if exists_legacy_v0_keydata().await? {
                            ...
                            return Err("'sydarnet' web wallet storage not found at this domain name".into());
                        } else {
                    ...
                    "mnemonic" => {
                        if argv.is_empty() {
                            ...
                            tprintln!(ctx, "please use 'legacy' for 12-word KDX and sydarnet web wallet mnemonics\r\n");
                            return Ok(());
                        }
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<sydarCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ...
                (KDX and sydarnet web wallet). Use 'account import' for additional help.",
                ),
            ...
            .derivation_scan(
                ...
                Some(Arc::new(move |processed: usize, _, balance, txid| {
                    if let Some(txid) = txid {
                        ...
                        tprintln!(ctx_, "Scanned {} derivations, found {} CSM", processed, kana_to_sydar_string(balance));
                    }
                })),
            )
            .await?;
