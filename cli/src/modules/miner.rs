use crate::imports::*;
use sydar_daemon::{CpuMinerConfig, locate_binaries};
...
impl DefaultSettings for MinerSettings {
    async fn defaults() -> Vec<(Self, Value)> {
        ...
        if let Ok(binaries) = locate_binaries(&root, "sydar-cpu-miner").await
...
impl Miner {
    ...
    async fn create_config(&self, ctx: &Arc<sydarCli>) -> Result<CpuMinerConfig> {
    ...
    async fn main(self: Arc<Self>, ctx: Arc<sydarCli>, mut argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
    ...
    async fn select(self: Arc<Self>, ctx: Arc<sydarCli>) -> Result<()> {
        let root = nw_sys::app::folder();

        let binaries = sydar_daemon::locate_binaries(root.as_str(), "sydar-cpu-miner").await?;

        if binaries.is_empty() {
            tprintln!(ctx, "No sydar-cpu-miner binaries found");
        } else {
            let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
            if let Some(selection) = ctx.term().select("Please select sydar-cpu-miner binary", &binaries).await? {
