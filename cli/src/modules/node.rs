...
pub struct Node {
    settings: SettingsStore<sydardSettings>,
    mute: Arc<AtomicBool>,
...
impl Default for Node {
    fn default() -> Self {
        Node {
            settings: SettingsStore::try_new("sydard").expect("Failed to create node settings store"),
...
impl Handler for Node {
    fn verb(&self, ctx: &Arc<dyn Context>) -> Option<&'static str> {
        if let Ok(ctx) = ctx.clone().downcast_arc::<sydarCli>() {
            ctx.daemons().clone().sydard.as_ref().map(|_| "node")
        } else {
            None
        }
    }
...
impl Node {
    ...
    async fn create_config(&self, ctx: &Arc<sydarCli>) -> Result<sydardConfig> {
        let location: String = self
            .settings
            .get(sydardSettings::Location)
        ...
        let mute = false;
        let config = sydardConfig::new(location.as_str(), network_id, mute);
        Ok(config)
    }

    async fn main(self: Arc<Self>, ctx: Arc<sydarCli>, mut argv: Vec<String>, cmd: &str) -> Result<()> {
        if argv.is_empty() {
            return self.display_help(ctx, argv).await;
        }
        let sydard = ctx.daemons().sydard();
        match argv.remove(0).as_str() {
            "start" => {
                let mute = self.mute.load(Ordering::SeqCst);
                if mute {
                    tprintln!(ctx, "starting sydar node... {}", style("(logs are muted, use 'node mute' to toggle)").dim());
                } else {
                    tprintln!(ctx, "starting sydar node... {}", style("(use 'node mute' to mute logging)").dim());
                }

                let wrpc_client = ctx.wallet().try_wrpc_client().ok_or(Error::custom("Unable to start node with non-wRPC client"))?;

                sydard.configure(self.create_config(&ctx).await?).await?;
                sydard.start().await?;
            ...
            "mute" | "logs" => {
                ...
                // sydard.mute(mute).await?;
                self.settings.set(sydardSettings::Mute, mute).await?;
            }
            "status" => {
                let status = sydard.status().await?;
                tprintln!(ctx, "{}", status);
            }
            "select" => {
            ...
            "version" => {
                sydard.configure(self.create_config(&ctx).await?).await?;
                let version = sydard.version().await?;
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<sydarCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("select", "Select sydard executable (binary) location"),
                ("version", "Display sydard executable version"),
                ("start", "Start the local sydar node instance"),
                ("stop", "Stop the local sydar node instance"),
                ("restart", "Restart the local sydar node instance"),
                ("kill", "Kill the local sydar node instance"),
                ("status", "Get the status of the local sydar node instance"),
    ...
    async fn select(self: Arc<Self>, ctx: Arc<sydarCli>, path: Option<String>) -> Result<()> {
        let root = nw_sys::app::folder();

        match path {
            None => {
                let binaries = sydar_daemon::locate_binaries(root.as_str(), "sydard").await?;

                if binaries.is_empty() {
                    tprintln!(ctx, "No sydard binaries found");
                } else {
                    let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
                    if let Some(selection) = ctx.term().select("Please select a sydard binary", &binaries).await? {
                        tprintln!(ctx, "selecting: {}", selection);
                        self.settings.set(sydardSettings::Location, selection.as_str()).await?;
                    } else {
            ...
            Some(path) => {
                if fs::exists(&path).await? {
                    ...
                    self.settings.set(sydardSettings::Location, path.as_str()).await?;
                } else {
                    twarnln!(ctx, "destination binary not found, please specify full path including the binary name");
                    twarnln!(ctx, "example: 'node select /home/user/testnet/sydard'");
                    tprintln!(ctx, "no selection is made");
                }
            }
        }
    ...
    pub async fn handle_event(&self, ctx: &Arc<sydarCli>, event: Event) -> Result<()> {
        let term = ctx.term();
        ...
        match event {
            ...
            Event::Exit(_code) => {
                tprintln!(ctx, "sydard has exited");
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
            Event::Error(error) => {
                tprintln!(ctx, "{}", style(format!("sydard error: {error}")).red());
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
