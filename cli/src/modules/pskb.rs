...
use sydar_wallet_core::account::pskb::finalize_pskt_one_or_more_sig_and_redeem_script;
use sydar_wallet_pskt::{
    prelude::{Bundle, PSKT, Signer, lock_script_sig_templating, script_sig_to_address, unlock_utxos_as_pskb},
    pskt::Inner,
};

#[derive(Default, Handler)]
#[help("Send a sydar transaction to a public address")]
pub struct Pskb;
    ...
    async fn main(self: Arc<Self>, ctx: &Arc<dyn Context>, mut argv: Vec<String>, _cmd: &str) -> Result<()> {
        ...
        match action.as_str() {
            "create" => {
                ...
                let amount_kana = try_parse_required_nonzero_sydar_as_kana_u64(argv.get(1))?;
                let outputs = PaymentOutputs::from((address, amount_kana));
                let priority_fee_kana = try_parse_optional_sydar_as_kana_i64(argv.get(2))?.unwrap_or(0);
            ...
            "script" => {
                ...
                match subcommand.as_str() {
                    "lock" => {
                        let amount_kana = try_parse_required_nonzero_sydar_as_kana_u64(argv.first())?;
                        let outputs = PaymentOutputs::from((script_p2sh, amount_kana));
                        // TODO fee_rate
                        let fee_rate = None;
                        let priority_fee_kana = try_parse_optional_sydar_as_kana_i64(argv.get(1))?.unwrap_or(0);
                        let abortable = Abortable::default();
                    ...
                    "unlock" => {
                        ...
                        // Get locked UTXO set.
                        let spend_utxos: Vec<sydar_rpc_core::RpcUtxosByAddressesEntry> =
                            ctx.wallet().rpc_api().get_utxos_by_addresses(vec![script_p2sh.clone()]).await?;
                        let priority_fee_kana = try_parse_optional_sydar_as_kana_i64(argv.first())?.unwrap_or(0) as u64;
                        ...
                        tprintln!(
                            ...
                            kana_to_sydar(total_locked_kana)
                        );
            ...
            "parse" => {
                ...
                let pskb = Self::parse_input_pskb(argv.first().unwrap().as_str())?;
                tprintln!(ctx, "{}", pskb.display_format(ctx.wallet().network_id()?, kana_to_sydar_string_with_suffix));
