...
#[help("Execute RPC commands against the connected sydar node")]
pub struct Rpc;

impl Rpc {
    fn println<T>(&self, ctx: &Arc<sydarCli>, v: T)
    where
        T: core::fmt::Debug,
    {
    ...
    async fn main(self: Arc<Self>, ctx: &Arc<dyn Context>, mut argv: Vec<String>, cmd: &str) -> Result<()> {
        ...
        match op {
            ...
            RpcApiOps::GetsydarDagInfo => {
                let result = rpc.get_sydar_dag_info_call(None, GetsydarDagInfoRequest {}).await?;
                self.println(&ctx, result);
            }
            ...
            RpcApiOps::GetBalanceByAddress => {
                ...
                for address in addresses {
                    ...
                    self.println(&ctx, kana_to_sydar(result.balance));
                }
            }
