use crate::cli::sydarCli;
use crate::imports::*;
use crate::result::Result;
use sydar_bip32::{Language, Mnemonic, WordCount};
use sydar_wallet_core::account::MULTISIG_ACCOUNT_KIND;
use sydar_wallet_core::storage::keydata::PrvKeyDataVariantKind;
// use sydar_wallet_core::runtime::wallet::AccountCreateArgsBip32;
// use sydar_wallet_core::runtime::{PrvKeyDataArgs, PrvKeyDataCreateArgs};
// use sydar_wallet_core::storage::AccountKind;
...
async fn create_multisig(ctx: &Arc<sydarCli>, account_name: Option<String>, mnemonic_phrase_word_count: WordCount) -> Result<()> {
    let term = ctx.term();
    let wallet = ctx.wallet();
...
pub(crate) async fn bip32_watch(ctx: &Arc<sydarCli>, name: Option<&str>) -> Result<()> {
    let term = ctx.term();
    let wallet = ctx.wallet();
...
pub(crate) async fn multisig_watch(ctx: &Arc<sydarCli>, name: Option<&str>) -> Result<()> {
    let term = ctx.term();
