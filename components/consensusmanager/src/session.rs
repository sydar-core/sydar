...
impl ConsensusSessionOwned {
    ...
    pub fn get_account_balance(&self, address: &sydar_addresses::Address) -> Option<u64> {
    ...
    pub async fn async_get_sydar_consensus_data(&self, hash: Hash) -> ConsensusResult<ExternalsydarConsensusData> {
        self.clone().spawn_blocking(move |c| c.get_sydar_consensus_data(hash)).await
    }
