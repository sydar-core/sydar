...
/// Calculates the k parameter of the sydar_CONSENSUS protocol such that anticones lager than k will be created
...
pub fn calculate_sydar_consensus_k(x: f64, delta: f64) -> u64 {
    assert!(x > 0.0);
...
impl<const BPS: u64> Bps<BPS> {
    ...
    /// Returns the sydar_CONSENSUS K value which was pre-computed for this BPS
    /// (see [`calculate_sydar_consensus_k`] and `gen_sydar_consensus_table` for the full calculation)
    #[rustfmt::skip]
    pub const fn sydar_consensus_k() -> KType {
        match BPS {
            ...
            31 => 352, 32 => 362,
            _ => panic!("see gen_sydar_consensus_table for currently supported values"),
        }
    }
    ...
    pub const fn max_block_parents() -> u8 {
        let val = (Self::sydar_consensus_k() / 2) as u8;
        if val < 10 {
            10
        } else if val > 16 {
            ...
            // be merged after a log number of rounds. For mainnet this requires an increase to the value of sydar_CONSENSUS
    ...
    pub const fn mergeset_size_limit() -> u64 {
        let val = Self::sydar_consensus_k() as u64 * 2;
        if val < 180 {
            180
        } else if val > 512 {
            // Bounded relatively low because we have storage complexity of O(#headers * mergeset_limit) coming
            // from reachability and sydar_CONSENSUS stores (sydar_CONSENSUS is avoidable but reachability is must).
            512
        } else {
            val
        }
    ...
    pub const fn pruning_depth() -> u64 {
        // Based on the analysis at https://github.com/sydarnet/docs/blob/main/Reference/prunality/Prunality.pdf
        ...
        let lower_bound = Self::finality_depth()
            ...
            + 4 * Self::mergeset_size_limit() * Self::sydar_consensus_k() as u64
            + 2 * Self::sydar_consensus_k() as u64
            + 2;
...
mod tests {
    ...
    #[ignore]
    #[test]
    fn gen_sydar_consensus_table() {
        println!("[BPS => K]");
        (1..=32).for_each(|bps| {
            let k = calculate_sydar_consensus_k(2.0 * NETWORK_DELAY_BOUND as f64 * bps as f64, sydar_CONSENSUS_TAIL_DELTA);
