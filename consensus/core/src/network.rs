...
pub enum NetworkIdError {
    #[error("Invalid network name prefix: {0}. The expected prefix is 'sydar'.")]
...
///
/// NetworkId is a unique identifier for a sydar network instance.
...
impl NetworkId {
    ...
    /// Returns a textual description of the network prefixed with `sydar-`
    pub fn to_prefixed(&self) -> String {
        format!("sydar-{}", self)
    }

    pub fn from_prefixed(prefixed: &str) -> Result<Self, NetworkIdError> {
        if let Some(stripped) = prefixed.strip_prefix("sydar-") {
