...
use sydar_consensus_core::{api::ConsensusApi, config::Config, mining_rules::MiningRules};
use sydar_consensus_notify::root::ConsensusNotificationRoot;
use sydar_consensusmanager::{ConsensusFactory, ConsensusInstance, DynConsensusCtl, SessionLock};
use sydar_core::{debug, time::unix_now, warn};
use sydar_database::{
    prelude::{
...
impl ConsensusFactory for Factory {
    fn new_active_consensus(&self) -> (ConsensusInstance, DynConsensusCtl) {
        ...
        let db = sydar_database::prelude::ConnBuilder::default()
            .with_db_path(dir)
