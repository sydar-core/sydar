...
use sydar_consensus_core::{
    ...
    trusted::{ExternalsydarConsensusData, TrustedBlock},
    tx::{
...
use sydar_consensusmanager::{SessionLock, SessionReadGuard};

use rayon::iter::{IntoParallelRefIterator, ParallelIterator};
use rocksdb::WriteBatch;
use sydar_core::info;
use sydar_database::prelude::StoreResultExt;
...
impl Consensus {
    ...
    fn estimate_network_hashes_per_second_impl(
        ...
        let window = match self.services.window_manager.block_window(sydar_consensus_data, WindowType::VaryingWindow(window_size)) {
            Ok(w) => w,
    ...
    pub fn intrusive_pruning_point_store_writes(
        ...
        let virtual_state = Arc::new(VirtualState {
            ...
            sydar_consensus_data: self.services.sydar_consensus_manager.sydar_consensus(&virtual_parents),
    ...
    fn get_and_verify_path_to_new_pruning_point(&self, new_pruning_point: Hash, syncer_sink: Hash) -> ConsensusResult<VecDeque<Hash>> {
        ...
        let Ok(candidate_sydar_consensus_data) = self.get_sydar_consensus_data(new_pruning_point) else {
        ...
        let Ok(selected_parent_sydar_consensus_data) =
            self.get_sydar_consensus_data(candidate_sydar_consensus_data.selected_parent)
        else {
            return Err(ConsensusError::General(
                "Catchup cannot be continued since the syncer pruning point could not be confirmed to be a valid pruning point",
            ));
        ...
        self.services
            ...
            .is_pruning_sample(
                candidate_sydar_consensus_data.blue_score,
                selected_parent_sydar_consensus_data.blue_score,
...
impl ConsensusApi for Consensus {
    ...
    fn get_account_balance(&self, address: &sydar_addresses::Address) -> Option<u64> {
    ...
    // sydar Account Model: Bypass parallel mempool population
    ...
    fn get_virtual_merge_depth_root(&self) -> Option<Hash> {
        ...
        let virtual_sydar_consensus_data = &virtual_state.sydar_consensus_data;
        let root = self.services.depth_manager.calc_merge_depth_root(virtual_sydar_consensus_data, pruning_point);
    ...
    fn get_virtual_merge_depth_blue_work_threshold(&self) -> BlueWorkType {
        ...
        self.get_virtual_merge_depth_root()
            .map_or(BlueWorkType::ZERO, |root| self.sydar_consensus_store.get_blue_work(root).unwrap())
    }

    fn get_sink(&self) -> Hash {
        self.lkg_virtual_state.load().sydar_consensus_data.selected_parent
    }
    ...
    fn get_current_block_color(&self, hash: Hash) -> Option<bool> {
        ...
        // Verify that the block is in future(retention root), where sydarConsensus data is complete
        ...
        for child in initial_children {
            if visited.insert(child) {
                let blue_work = self.sydar_consensus_store.get_blue_work(child).unwrap();
        ...
        while let Some(Reverse(SortableBlock { hash: decedent, .. })) = heap.pop() {
            if self.services.reachability_service.is_chain_ancestor_of(decedent, sink) {
                let decedent_data = self.get_sydar_consensus_data(decedent).unwrap();
                ...
                // unreachable.
                sydar_core::warn!("DAG topology inconsistency: {decedent} is expected to be a merging block of {hash}");
    ...
    fn get_chain_block_samples(&self) -> Vec<DaaScoreTimestamp> {
        ...
        if self.config.net.network_type == NetworkType::Mainnet {
            ...
            // Source: https://github.com/sydargang/sydard-py-explorer/blob/main/src/tx_timestamp_estimation.ipynb
            // For context see also: https://github.com/sydargang/sydard-py-explorer/blob/main/src/genesis_proof.ipynb
    ...
    fn get_virtual_utxos(
        ...
        // sydar Account Model: UTXO iterator is disabled.
    ...
    fn get_sydar_consensus_data(&self, hash: Hash) -> ConsensusResult<ExternalsydarConsensusData> {
        ...
        let sydar_consensus =
            self.sydar_consensus_store.get_data(hash).optional().unwrap().ok_or(ConsensusError::MissingData(hash))?;
        Ok((&*sydar_consensus).into())
    }
    ...
    fn estimate_network_hashes_per_second(&self, start_hash: Option<Hash>, window_size: usize) -> ConsensusResult<u64> {
        ...
        match start_hash {
            Some(hash) => {
                ...
                if !self.headers_store.has(sydar_consensus_data.selected_parent).unwrap() {
                    return Err(ConsensusError::DifficultyError(DifficultyError::InsufficientWindowData(0)));
                }
                self.estimate_network_hashes_per_second_impl(&sydar_consensus_data, window_size)
            }
            None => {
                let virtual_state = self.lkg_virtual_state.load();
                self.estimate_network_hashes_per_second_impl(&virtual_state.sydar_consensus_data, window_size)
            }
        }
    }
    ...
    fn finality_point(&self) -> Hash {
        self.virtual_processor.virtual_finality_point(&self.lkg_virtual_state.load().sydar_consensus_data, self.pruning_point())
    }
