use crate::{
    ...
    model::stores::{
        ...
        account_store::DbAccountStore, // <--- 1. IMPORT sydar BANK
        ...
        sydar_consensus::{CompactsydarConsensusData, DbsydarConsensusStore},
    ...
    processes::{reachability::inquirer as reachability, relations, sydar_consensus::ordering::SortableBlock},
};

use super::cache_policy_builder::CachePolicyBuilder as PolicyBuilder;
use parking_lot::RwLock;
use sydar_consensus_core::{BlockHashSet, blockstatus::BlockStatus};
...
pub struct ConsensusStorage {
    ...
    // Append-only stores
    pub sydar_consensus_store: Arc<DbsydarConsensusStore>,
...
impl ConsensusStorage {
    pub fn new(db: Arc<DB>, config: Arc<Config>) -> Arc<Self> {
        ...
        let sydar_consensus_compact_bytes = size_of::<Hash>() + size_of::<CompactsydarConsensusData>();
        ...
        let reachability_data_builder = PolicyBuilder::new()
            ...
            .untracked();
        let sydar_consensus_compact_builder = PolicyBuilder::new()
            .max_items(pruning_size_for_caches)
            .bytes_budget(sydar_consensus_compact_budget)
            .unit_bytes(sydar_consensus_compact_bytes)
        ...
        let median_window_builder = PolicyBuilder::new()
            ...
            .untracked();
        let sydar_consensus_builder =
            PolicyBuilder::new().bytes_budget(sydar_consensus_budget).min_items(level_lower_bound).tracked_bytes();
        ...
        let sydar_consensus_store = Arc::new(DbsydarConsensusStore::new(
            db.clone(),
            0,
            sydar_consensus_builder.downscale(0).build(),
            sydar_consensus_compact_builder.downscale(0).build(),
        ));
        ...
        // Initialize the Account Store (sydar Bank) using the utxo_set_cache_size for cache allocation
