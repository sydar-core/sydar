...
use sydar_consensus_notify::{notification::Notification, root::ConsensusNotificationRoot};
use sydar_consensusmanager::{ConsensusFactory, ConsensusInstance, DynConsensusCtl};
use sydar_core::{core::Core, service::Service};
use sydar_database::utils::DbLifetime;
use sydar_hashes::Hash;
use sydar_notify::subscription::context::SubscriptionContext;

use super::Consensus;
use super::services::{DbDagTraversalManager, DbsydarConsensusManager, DbWindowManager};
...
use crate::{
    ...
    model::{
        ...
        stores::{
            DB, headers::HeaderStoreReader, reachability::DbReachabilityStore, sydar_consensus::DbsydarConsensusStore,
...
impl TestConsensus {
    ...
    pub fn build_header_with_parents(&self, hash: Hash, parents: Vec<Hash>) -> Header {
        ...
        let sydar_consensus_data = self.consensus.services.sydar_consensus_manager.sydar_consensus(header.direct_parents());
        let daa_window = self.consensus.services.window_manager.block_daa_window(&sydar_consensus_data).unwrap();
        header.bits = self.consensus.services.window_manager.calculate_difficulty_bits(&sydar_consensus_data, &daa_window);
        header.daa_score = daa_window.daa_score;
        header.timestamp = self.consensus.services.window_manager.calc_past_median_time(&sydar_consensus_data).unwrap().0 + 1;
        header.blue_score = sydar_consensus_data.blue_score;
        header.blue_work = sydar_consensus_data.blue_work;
    ...
    /// See `sydar_consensus_core::errors::block::RuleError` for the complete list of possible validation rules.
    ...
    pub fn sydar_consensus_store(&self) -> &Arc<DbsydarConsensusStore> {
        &self.consensus.sydar_consensus_store
    }
    ...
    pub fn sydar_consensus_manager(&self) -> &DbsydarConsensusManager {
        &self.consensus.services.sydar_consensus_manager
    }
}
