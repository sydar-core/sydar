...
use sydar_consensus_core::{BlockHashMap, BlockHasher, BlockLevel, blockhash::BlockHashes};
use sydar_database::prelude::{BatchDbWriter, CachePolicy, DbWriter};
use sydar_database::prelude::{CachedDbAccess, DbKey, DirectDbWriter};
use sydar_database::prelude::{DB, StoreResult};
use sydar_database::prelude::{DirectWriter, MemoryWriter};
use sydar_database::prelude::{ReadLock, StoreError};
...
impl RelationsStoreReader for StagingRelationsStore<'_> {
    ...
    fn counts(&self) -> Result<(usize, usize), StoreError> {
        let count = self
            ...
            .iterator()
            .map(|r| r.unwrap().0)
            .map(|k| <[u8; sydar_hashes::HASH_SIZE]>::try_from(&k[..]).unwrap())
            .map(Hash::from_bytes)
...
mod tests {
    ...
    use sydar_utils::mem_size::MemMode;
    ...
    fn test_db_relations_store() {
        let (lt, db) = create_temp_db!(sydar_database::prelude::ConnBuilder::default().with_files_limit(10));
