use crate::processes::sydar_consensus::ordering::SortableBlock;
use sydar_consensus_core::trusted::ExternalsydarConsensusData;
use sydar_consensus_core::{BlockHashMap, BlockHasher, BlockLevel, HashMapCustomHasher};
use sydar_consensus_core::{BlueWorkType, blockhash::BlockHashes};
use sydar_database::prelude::DB;
use sydar_database::prelude::{BatchDbWriter, CachedDbAccess, DbKey};
use sydar_database::prelude::{CachePolicy, StoreError};
use sydar_database::registry::{DatabaseStorePrefixes, SEPARATOR};
...
pub use sydar_consensus_core::{HashKTypeMap, KType};

#[derive(Clone, Serialize, Deserialize, Default)]
pub struct sydarConsensusData {
    pub blue_score: u64,
...
pub struct CompactsydarConsensusData {
    pub blue_score: u64,
    pub blue_work: BlueWorkType,
    pub selected_parent: Hash,
}

impl MemSizeEstimator for sydarConsensusData {
...
impl MemSizeEstimator for CompactsydarConsensusData {}

impl From<&sydarConsensusData> for CompactsydarConsensusData {
    #[inline(always)]
    fn from(value: &sydarConsensusData) -> Self {
        Self { blue_score: value.blue_score, blue_work: value.blue_work, selected_parent: value.selected_parent }
    }
}

impl From<ExternalsydarConsensusData> for sydarConsensusData {
    fn from(value: ExternalsydarConsensusData) -> Self {
        Self {
...
impl From<&sydarConsensusData> for ExternalsydarConsensusData {
    fn from(value: &sydarConsensusData) -> Self {
        Self {
            blue_score: value.blue_score,
            blue_work: value.blue_work,
...
impl sydarConsensusData {
    ...
    pub fn ascending_mergeset_without_selected_parent<'a>(
        &'a self,
        store: &'a (impl sydarConsensusStoreReader + ?Sized),
    ...
    pub fn to_compact(&self) -> CompactsydarConsensusData {
        self.into()
    }
...
pub trait sydarConsensusStoreReader {
    ...
    fn get_data(&self, hash: Hash) -> Result<Arc<sydarConsensusData>, StoreError>;

    fn get_compact_data(&self, hash: Hash) -> Result<CompactsydarConsensusData, StoreError>;

    /// Check if the store contains data for the requested hash
    fn has(&self, hash: Hash) -> Result<bool, StoreError>;
}

pub trait sydarConsensusStore: sydarConsensusStoreReader {
    /// Insert sydar_CONSENSUS data for block `hash` into the store. Note that sydar_CONSENSUS data
    ...
    fn insert(&self, hash: Hash, data: Arc<sydarConsensusData>) -> Result<(), StoreError>;
    fn delete(&self, hash: Hash) -> Result<(), StoreError>;
}

/// A DB + cache implementation of `sydarConsensusStore` trait, with concurrency support.
#[derive(Clone)]
pub struct DbsydarConsensusStore {
    db: Arc<DB>,
    level: BlockLevel,
    access: CachedDbAccess<Hash, Arc<sydarConsensusData>, BlockHasher>,
    compact_access: CachedDbAccess<Hash, CompactsydarConsensusData, BlockHasher>,
}

impl DbsydarConsensusStore {
    pub fn new(db: Arc<DB>, level: BlockLevel, cache_policy: CachePolicy, compact_cache_policy: CachePolicy) -> Self {
        ...
        let prefix = DatabaseStorePrefixes::sydarConsensus.into_iter().chain(lvl_bytes).collect_vec();
        let compact_prefix = DatabaseStorePrefixes::sydarConsensusCompact.into_iter().chain(lvl_bytes).collect_vec();
        Self {
            db: Arc::clone(&db),
            level,
    ...
    pub fn new_temp(
        ...
        let prefix = DatabaseStorePrefixes::TempsydarConsensus.into_iter().chain(lvl_bytes).chain(temp_index_bytes).collect_vec();
        let compact_prefix =
            DatabaseStorePrefixes::TempsydarConsensusCompact.into_iter().chain(lvl_bytes).chain(temp_index_bytes).collect_vec();
        Self {
            db: Arc::clone(&db),
            level,
            access: CachedDbAccess::new(db.clone(), cache_policy, prefix),
    ...
    pub fn insert_batch(&self, batch: &mut WriteBatch, hash: Hash, data: &Arc<sydarConsensusData>) -> Result<(), StoreError> {
    ...
    pub fn update_batch(&self, batch: &mut WriteBatch, hash: Hash, data: &Arc<sydarConsensusData>) -> Result<(), StoreError> {
        self.access.write(BatchDbWriter::new(batch), hash, data.clone())?;
        self.compact_access.write(BatchDbWriter::new(batch), hash, data.to_compact())?;
        Ok(())
    }
...
impl sydarConsensusStoreReader for DbsydarConsensusStore {
    fn get_blue_score(&self, hash: Hash) -> Result<u64, StoreError> {
        if let Some(sydar_consensus_data) = self.access.read_from_cache(hash) {
            return Ok(sydar_consensus_data.blue_score);
        }
    ...
    fn get_blue_work(&self, hash: Hash) -> Result<BlueWorkType, StoreError> {
        if let Some(sydar_consensus_data) = self.access.read_from_cache(hash) {
            return Ok(sydar_consensus_data.blue_work);
        }
    ...
    fn get_selected_parent(&self, hash: Hash) -> Result<Hash, StoreError> {
        if let Some(sydar_consensus_data) = self.access.read_from_cache(hash) {
            return Ok(sydar_consensus_data.selected_parent);
        }
...
impl sydarConsensusStore for DbsydarConsensusStore {
    fn insert(&self, hash: Hash, data: Arc<sydarConsensusData>) -> Result<(), StoreError> {
        if self.access.has(hash)? {
            return Err(StoreError::HashAlreadyExists(hash));
        }
...
/// An in-memory implementation of `sydarConsensusStore` trait to be used for tests.
/// Uses `RefCell` for interior mutability in order to workaround `insert`
/// being non-mutable.
pub struct MemorysydarConsensusStore {
...
impl MemorysydarConsensusStore {
    pub fn new() -> Self {
        Self {
    ...
    pub fn key_not_found_error(hash: Hash) -> StoreError {
        StoreError::KeyNotFound(DbKey::new(DatabaseStorePrefixes::sydarConsensus.as_ref(), hash))
    }
}

impl Default for MemorysydarConsensusStore {
    fn default() -> Self {
        Self::new()
    }
}

impl sydarConsensusStore for MemorysydarConsensusStore {
    fn insert(&self, hash: Hash, data: Arc<sydarConsensusData>) -> Result<(), StoreError> {
        if self.has(hash)? {
            return Err(StoreError::HashAlreadyExists(hash));
        }
...
impl sydarConsensusStoreReader for MemorysydarConsensusStore {
    fn get_blue_score(&self, hash: Hash) -> Result<u64, StoreError> {
    ...
    fn get_data(&self, hash: Hash) -> Result<Arc<sydarConsensusData>, StoreError> {
        ...
        Ok(Arc::new(sydarConsensusData::new(
...
mod tests {
    ...
    fn test_mergeset_iterators() {
        let store = MemorysydarConsensusStore::new();

        let factory = |w: u64| {
            Arc::new(sydarConsensusData {
                blue_score: Default::default(),
                blue_work: w.into(),
        ...
        let mut data = sydarConsensusData::new_with_selected_parent(1.into(), 5);
