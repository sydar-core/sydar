...
impl HeaderProcessor {
    ...
    pub fn check_median_timestamp(&self, ctx: &mut HeaderProcessingContext, header: &Header) -> BlockProcessResult<()> {
        let (past_median_time, window) = self.window_manager.calc_past_median_time(ctx.sydar_consensus_data())?;
    ...
    pub fn check_mergeset_size_limit(&self, ctx: &mut HeaderProcessingContext) -> BlockProcessResult<()> {
        let mergeset_size = ctx.sydar_consensus_data().mergeset_size() as u64;
    ...
    fn check_blue_score(&self, ctx: &mut HeaderProcessingContext, header: &Header) -> BlockProcessResult<()> {
        let gd_blue_score = ctx.sydar_consensus_data().blue_score;
    ...
    fn check_blue_work(&self, ctx: &mut HeaderProcessingContext, header: &Header) -> BlockProcessResult<()> {
        let gd_blue_work = ctx.sydar_consensus_data().blue_work;
    ...
    pub fn check_bounded_merge_depth(&self, ctx: &mut HeaderProcessingContext) -> BlockProcessResult<()> {
        let sydar_consensus_data = ctx.sydar_consensus_data();
        let merge_depth_root = self.depth_manager.calc_merge_depth_root(sydar_consensus_data, ctx.pruning_point);
        let finality_point = self.depth_manager.calc_finality_point(sydar_consensus_data, ctx.pruning_point);
