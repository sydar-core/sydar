use crate::{
    ...
    processes::{
        reachability::inquirer as reachability, relations::RelationsStoreExtensions, sydar_consensus::ordering::SortableBlock,
    },
};
...
use sydar_database::prelude::{StoreResultExt, StoreResultUnitExt};
...
pub struct HeaderProcessingContext {
    ...
    // Staging data
    pub sydar_consensus_data: Option<Arc<sydarConsensusData>>,
...
impl HeaderProcessingContext {
    ...
    /// Returns the primary (level 0) sydar_CONSENSUS data of this header.
    /// NOTE: expected to be called only after sydar_CONSENSUS computation was pushed into the context
    pub fn sydar_consensus_data(&self) -> &Arc<sydarConsensusData> {
        self.sydar_consensus_data.as_ref().unwrap()
    }
}
...
impl HeaderProcessor {
    ...
    // Runs partial header validation for trusted blocks (currently validates only header-in-isolation and computes sydar_CONSENSUS).
    ...
    /// Runs the sydar_CONSENSUS algorithm and writes the data into the context (if hasn't run already)
    fn sydar_consensus(&self, ctx: &mut HeaderProcessingContext) {
        let sydar_consensus_data = self
            .sydar_consensus_store
            .get_data(ctx.hash)
            .optional()
            .unwrap()
            .unwrap_or_else(|| Arc::new(self.sydar_consensus_manager.sydar_consensus(&ctx.known_direct_parents)));
        self.counters.mergeset_counts.fetch_add(sydar_consensus_data.mergeset_size() as u64, Ordering::Relaxed);
        ctx.sydar_consensus_data = Some(sydar_consensus_data);
    }

    fn commit_header(&self, ctx: HeaderProcessingContext, header: &Header) {
        let sydar_consensus_data = ctx.sydar_consensus_data.as_ref().unwrap();
        ...
        //
        self.sydar_consensus_store.insert_batch(&mut batch, ctx.hash, sydar_consensus_data).unwrap();
        ...
        let selected_parent = sydar_consensus_data.selected_parent;
        let mut reachability_mergeset = sydar_consensus_data.unordered_mergeset_without_selected_parent();
    ...
    fn commit_trusted_header(&self, ctx: HeaderProcessingContext, _header: &Header) {
        ...
        self.sydar_consensus_store.insert_batch(&mut batch, ctx.hash, sydar_consensus_data).idempotent().unwrap();
    ...
    pub fn process_genesis(&self) {
        ...
        ctx.sydar_consensus_data = Some(Arc::new(self.sydar_consensus_manager.genesis_sydar_consensus_data()));
    ...
    pub fn init(&self) {
        ...
        self.sydar_consensus_store
            .insert_batch(&mut batch, ORIGIN, &self.sydar_consensus_manager.origin_sydar_consensus_data())
            .unwrap();
        let mut hst_write = self.headers_selected_tip_store.write();
