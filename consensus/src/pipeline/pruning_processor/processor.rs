...
use crate::{
    ...
    model::{
        ...
        stores::{
            ...
            sydar_consensus::{CompactsydarConsensusData, sydarConsensusStoreReader},
...
use sydar_consensus_core::{
    ...
    trusted::ExternalsydarConsensusData,
};
use sydar_consensusmanager::SessionLock;
use sydar_core::{debug, info, trace, warn};
use sydar_database::prelude::{BatchDbWriter, DB, MemoryWriter, StoreResultExt};
use sydar_hashes::Hash;
use sydar_muhash::MuHash;
use sydar_utils::iter::IterExtensions;
use std::{
...
pub enum PruningProcessingMessage {
    Exit,
    Process { sink_sydar_consensus_data: CompactsydarConsensusData },
}
...
impl PruningProcessor {
    ...
    pub fn worker(self: &Arc<Self>) {
        ...
        while let Ok(PruningProcessingMessage::Process { sink_sydar_consensus_data }) = self.receiver.recv() {
            if !recovered {
            ...
            self.advance_pruning_point_if_possible(sink_sydar_consensus_data);
        }
    }
    ...
    fn advance_pruning_point_if_possible(&self, sink_sydar_consensus_data: CompactsydarConsensusData) {
        let pruning_point_read = self.pruning_point_store.upgradable_read();
        let (current_pruning_point, current_index) = pruning_point_read.pruning_point_and_index().unwrap();
        let new_pruning_points = self.pruning_point_manager.next_pruning_points(sink_sydar_consensus_data, current_pruning_point);
    ...
    fn prune(&self, new_pruning_point: Hash, retention_period_root: Hash) {
        ...
        let mut keep_relations: BlockHashMap<BlockLevel> = std::iter::empty()
            ...
            .chain(data.sydar_consensus_blocks.iter().map(|gd| gd.hash))
...

        {
            let mut counter = 0;
            let mut batch = WriteBatch::default();
            // At this point keep_relations only holds level-0 relations which is the correct filtering criteria for primary sydar_CONSENSUS
            for kept in keep_relations.keys().copied() {
                let Some(sydar_consensus) = self.sydar_consensus_store.get_data(kept).optional().unwrap() else {
                    continue;
                };
                if sydar_consensus.unordered_mergeset().any(|h| !keep_relations.contains_key(&h)) {
                    let mut mutable_sydar_consensus: ExternalsydarConsensusData = sydar_consensus.as_ref().into();
                    mutable_sydar_consensus.mergeset_blues.retain(|h| keep_relations.contains_key(h));
                    mutable_sydar_consensus.mergeset_reds.retain(|h| keep_relations.contains_key(h));
                    mutable_sydar_consensus.blues_anticone_sizes.retain(|k, _| keep_relations.contains_key(k));
                    if !keep_relations.contains_key(&mutable_sydar_consensus.selected_parent) {
                        mutable_sydar_consensus.selected_parent = ORIGIN;
                    }
                    counter += 1;
                    self.sydar_consensus_store
                        .update_batch(&mut batch, kept, &Arc::new(mutable_sydar_consensus.into()))
                        .unwrap();
                }
            }
            self.db.write(batch).unwrap();
            info!("Header and Block pruning: updated sydar_consensus data for {} blocks", counter);
        }

        // No need to hold the prune guard while we continue populating keep_relations
        drop(prune_guard);

        // Add additional levels only after filtering sydar_CONSENSUS data via level 0
        ...
        while let Some(current) = queue.pop_front() {
            ...
            if !keep_blocks.contains(&current) {
                ...
                if let Some(&affiliated_proof_level) = keep_relations.get(&current) {
                    ...
                    // delete relations and sydar_consensus unless current is in level 0 of the pruning proof
                    if affiliated_proof_level > 0 {
                        ...
                        self.sydar_consensus_store.delete_batch(&mut batch, current).optional().unwrap();
                    }
                    ...
                    counter += 1;
                    // Prune data related to headers: relations, reachability, sydar_consensus
    ...
    fn confirm_pruning_depth_below_virtual(&self, pruning_point: Hash) -> bool {
        ...
        virtual_state.sydar_consensus_data.blue_score >= pp_bs + self.config.params.pruning_depth()
    }
    ...
    fn assert_data_rebuilding(&self, ref_data: Arc<PruningPointTrustedData>, new_pruning_point: Hash) {
        ...
        assert_eq!(
            ref_data.sydar_consensus_blocks.iter().map(|gd| gd.hash).collect::<BlockHashSet>(),
            built_data.sydar_consensus_blocks.iter().map(|gd| gd.hash).collect::<BlockHashSet>()
        );
