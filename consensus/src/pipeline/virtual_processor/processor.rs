use crate::{
    consensus::{
        services::{
            ...
            DbsydarConsensusManager, DbWindowManager,
        },
    ...
    model::{
        ...
        stores::{
            ...
            sydar_consensus::{DbsydarConsensusStore, sydarConsensusData, sydarConsensusStoreReader},
    ...
    processes::{
        ...
        sydar_consensus::ordering::SortableBlock,
...
use sydar_consensusmanager::SessionLock;
use sydar_core::{debug, info, time::unix_now, trace, warn};
use sydar_database::prelude::{StoreError, StoreResultExt, StoreResultUnitExt};
use sydar_dilithium::{DilithiumKeyPair, DilithiumSignature, PUBKEY_SIZE, sydar_MODE, SIG_SIZE};
use sydar_hashes::{Hash, ZERO_HASH};
use sydar_muhash::MuHash;
use sydar_notify::{events::EventType, notifier::Notify};
...
use sydar_utils::binary_heap::BinaryHeapExtensions;
use std::{
    cmp::min,
...
pub struct VirtualStateProcessor {
    ...
    pub(super) sydar_consensus_store: Arc<DbsydarConsensusStore>,
    ...
    pub(super) sydar_consensus_manager: DbsydarConsensusManager,
...
impl VirtualStateProcessor {
    ...
    pub fn new(
        ...
        Self {
            ...
            sydar_consensus_store: storage.sydar_consensus_store.clone(),
            ...
            sydar_consensus_manager: services.sydar_consensus_manager.clone(),
    ...
    fn resolve_virtual(self: &Arc<Self>) {
        ...
        let finality_point = self.virtual_finality_point(&prev_state.sydar_consensus_data, pruning_point);
        ...
        let tips = self
            ...
            .collect_vec();
        drop(prune_guard);
        let prev_sink = prev_state.sydar_consensus_data.selected_parent;
        let mut accumulated_diff = sydar_consensus_core::utxo::utxo_diff::UtxoDiff::default();
        ...
        let (virtual_parents, virtual_sydar_consensus_data) =
            self.pick_virtual_parents(new_sink, virtual_parent_candidates, pruning_point);
        assert_eq!(virtual_sydar_consensus_data.selected_parent, new_sink);
        ...
        let sink_sydar_consensus_data = Lazy::new(|| self.sydar_consensus_store.get_data(new_sink).unwrap());
        // Cache the DAA and Median time windows of the sink for future use, as well as prepare for virtual's window calculations
        self.cache_sink_windows(new_sink, prev_sink, &sink_sydar_consensus_data);

        let new_virtual_state = self
            .calculate_and_commit_virtual_state(
                virtual_read,
                virtual_parents,
                virtual_sydar_consensus_data,
                sink_multiset,
        ...
        let compact_sink_sydar_consensus_data = if let Some(sink_sydar_consensus_data) = Lazy::get(&sink_sydar_consensus_data)
        {
            // If we had to retrieve the full data, we convert it to compact
            sink_sydar_consensus_data.to_compact()
        } else {
            // Else we query the compact data directly.
            self.sydar_consensus_store.get_compact_data(new_sink).unwrap()
        };
        ...
        self.pruning_sender
            .send(PruningProcessingMessage::Process { sink_sydar_consensus_data: compact_sink_sydar_consensus_data })
            .unwrap();
        ...
        self.notification_root
            .notify(Notification::SinkBlueScoreChanged(SinkBlueScoreChangedNotification::new(
                compact_sink_sydar_consensus_data.blue_score,
            )))
            .expect("expecting an open unbounded channel");
        self.notification_root
    ...
    pub(crate) fn virtual_finality_point(&self, virtual_sydar_consensus_data: &sydarConsensusData, pruning_point: Hash) -> Hash {
        let finality_point = self.depth_manager.calc_finality_point(virtual_sydar_consensus_data, pruning_point);
        ...
        for (selected_parent, current) in self.reachability_service.forward_chain_iterator(split_point, to, true).tuple_windows() {
            ...
            match self.utxo_diffs_store.get(current) {
                ...
                Err(StoreError::KeyNotFound(_)) => {
                    ...
                    let mergeset_data = self.sydar_consensus_store.get_data(current).unwrap();
                    let pov_daa_score = header.daa_score;

                    let selected_parent_multiset_hash = self.utxo_multisets_store.get(selected_parent).unwrap();
                    let selected_parent_utxo_view = sydar_consensus_core::utxo::utxo_collection::UtxoCollection::default(); // DHYAN DE: Maine '_' hata diya!
    ...
    fn calculate_and_commit_virtual_state(
        ...
        virtual_sydar_consensus_data: sydarConsensusData,
        selected_parent_multiset: MuHash,
        accumulated_diff: &mut UtxoDiff,
        chain_path: &ChainPath,
    ) -> Result<Arc<VirtualState>, RuleError> {
        let new_virtual_state =
            self.calculate_virtual_state(&virtual_read, virtual_parents, virtual_sydar_consensus_data, selected_parent_multiset)?;
    ...
    pub(crate) fn calculate_virtual_state(
        ...
        let virtual_daa_window = self.window_manager.block_daa_window(&virtual_sydar_consensus_data)?;
        let virtual_bits = self.window_manager.calculate_difficulty_bits(&virtual_sydar_consensus_data, &virtual_daa_window);
        let virtual_past_median_time = self.window_manager.calc_past_median_time(&virtual_sydar_consensus_data)?.0;

        // Use Default to avoid Hash-Registry errors
        let mergeset_rewards = sydar_consensus_core::BlockHashMap::default();

        let empty_miner_data = sydar_consensus_core::coinbase::MinerData {
            script_public_key: sydar_consensus_core::tx::ScriptPublicKey::new(0, sydar_consensus_core::tx::ScriptVec::new()),
            extra_data: vec![],
        };
    ...
    fn commit_virtual_state(
        ...
        // ---------------------------------------------------------
        // sydar ACCOUNT MODEL: TRANSACTION EXECUTION ENGINE
        // ---------------------------------------------------------
                ...
                for (i, tx) in txs.iter().enumerate() {
                    ...
                    for output in tx.outputs.iter() {
                        ...
                        self.account_store
                            ...
                            .expect("sydar: CRITICAL — failed to reverse balance during reorg, state may be inconsistent");
                    }
                ...
                for (i, tx) in txs.iter().enumerate() {
                        ...
                        if let Ok(coinbase_data) = self.coinbase_manager.deserialize_coinbase_payload(&tx.payload) {
                            ...
                            if total_reward > 0 {
                                ...
                                self.account_store
                                    ...
                                    .expect("sydar: CRITICAL — failed to credit miner reward, state may be inconsistent");

                                // TODO: Treasury address ko string se ScriptPublicKey me convert karke dev_fee bhi add karni hai

                                println!("sydar REWARD: Added {} Kana to Miner! (Dev Fee: {})", miner_reward, dev_fee);
                            }
                        } else {
                            println!("sydar REWARD ERROR: Failed to decode coinbase payload!");
                        }
                    }
                    // ==========================================
                    ...
                    // ==========================================
                        ...
                        if tx.payload.len() < min_payload {
                            panic!(
                                "sydar: CRITICAL — account tx undersized payload ({}) in commit_virtual_state",
                                tx.payload.len()
                            );
                        }
                        ...
                        // Defense-in-depth: re-verify Dilithium signature
                        {
                            ...
                            assert!(
                                DilithiumKeyPair::verify(sender_pubkey, &sig, &sighash, b"", sydar_MODE),
                        ...
                        // Verify nonce
                        let current_nonce =
                            self.account_store.get_nonce(&sender_spk).expect("sydar: CRITICAL — failed to read nonce");
                        assert_eq!(
                            expected_nonce, current_nonce,
                            "sydar: CRITICAL — nonce mismatch (have {}, tx claims {})",
                        ...
                        total_spent += tx.gas;
                        let balance =
                            self.account_store.get_balance(&sender_spk).expect("sydar: CRITICAL — failed to read balance");
                        assert!(
                            balance >= total_spent,
                            "sydar: CRITICAL — insufficient balance (have {}, need {})",
                            balance,
                            total_spent
                        );
    ...
    fn cache_sink_windows(
        ...
        new_sink: Hash,
        prev_sink: Hash,
        sink_sydar_consensus_data: &impl Deref<Target = Arc<sydarConsensusData>>,
    ) {
        ...
        if new_sink != prev_sink {
            ...
            if !self.block_window_cache_for_difficulty.contains_key(&new_sink) {
                self.block_window_cache_for_difficulty
                    .insert(new_sink, self.window_manager.block_daa_window(sink_sydar_consensus_data.deref()).unwrap().window);
            };

            if !self.block_window_cache_for_past_median_time.contains_key(&new_sink) {
                self.block_window_cache_for_past_median_time
                    .insert(new_sink, self.window_manager.calc_past_median_time(sink_sydar_consensus_data.deref()).unwrap().1);
            };
        }
    }
    ...
    pub(super) fn sink_search_algorithm(
        ...
        let mut heap = tips
            .into_iter()
            .map(|block| SortableBlock { hash: block, blue_work: self.sydar_consensus_store.get_blue_work(block).unwrap() })
        ...
        loop {
            ...
            if self.reachability_service.is_chain_ancestor_of(finality_point, candidate) {
                ...
                if diff_point == candidate {
                    ...
                    let filtering_blue_work = self.sydar_consensus_store.get_blue_work(filtering_root).unwrap_or_default();
                    return (
                        candidate,
            ...
            for parent in self.relations_service.get_parents(candidate).unwrap().iter().copied() {
                ...
                {
                    heap.push(SortableBlock { hash: parent, blue_work: self.sydar_consensus_store.get_blue_work(parent).unwrap() });
                }
            }
            drop(prune_guard);
        }
    ...
    fn remove_bounded_merge_breaking_parents(
        ...
        let mut sydar_consensus_data = self.sydar_consensus_manager.sydar_consensus(&virtual_parents);
        let merge_depth_root = self.depth_manager.calc_merge_depth_root(&sydar_consensus_data, current_pruning_point);
        ...
        for red in sydar_consensus_data.mergeset_reds.iter().copied() {
            ...
            if kosherizing_blues.is_none() {
                kosherizing_blues = Some(self.depth_manager.kosherizing_blues(&sydar_consensus_data, merge_depth_root).collect());
            }
        ...
        if !bad_reds.is_empty() {
            ...
            // Recompute sydar_consensus data since parents changed
            sydar_consensus_data = self.sydar_consensus_manager.sydar_consensus(&virtual_parents);
        }

        (virtual_parents, sydar_consensus_data)
    }
    ...
    pub fn validate_mempool_transaction(&self, mutable_tx: &mut MutableTransaction, args: &TransactionValidationArgs) -> TxResult<()> {
        ...
        let dummy_utxo_view = sydar_consensus_core::utxo::utxo_collection::UtxoCollection::default(); // sydar ACCOUNT BYPASS
    ...
    pub fn build_block_template(
        ...
        let virtual_utxo_view = sydar_consensus_core::utxo::utxo_collection::UtxoCollection::default();
    ...
    pub(crate) fn validate_block_template_transactions(
        ...
        // sydar ACCOUNT MODEL: All transactions are considered valid at this stage.
        Ok(())
    }
    ...
    pub(crate) fn build_block_template_from_virtual_state(
        ...
        let header_pruning_point =
            self.pruning_point_manager.expected_header_pruning_point(virtual_state.sydar_consensus_data.to_compact()).pruning_point;
        let _coinbase = self
            .coinbase_manager
            .expected_coinbase_transaction(
                virtual_state.daa_score,
                miner_data.clone(),
                &virtual_state.sydar_consensus_data,
                &virtual_state.mergeset_rewards,
                &virtual_state.mergeset_non_daa,
            )
            .unwrap();

        // --- sydar HYBRID SURGERY (ACCOUNT + OBJECT MODEL) ---
        let mut txs = txs;

        let blue_score = virtual_state.sydar_consensus_data.blue_score;
        let subsidy = self.coinbase_manager.calc_block_subsidy(blue_score);
        let dynamic_payload = self
            .coinbase_manager
            .serialize_coinbase_payload(&sydar_consensus_core::coinbase::CoinbaseData {
                blue_score,
                subsidy,
        ...
        let state_receipt = Transaction::new(
            ...
            vec![],                                                   // outputs
            0,                                                        // lock_time
            sydar_consensus_core::subnets::SUBNETWORK_ID_COINBASE, // Network tag
            0,                                                        // gas
            dynamic_payload,
        );
        ...
        let accepted_id_merkle_root = self.calc_accepted_id_merkle_root(
            ...
            virtual_state.sydar_consensus_data.selected_parent,
        );
        ...
        let header = Header::new_finalized(
            ...
            0,
            virtual_state.daa_score,
            virtual_state.sydar_consensus_data.blue_work,
            virtual_state.sydar_consensus_data.blue_score,
            header_pruning_point,
        );
        let selected_parent_hash = virtual_state.sydar_consensus_data.selected_parent;
    ...
    pub fn process_genesis(self: &Arc<Self>) {
        ...
        self.commit_virtual_state(
            ...
            Arc::new(VirtualState::from_genesis(
                &self.genesis,
                self.sydar_consensus_manager.sydar_consensus(&[self.genesis.hash]),
            )),
            &Default::default(),
            &Default::default(),
        );
    ...
    pub fn import_pruning_point_utxo_set(
        ...
        let dummy_view = sydar_consensus_core::utxo::utxo_collection::UtxoCollection::default();
        let new_pruning_point_transactions = vec![]; // sydar ACCOUNT BYPASS: Dummy variable
        ...
        let virtual_sydar_consensus_data = self.sydar_consensus_manager.sydar_consensus(&virtual_parents);
    ...
    pub fn are_pruning_points_violating_finality(&self, pp_list: PruningPointsList) -> bool {
        ...
        let vf = self.virtual_finality_point(&self.lkg_virtual_state.load().sydar_consensus_data, current_pp);
        let vff = self.depth_manager.calc_finality_point(&self.sydar_consensus_store.get_data(vf).unwrap(), current_pp);
