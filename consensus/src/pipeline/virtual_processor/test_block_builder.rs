...
impl TestBlockBuilder {
    ...
    pub(crate) fn build_block_template_with_parents(
        ...
        let sink = virtual_state.sydar_consensus_data.selected_parent;
        ...
        let (pov_virtual_parents, pov_virtual_sydar_consensus_data) =
        ...
        let pov_virtual_state = self.calculate_virtual_state(
            &virtual_read,
            pov_virtual_parents,
            pov_virtual_sydar_consensus_data,
            sydar_muhash::MuHash::new(),
        )?;

        let dummy_utxo_view = sydar_consensus_core::utxo::utxo_collection::UtxoCollection::default();
