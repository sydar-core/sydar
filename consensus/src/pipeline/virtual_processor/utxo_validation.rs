...
use crate::{
    ...
    model::stores::{
        ...
        sydar_consensus::{CompactsydarConsensusData, sydarConsensusData},
    },
    processes::{
...
pub(super) struct UtxoProcessingContext<'a> {
    pub sydar_consensus_data: Refs<'a, sydarConsensusData>,
...
impl<'a> UtxoProcessingContext<'a> {
    pub fn new(sydar_consensus_data: Refs<'a, sydarConsensusData>, selected_parent_multiset_hash: MuHash) -> Self {
        let mergeset_size = sydar_consensus_data.mergeset_size();
        Self {
    ...
    pub fn selected_parent(&self) -> Hash {
        self.sydar_consensus_data.selected_parent
    }
}
...
impl VirtualStateProcessor {
    ...
    pub(super) fn calculate_utxo_state<V: UtxoView + Sync>(
        ...
        for (i, (merged_block, txs)) in once((ctx.selected_parent(), selected_parent_transactions))
            .chain(
                ctx.sydar_consensus_data
                    .consensus_ordered_mergeset_without_selected_parent(self.sydar_consensus_store.deref())
    ...
    pub(super) fn verify_expected_utxo_state<V: UtxoView + Sync>(
        ...
        let reply = self.verify_header_pruning_point(header, ctx.sydar_consensus_data.to_compact())?;
    ...
    fn verify_header_pruning_point(
        &self,
        header: &Header,
        sydar_consensus_data: CompactsydarConsensusData,
    ) -> BlockProcessResult<PruningPointReply> {
        let reply = self.pruning_point_manager.expected_header_pruning_point(sydar_consensus_data);
    ...
    fn validate_coinbase_transaction(
        ...
        _sydar_consensus_data: &sydarConsensusData,
    ...
    pub(super) fn validate_mempool_transaction_in_utxo_context(
        ...
        // sydar ACCOUNT MODEL: Skip UTXO validation for account transactions
