...
## Sequential processing flow (in go-sydard)

Below we detail the current state of affairs in *go-sydard* and discuss future parallelism opportunities. Processing dependencies between various stages are detailed in square brackets [***deps; type***].
...
* Pow:
    ...
    * check difficulty and blue work
        * run sydar_CONSENSUS and stage [***reachability; read*** | ***sydar_consensus; write***]
...
* Post pow (aka "*header in context*"):
    ...
    * check mergeset size limit (could be done following sydar_CONSENSUS)
...
The current code design (*go-sydard*) already logically supports this since the various processing stages were already decoupled for supporting efficient IBD.
