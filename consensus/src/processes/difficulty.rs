...
use sydar_core::{info, log::RAIGAD_KEYWORD};
use sydar_hashes::Hash;
use sydar_math::{Uint256, Uint320};
use std::{
    cmp::{Ordering, max},
    ops::Deref,
    sync::{
...
use super::sydar_consensus::ordering::SortableBlock;
...
trait DifficultyManagerExtension {
    ...
    #[inline]
    #[must_use]
    fn internal_calc_daa_score(&self, sydar_consensus_data: &sydarConsensusData, mergeset_non_daa: &BlockHashSet) -> u64 {
        let sp_daa_score = self.headers_store().get_daa_score(sydar_consensus_data.selected_parent).unwrap();
        sp_daa_score + (sydar_consensus_data.mergeset_size() - mergeset_non_daa.len()) as u64
    }
...
/// A difficulty manager based on sampled block windows, implementing [KIP-0004](https://github.com/sydarnet/kips/blob/master/kip-0004.md)
#[derive(Clone)]
pub struct SampledDifficultyManager<T: HeaderStoreReader, U: sydarConsensusStoreReader> {
    headers_store: Arc<T>,
    _sydar_consensus_store: Arc<U>,
    genesis_hash: Hash,
    genesis_bits: u32,
...
impl<T: HeaderStoreReader, U: sydarConsensusStoreReader> SampledDifficultyManager<T, U> {
    pub fn new(
        headers_store: Arc<T>,
        sydar_consensus_store: Arc<U>,
        genesis_hash: Hash,
        genesis_bits: u32,
        ...
        Self {
            headers_store,
            _sydar_consensus_store: sydar_consensus_store,
            genesis_hash,
            genesis_bits,
    ...
    #[inline]
    #[must_use]
    pub fn lowest_daa_blue_score(&self, sydar_consensus_data: &sydarConsensusData) -> u64 {
        let difficulty_full_window_size = self.difficulty_full_window_size();
        sydar_consensus_data.blue_score.max(difficulty_full_window_size) - difficulty_full_window_size
    }

    #[inline]
    #[must_use]
    pub fn calc_daa_score(&self, sydar_consensus_data: &sydarConsensusData, mergeset_non_daa: &BlockHashSet) -> u64 {
        self.internal_calc_daa_score(sydar_consensus_data, mergeset_non_daa)
    }

    pub fn calc_daa_score_and_mergeset_non_daa_blocks(
        &self,
        sydar_consensus_data: &sydarConsensusData,
        store: &(impl sydarConsensusStoreReader + ?Sized),
    ) -> (u64, BlockHashSet) {
        let lowest_daa_blue_score = self.lowest_daa_blue_score(sydar_consensus_data);
        let mergeset_non_daa: BlockHashSet = sydar_consensus_data
            .unordered_mergeset()
            .filter(|hash| store.get_blue_score(*hash).unwrap() < lowest_daa_blue_score)
            .collect();
        (self.internal_calc_daa_score(sydar_consensus_data, &mergeset_non_daa), mergeset_non_daa)
    }

    pub fn calculate_difficulty_bits(&self, window: &BlockWindowHeap, sydar_consensus_data: &sydarConsensusData) -> u32 {
...
impl<T: HeaderStoreReader, U: sydarConsensusStoreReader> DifficultyManagerExtension for SampledDifficultyManager<T, U> {
    fn headers_store(&self) -> &dyn HeaderStoreReader {
...
#[cfg(test)]
mod tests {
    use sydar_consensus_core::{BlockLevel, BlueWorkType, MAX_WORK_LEVEL};
    use sydar_math::{Uint256, Uint320};
    use sydar_pow::calc_level_from_pow;

    use crate::processes::difficulty::{calc_work, level_work};
    use sydar_utils::hex::ToHex;
