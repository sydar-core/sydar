...
use sydar_database::prelude::StoreResultUnitExt;
...
> PruningPointManager<S, T, U, V, W, Y>
{
    ...
    ///
    /// Let B denote the current block (represented by `sydar_consensus_data`)
    /// Assumptions:
    ...
    pub fn expected_header_pruning_point(&self, sydar_consensus_data: CompactsydarConsensusData) -> PruningPointReply {
        //
        // Note that past pruning samples are only assumed to have a header store entry and a pruning sample
        // store entry, se we only use these stores here (and specifically do not use the sydar_consensus store)
        //

        let pruning_depth = self.pruning_depth;
        let finality_depth = self.finality_depth;

        let selected_parent_blue_score = self.headers_store.get_blue_score(sydar_consensus_data.selected_parent).unwrap();

        let pruning_sample = if sydar_consensus_data.selected_parent == self.genesis_hash {
            self.genesis_hash
        } else {
            let selected_parent_pruning_sample =
                self.pruning_samples_store.pruning_sample_from_pov(sydar_consensus_data.selected_parent).unwrap();
        ...
        let is_self_pruning_sample =
            self.is_pruning_sample(sydar_consensus_data.blue_score, selected_parent_blue_score, finality_depth);
        let selected_parent_pruning_point =
            self.headers_store.get_header(sydar_consensus_data.selected_parent).unwrap().pruning_point;
        let mut steps = 1;
        ...
        let pruning_point = loop {
            ...
            if current_blue_score + pruning_depth <= sydar_consensus_data.blue_score {
                break current;
            }
    ...
    pub fn next_pruning_points(
        &self,
        sink_sydar_consensus: CompactsydarConsensusData,
        current_pruning_point: Hash,
    ) -> Vec<Hash> {
        if sink_sydar_consensus.selected_parent.is_origin() {
        ...
        if current_pruning_point_blue_score + self.pruning_depth > sink_sydar_consensus.blue_score {
            // During initial IBD the sink can be close to the global pruning point.
            return vec![];
        }

        let sink_pruning_point = self.expected_header_pruning_point(sink_sydar_consensus).pruning_point;
    ...
    pub fn is_valid_pruning_point(&self, pp_candidate: Hash, tip: Hash) -> bool {
        ...
        let tip_bs = self.sydar_consensus_store.get_blue_score(tip).unwrap();
    ...
    pub fn pruning_points_on_path_to_syncer_sink(
        ...
        for current in self.reachability_service.forward_chain_iterator(pruning_point, syncer_sink, true).skip(1) {
            ...
            let reply = self.expected_header_pruning_point(self.sydar_consensus_store.get_compact_data(current).unwrap());
...
#[cfg(test)]
mod tests {
    use sydar_consensus_core::{config::params::Params, network::NetworkType};
    ...
    fn assert_pruning_depth_consistency() {
        for net in NetworkType::iter() {
            ...
            let sydar_consensus_k = params.sydar_consensus_k();

            // Assert P is not a multiple of F +- noise(K)
            let mod_after = pruning_depth % finality_depth;
            assert!((sydar_consensus_k as u64) < mod_after && mod_after < finality_depth - sydar_consensus_k as u64);
        }
    }
}
