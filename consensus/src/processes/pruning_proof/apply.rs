...
use sydar_utils::{binary_heap::BinaryHeapExtensions, vec::VecExtensions};
...
use crate::{
    model::{
        ...
        stores::{
            ...
            sydar_consensus::{sydarConsensusData, sydarConsensusStore},
    ...
    processes::{
        ...
        sydar_consensus::{mergeset::unordered_mergeset_without_selected_parent, ordering::SortableBlock},
    },
};
...
impl PruningProofManager {
    pub fn apply_proof(&self, proof: PruningPointProof, trusted_set: &[TrustedBlock]) -> PruningImportResult<()> {
        ...
        let mut trusted_gd_map: BlockHashMap<sydarConsensusData> = BlockHashMap::new();
        // This loop expands the proof with the headers of the trusted set
        // and creates a hash to sydar_consensus data map of the trusted set
        for tb in trusted_set.iter() {
            trusted_gd_map.insert(tb.block.hash(), tb.sydar_consensus.clone().into());
        ...
        // Populate sydar_consensus_store and relation store for every block in the proof
        ...
        for header in expanded_proof[0].iter() {
            ...
            let gd = if let Some(gd) = trusted_gd_map.get(&header.hash) {
                ...
                let calculated_gd = self.sydar_consensus_manager.sydar_consensus(&parents);
                // Override the sydar_consensus data with the real blue score and blue work
                sydarConsensusData {
            ...
            self.sydar_consensus_store.insert(header.hash, Arc::new(gd)).unwrap();
