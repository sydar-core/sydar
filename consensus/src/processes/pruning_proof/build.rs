...
use sydar_core::{debug, trace};
use sydar_database::prelude::*;
use sydar_hashes::Hash;
use sydar_utils::binary_heap::TopK;
...
use crate::{
    ...
    processes::{
        pruning_proof::{ProofInternalError, sydarConsensusReaderExt},
        reachability::inquirer as reachability,
        relations::RelationsStoreExtensions,
        sydar_consensus::{ordering::SortableBlock, protocol::sydarConsensusManager},
    },
};
...
struct LevelProofContext {
    sydar_consensus_store: Arc<DbsydarConsensusStore>,
    tip: Hash,
    root: Hash,
    count: u64,
}
...
/// Utility for creating retry-indexed temporary sydar_consensus stores.
///
/// Each call to `new_store` returns a fresh temporary `DbsydarConsensusStore` for the
/// given level, using an incrementing retry index to avoid namespace collisions.
/// This is used when multiple sydar_consensus attempts may be required for the same level.
struct sydarConsensusStoreFactory {
    db: Arc<DB>,
...
impl sydarConsensusStoreFactory {
    fn new(db: Arc<DB>, cache_policy: CachePolicy, level: BlockLevel) -> Self {
        Self { db, cache_policy, level, retries: 0 }
    }

    /// Creates a fresh temporary sydar_consensus store for the next retry attempt
    fn new_store(&mut self) -> Arc<DbsydarConsensusStore> {
        self.retries += 1;
        Arc::new(DbsydarConsensusStore::new_temp(
            self.db.clone(),
            self.level,
            self.cache_policy,
            self.cache_policy,
...
impl PruningProofManager {
    ...
    fn proof_from_descriptor(&self, descriptor: &PruningProofDescriptor) -> PruningPointProof {
        ...
        let (_db_lifetime, temp_db) = sydar_database::create_temp_db!(ConnBuilder::default().with_files_limit(10));
    ...
    fn calc_new_proof(&self, pp: Hash, prev_descriptor: Option<&PruningProofDescriptor>) -> PruningProofDescriptor {
        ...
        for level in (0..=self.max_block_level).rev() {
            ...
            let required_block = if level != self.max_block_level {
                let LevelProofContext { sydar_consensus_store: next_level_gd_store, tip: next_level_tip, .. } =
    ...
    ///    (e.g. future-size tracking, sydar_consensus inputs) is observed consistently.
    /// 3. Whenever the current block is a viable root candidate (sufficient base-level depth,
    ///    sufficient future size, and inclusion constraints), populate a temporary sydar_consensus store
    ...
    fn calc_level_proof_context(
        ...
        // the temporary sydar_consensus instance at this level.
        ...
        //   lacks sufficient blues here, we avoid premature sydar_consensus attempts.
        ...
        // Each sydar_consensus attempt uses a fresh temp store namespace (indexed internally by `retries`).
        let mut sydar_consensus_factory = sydarConsensusStoreFactory::new(db.clone(), cache_policy, level);
        ...
        let mut try_root = |relations_store: &DbRelationsStore, root: Hash, future_size: u64| -> Option<LevelProofContext> {
            // Populate sydar_consensus for `future(root) ∩ past(tip)` and test depth requirements.
            let (sydar_consensus_store, has_required_block, count) = self.populate_level_proof_sydar_consensus_data(
                relations_store,
                &mut sydar_consensus_factory,
                &mut reachability_factory,
                root,
                tip,
                required,
                level,
                self.sydar_consensus_k,
            );

            // Realized blue depth for this root, computed from the level-specific sydar_consensus
            let current_level_score = sydar_consensus_store.get_blue_score(tip).unwrap();

            // Log all non-trivial cases
            if tip != self.genesis_hash {
                debug!(
                    "level: {}, future: {}, blue score: {}, count: {}, retries: {}",
                    level, future_size, current_level_score, count, sydar_consensus_factory.retries
                );
            }
            ...
            if has_required_block && (root == self.genesis_hash || current_level_score >= 2 * self.pruning_proof_m) {
                Some(LevelProofContext { sydar_consensus_store, tip, root, count })
            } else {
                None
            }
        };
        ...
        while let Some(SortableBlock { hash: current, .. }) = queue.pop() {
            ...
            if self.reachability_service.is_dag_ancestor_of(current, required) {
                // If the root appears immediately viable, attempt sydar_consensus now.
                    ...
                    || (future_size >= required_future_size && base_level_depth >= required_base_level_depth)
                {
                    ...
                    // Defer sydar_consensus and keep as a fallback candidate.
        ...
        // With a fixed root, sydar_consensus selection is deterministic, and if the new tip is in the future of the
    ...
    /// Forward-traverses from `root` toward `tip`, and inserts sydar_consensus data for each visited block.
    ///
    ...
    fn populate_level_proof_sydar_consensus_data(
        &self,
        relations_store: &DbRelationsStore,
        sydar_consensus_factory: &mut sydarConsensusStoreFactory,
    ...
    ) -> (Arc<DbsydarConsensusStore>, bool, u64) {
        debug!("Populating GD for root {} at level {} (retry {})", root, level, sydar_consensus_factory.retries.saturating_sub(1));

        let sydar_consensus_store = sydar_consensus_factory.new_store();
        ...
        // Create a sydar_consensus manager over the restricted relations view
        let sydar_consensus_manager = sydarConsensusManager::with_level(
            root,
            sydar_consensus_k,
        ...
        sydar_consensus_store.insert(root, Arc::new(sydar_consensus_manager.genesis_sydar_consensus_data())).unwrap();
        ...
        while let Some(Reverse(SortableBlock { hash: current, .. })) = queue.pop() {
            ...
            let sydar_consensus_data = Arc::new(sydar_consensus_manager.sydar_consensus(parents.as_slice()));
            sydar_consensus_store.insert(current, sydar_consensus_data.clone()).unwrap();

            reachability_tip = sydar_consensus_manager.find_selected_parent([reachability_tip, current]);
        ...
        (sydar_consensus_store, has_required_block, count)
    }
