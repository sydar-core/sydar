...
use sydar_consensus_core::{
    ...
    trusted::{TrustedHeader, TrustedsydarConsensusData},
};
use sydar_core::info;
use sydar_database::prelude::StoreResultExt;
use sydar_hashes::Hash;
use sydar_pow::calc_block_level;
use thiserror::Error;

use crate::{
    consensus::{
        services::{DbDagTraversalManager, DbParentsManager, DbsydarConsensusManager, DbWindowManager},
    ...
    model::{
        ...
        stores::{
            ...
            sydar_consensus::{DbsydarConsensusStore, sydarConsensusStoreReader},
...
pub struct PruningProofManager {
    ...
    sydar_consensus_manager: DbsydarConsensusManager,
...
impl PruningProofManager {
    ...
    ///
    /// The returned hashes are guaranteed to have sydar_CONSENSUS data
    pub(crate) fn get_sydar_consensus_chain_k_depth(&self, hash: Hash, sydar_consensus_k: KType) -> Vec<Hash> {
        let mut hashes = Vec::with_capacity(sydar_consensus_k as usize + 1);
        let mut current = hash;
        for _ in 0..=sydar_consensus_k {
            hashes.push(current);
            let Some(parent) = self.sydar_consensus_store.get_selected_parent(current).optional().unwrap() else {
                break;
            };
    ...
    pub(crate) fn calculate_pruning_point_anticone_and_trusted_data(
        ...
        let mut anticone = self.sydar_consensus_manager.sort_blocks(anticone);
        anticone.insert(0, pruning_point);

        let mut daa_window_blocks = BlockHashMap::new();
        let mut sydar_consensus_blocks = BlockHashMap::new();

        let sydar_consensus_k = self.sydar_consensus_k;
        ...
        for anticone_block in anticone.iter().copied() {
            let sydar_consensus = self.sydar_consensus_store.get_data(anticone_block).unwrap();
            let window = self.window_manager.block_window(&sydar_consensus, WindowType::DifficultyWindow).unwrap();
            let cover = self.window_manager.consecutive_cover_for_window(sydar_consensus, &window);

            for hash in cover {
                if let Entry::Vacant(e) = daa_window_blocks.entry(hash) {
                    e.insert(TrustedHeader {
                        header: self.headers_store.get_header(hash).unwrap(),
                        sydar_consensus: (&*self.sydar_consensus_store.get_data(hash).unwrap()).into(),
                    });
                }
            }

            let sydar_consensus_chain = self.get_sydar_consensus_chain_k_depth(anticone_block, sydar_consensus_k);
            for hash in sydar_consensus_chain {
                if let Entry::Vacant(e) = sydar_consensus_blocks.entry(hash) {
                    let sydar_consensus = self.sydar_consensus_store.get_data(hash).unwrap();
                    e.insert((&*sydar_consensus).into());

                    // We fill `sydar_consensus_blocks` only for sydard-go legacy reasons, but the real set we
                    // send is `daa_window_blocks` which represents the full trusted sub-DAG in the antifuture
                    // of the pruning point which sydard-rust nodes expect to get when synced with headers proof
                    if let Entry::Vacant(e) = daa_window_blocks.entry(hash) {
                        e.insert(TrustedHeader {
                            header: self.headers_store.get_header(hash).unwrap(),
                            sydar_consensus: (&*sydar_consensus).into(),
                        });
                    }
                }
            }
        ...
        while let Some(current) = queue.pop_front() {
            if let Entry::Vacant(e) = daa_window_blocks.entry(current) {
                ...
                let sydar_consensus = (&*self.sydar_consensus_store.get_data(current).unwrap()).into();
                e.insert(TrustedHeader { header, sydar_consensus });
            }
        ...
        PruningPointTrustedData {
            ...
            sydar_consensus_blocks: sydar_consensus_blocks
                .into_iter()
                .map(|(hash, sydar_consensus)| TrustedsydarConsensusData { hash, sydar_consensus })
                .collect_vec(),
        }
    }
    ...
    pub fn get_pruning_point_anticone_and_trusted_data(&self) -> ConsensusResult<Arc<PruningPointTrustedData>> {
        ...
        if virtual_state.sydar_consensus_data.blue_score >= pp_bs + self.anticone_finalization_depth {
...
trait sydarConsensusReaderExt
where
    Self: sydarConsensusStoreReader,
{
...
impl<T: sydarConsensusStoreReader> sydarConsensusReaderExt for T {}
