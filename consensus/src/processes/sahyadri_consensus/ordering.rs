...
use sydar_consensus_core::BlueWorkType;
use sydar_hashes::Hash;
use serde::{Deserialize, Serialize};

use crate::model::{
    services::reachability::ReachabilityService,
    stores::{headers::HeaderStoreReader, relations::RelationsStoreReader, sydar_consensus::sydarConsensusStoreReader},
};

use super::protocol::sydarConsensusManager;
    ...
    sydarConsensusManager<T, S, U, V>
{
    pub fn sort_blocks(&self, blocks: impl IntoIterator<Item = Hash>) -> Vec<Hash> {
        ...
        sorted_blocks.sort_by_cached_key(|block| SortableBlock {
            hash: *block,
            blue_work: self.sydar_consensus_store.get_blue_work(*block).unwrap(),
        });
        sorted_blocks
    }
}
