...
use crate::{
    model::{
        ...
        stores::{
            ...
            sydar_consensus::{HashKTypeMap, KType, sydarConsensusData, sydarConsensusStoreReader},
        },
    },
...
#[derive(Clone)]
pub struct sydarConsensusManager<
...
> {
    genesis_hash: Hash,
    pub(super) k: KType,
    pub(super) sydar_consensus_store: Arc<T>,
...
impl<T: sydarConsensusStoreReader, S: RelationsStoreReader, U: ReachabilityService, V: HeaderStoreReader>
    sydarConsensusManager<T, S, U, V>
{
    pub fn new(
        genesis_hash: Hash,
        k: KType,
        ...
        Self { genesis_hash, k, sydar_consensus_store, relations_store, reachability_service, headers_store, level_work: 0.into() }
    }
    ...
    pub fn genesis_sydar_consensus_data(&self) -> sydarConsensusData {
        sydarConsensusData::new(
            0,
            Default::default(),
            blockhash::ORIGIN,
    ...
    pub fn origin_sydar_consensus_data(&self) -> Arc<sydarConsensusData> {
        Arc::new(sydarConsensusData::new(
            0,
            Default::default(),
            0.into(),
            BlockHashes::new(Vec::new()),
    ...
    pub fn find_selected_parent(&self, parents: impl IntoIterator<Item = Hash>) -> Hash {
        parents
            .into_iter()
            .map(|parent| SortableBlock { hash: parent, blue_work: self.sydar_consensus_store.get_blue_work(parent).unwrap() })
            .max()
            .unwrap()
            .hash
    }

    /// Runs the sydar_CONSENSUS protocol and calculates the block sydarConsensusData by the given parents.
    ...
    pub fn sydar_consensus(&self, parents: &[Hash]) -> sydarConsensusData {
        assert!(!parents.is_empty(), "genesis must be added via a call to init");

        // Run the sydar_CONSENSUS parent selection algorithm
        ...
        if selected_parent.is_origin() {
            ...
            return sydarConsensusData::new_with_selected_parent(selected_parent, 1); // k is only a capacity hint here
        }
        let k = self.k;
        // Initialize new sydar_CONSENSUS block data with the selected parent
        let mut new_block_data = sydarConsensusData::new_with_selected_parent(selected_parent, k);
        ...
        let blue_score =
            self.sydar_consensus_store.get_blue_score(selected_parent).unwrap() + new_block_data.mergeset_blues.len() as u64;
        ...
        let added_blue_work: BlueWorkType = new_block_data
            ...
            .sum();
        let blue_work: BlueWorkType = self.sydar_consensus_store.get_blue_work(selected_parent).unwrap() + added_blue_work;
    ...
    fn check_blue_candidate_with_chain_block(
        &self,
        new_block_data: &sydarConsensusData,
    ...
    fn blue_anticone_size(&self, block: Hash, context: &sydarConsensusData) -> KType {
        ...
        loop {
            ...
            current_blues_anticone_sizes = self.sydar_consensus_store.get_blues_anticone_sizes(current_selected_parent).unwrap();
            current_selected_parent = self.sydar_consensus_store.get_selected_parent(current_selected_parent).unwrap();
        }
    }

    fn check_blue_candidate(&self, new_block_data: &sydarConsensusData, blue_candidate: Hash, k: KType) -> ColoringOutput {
        ...
        loop {
            ...
            chain_block = ChainBlock {
                ...
                data: self.sydar_consensus_store.get_data(chain_block.data.selected_parent).unwrap().into(),
            }
        }
    }
}

/// Chain block with attached sydar_consensus data
struct ChainBlock<'a> {
    hash: Option<Hash>, // if set to `None`, signals being the new block
    data: Refs<'a, sydarConsensusData>,
}

/// Represents the intermediate sydar_CONSENSUS coloring state for the current candidate
enum ColoringState {
    Blue,
    Red,
    Pending,
...
/// Represents the final output of sydar_CONSENSUS coloring for the current candidate
enum ColoringOutput {
