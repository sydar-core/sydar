...
use sydar_consensus_core::errors::sync::{SyncManagerError, SyncManagerResult};
use sydar_database::prelude::StoreResultExt;
use sydar_hashes::Hash;
use sydar_math::uint::malachite_base::num::arithmetic::traits::CeilingLogBase2;
...
use crate::model::{
    ...
    stores::{
        ...
        relations::RelationsStoreReader, sydar_consensus::sydarConsensusStoreReader, selected_chain::SelectedChainStoreReader,
...
> SyncManager<S, T, U, V, W, X, Y>
{
    ...
    pub fn antipast_hashes_between(&self, low: Hash, high: Hash, max_blocks: Option<usize>) -> (Vec<Hash>, Hash) {
        ...
        let low_bs = self.sydar_consensus_store.get_blue_score(low).unwrap();
        let high_bs = self.sydar_consensus_store.get_blue_score(high).unwrap();
        ...
        for current in self.reachability_service.forward_chain_iterator(low, high, true).skip(1) {
            let gd = self.sydar_consensus_store.get_data(current).unwrap();
            if blocks.len() + gd.mergeset_size() > max_blocks {
                break;
            }
            blocks.extend(
                gd.consensus_ordered_mergeset(self.sydar_consensus_store.deref())
    ...
    pub fn create_block_locator_from_pruning_point(
        ...
        loop {
            ...
            let current_gd = self.sydar_consensus_store.get_compact_data(current).unwrap();
