...
use crate::model::{
    ...
    stores::{
        reachability::ReachabilityStoreReader, relations::RelationsStoreReader, sydar_consensus::sydarConsensusStoreReader,
    },
};
...
#[derive(Clone)]
pub struct DagTraversalManager<T: sydarConsensusStoreReader, U: ReachabilityStoreReader, V: RelationsStoreReader> {
    genesis_hash: Hash,
...
impl<T: sydarConsensusStoreReader, U: ReachabilityStoreReader, V: RelationsStoreReader> DagTraversalManager<T, U, V> {
    pub fn new(
        genesis_hash: Hash,
        sydar_consensus_store: Arc<T>,
        relations_store: V,
        ...
        Self { genesis_hash, sydar_consensus_store, relations_store, reachability_service }
    }
    ...
    pub fn lowest_chain_block_above_or_equal_to_blue_score(&self, high: Hash, blue_score: u64) -> Hash {
        let high_gd = self.sydar_consensus_store.get_compact_data(high).unwrap();
        ...
        while current != self.genesis_hash {
            ...
            let selected_parent_gd = self.sydar_consensus_store.get_compact_data(current_gd.selected_parent).unwrap();
