use crate::{
    ...
    processes::sydar_consensus::ordering::SortableBlock,
};
...
pub trait WindowManager {
    ...
    fn calc_daa_window(&self, sydar_consensus_data: &sydarConsensusData, window: Arc<BlockWindowHeap>) -> DaaWindow;
    fn block_daa_window(&self, sydar_consensus_data: &sydarConsensusData) -> Result<DaaWindow, RuleError>;
    fn calculate_difficulty_bits(&self, sydar_consensus_data: &sydarConsensusData, daa_window: &DaaWindow) -> u32;
    fn calc_past_median_time(&self, sydar_consensus_data: &sydarConsensusData)
    ...
    fn window_size(&self, sydar_consensus_data: &sydarConsensusData, window_type: WindowType) -> usize;
    fn sample_rate(&self, sydar_consensus_data: &sydarConsensusData, window_type: WindowType) -> u64;

    /// Returns the full consecutive sub-DAG containing all blocks required to restore the (possibly sampled) window.
    fn consecutive_cover_for_window(&self, sydar_consensus_data: Arc<sydarConsensusData>, window: &BlockWindowHeap)
    -> Vec<Hash>;
}
...
/// A sampled window manager implementing [KIP-0004](https://github.com/sydarnet/kips/blob/master/kip-0004.md)
#[derive(Clone)]
pub struct SampledWindowManager<
    T: sydarConsensusStoreReader,
    U: BlockWindowCacheReader + BlockWindowCacheWriter,
    V: HeaderStoreReader,
    W: DaaStoreReader,
> {
    genesis_hash: Hash,
    sydar_consensus_store: Arc<T>,
    headers_store: Arc<V>,
    _daa_store: Arc<W>,
...
impl<T: sydarConsensusStoreReader, U: BlockWindowCacheReader + BlockWindowCacheWriter, V: HeaderStoreReader, W: DaaStoreReader>
    SampledWindowManager<T, U, V, W>
{
    ...
    fn build_block_window(
        ...
        let window_size = self.window_size(sydar_consensus_data, window_type);
        let sample_rate = self.sample_rate(sydar_consensus_data, window_type);

        // First, we handle all edge cases
        if window_size == 0 {
            return Ok(Arc::new(BlockWindowHeap::new()));
        }
        if sydar_consensus_data.selected_parent == self.genesis_hash {
        ...
        let selected_parent_blue_work = self.sydar_consensus_store.get_blue_work(sydar_consensus_data.selected_parent).unwrap();
        ...
        // else we populate the window with the passed sydar_consensus_data.
        ...
        let mut current_sydar_consensus = self.sydar_consensus_store.get_data(sydar_consensus_data.selected_parent).unwrap();

        // Note: no need to check for cache here, as we already tried to initialize from the passed sydar_consensus's selected parent cache in `self.try_init_from_cache`

        // Walk down the chain until we cross the window boundaries.
        loop {
            // check if we may exit early.
            if current_sydar_consensus.selected_parent.is_origin() {
            ...
            } else if current_sydar_consensus.selected_parent == self.genesis_hash {
                break;
            }

            let parent_sydar_consensus =
                self.sydar_consensus_store.get_data(current_sydar_consensus.selected_parent).unwrap();

            // No need to further iterate since past of selected parent has only lower blue work
            if !window_heap.can_push(current_sydar_consensus.selected_parent, parent_sydar_consensus.blue_work) {
                break;
            }
            ...
            self.push_mergeset(
                &mut &mut window_heap,
                sample_rate,
                &current_sydar_consensus,
                parent_sydar_consensus.blue_work,
                None::<fn(Hash)>,
            );

            // see if we can inherit and merge with the selected parent cache
            if self.try_merge_with_selected_parent_cache(&mut window_heap, &cache, &current_sydar_consensus.selected_parent) {
                // if successful, we may break out of the loop, with the window already filled.
                break;
            };

            // update the current sydar_consensus to the parent sydar_consensus, and continue the loop.
            current_sydar_consensus = parent_sydar_consensus;
        }
    ...
    fn push_mergeset(
        ...
        if let Some(mut mergeset_non_daa_inserter) = mergeset_non_daa_inserter {
            ...
            for block in self.sampled_mergeset_iterator(sample_rate, sydar_consensus_data, selected_parent_blue_work) {
                match block {
    ...
    fn try_init_from_cache(
        ...
        cache.get(&sydar_consensus_data.selected_parent).map(|selected_parent_window| {
            ...
            self.push_mergeset(&mut heap, sample_rate, sydar_consensus_data, selected_parent_blue_work, mergeset_non_daa_inserter);
    ...
    fn sampled_mergeset_iterator<'a>(
        ...
        let selected_parent_block = SortableBlock::new(sydar_consensus_data.selected_parent, selected_parent_blue_work);
        let selected_parent_daa_score = self.headers_store.get_daa_score(sydar_consensus_data.selected_parent).unwrap();
        let blue_score_threshold = self.difficulty_manager.lowest_daa_blue_score(sydar_consensus_data);
        let mut index: u64 = 0;

        once(selected_parent_block)
            .chain(sydar_consensus_data.descending_mergeset_without_selected_parent(self.sydar_consensus_store.deref()))
            .filter_map(move |block| {
                let blue_score = self.sydar_consensus_store.get_blue_score(block.hash).unwrap();
    ...
    WindowManager for SampledWindowManager<T, U, V, W>
{
    fn block_window(
        ...
        self.build_block_window(sydar_consensus_data, window_type, |_| {})
    }

    fn calc_daa_window(&self, sydar_consensus_data: &sydarConsensusData, window: Arc<BlockWindowHeap>) -> DaaWindow {
        let (daa_score, mergeset_non_daa) = self
            .difficulty_manager
            .calc_daa_score_and_mergeset_non_daa_blocks(sydar_consensus_data, self.sydar_consensus_store.deref());
        DaaWindow::new(window, daa_score, mergeset_non_daa)
    }

    fn block_daa_window(&self, sydar_consensus_data: &sydarConsensusData) -> Result<DaaWindow, RuleError> {
        let mut mergeset_non_daa = BlockHashSet::default();
        let window = self.build_block_window(sydar_consensus_data, WindowType::DifficultyWindow, |hash| {
            mergeset_non_daa.insert(hash);
        })?;
        let daa_score = self.difficulty_manager.calc_daa_score(sydar_consensus_data, &mergeset_non_daa);
        Ok(DaaWindow::new(window, daa_score, mergeset_non_daa))
    }

    fn calculate_difficulty_bits(&self, sydar_consensus_data: &sydarConsensusData, daa_window: &DaaWindow) -> u32 {
        self.difficulty_manager.calculate_difficulty_bits(&daa_window.window, sydar_consensus_data)
    }
    ...
    fn calc_past_median_time(
        ...
        let window = self.block_window(sydar_consensus_data, WindowType::MedianTimeWindow)?;
        let past_median_time =
            self.past_median_time_manager.calc_past_median_time(&window, sydar_consensus_data.selected_parent)?;
    ...
    fn calc_past_median_time_for_known_hash(&self, hash: Hash) -> Result<u64, RuleError> {
        if let Some(window) = self.block_window_cache_for_past_median_time.get(&hash) {
            let past_median_time = self
                .past_median_time_manager
                .calc_past_median_time(&window, self.sydar_consensus_store.get_selected_parent(hash).unwrap())?;
            Ok(past_median_time)
        } else {
            let sydar_consensus_data = self.sydar_consensus_store.get_data(hash).unwrap();
            let (past_median_time, window) = self.calc_past_median_time(&sydar_consensus_data)?;
    ...
    fn window_size(&self, _sydar_consensus_data: &sydarConsensusData, window_type: WindowType) -> usize {
        match window_type {
    ...
    fn sample_rate(&self, _sydar_consensus_data: &sydarConsensusData, window_type: WindowType) -> u64 {
        match window_type {
    ...
    fn consecutive_cover_for_window(&self, mut sydar_consensus: Arc<sydarConsensusData>, window: &BlockWindowHeap) -> Vec<Hash> {
        ...
        while !unvisited.is_empty() {
            assert!(!sydar_consensus.selected_parent.is_origin(), "unvisited still not empty");
            ...
            for merged in sydar_consensus.unordered_mergeset() {
                cover.insert(merged);
                if unvisited.remove(&merged) {
                    ...
                    cover.insert(self.sydar_consensus_store.get_selected_parent(merged).unwrap());
                }
            }
            if unvisited.is_empty() {
                break;
            }
            sydar_consensus = self.sydar_consensus_store.get_data(sydar_consensus.selected_parent).unwrap();
        }
