...
impl AppenderSpec {
    pub fn console(name: &'static str, level: Option<LevelFilter>) -> Self {
        Self::new(
            name,
            level,
            Box::new(ConsoleAppender::builder().encoder(Box::new(RaigadEncoder::new(LOG_LINE_PATTERN_COLORED))).build()),
        )
    }
...
pub const RAIGAD_KEYWORD: &str = "raigad";
const RAIGAD_LOG_LINE_PATTERN_COLORED: &str = "{d(%Y-%m-%d %H:%M:%S%.3f%:z)} [{h({(CRND):5.5})}] {m}{n}";

// TODO (post HF): remove or hide the custom encoder
#[derive(Debug)]
struct RaigadEncoder {
    general_encoder: PatternEncoder,
    raigad_encoder: PatternEncoder,
    keyword: &'static str,
}
...
impl RaigadEncoder {
    fn new(pattern: &str) -> Self {
        RaigadEncoder {
            ...
            raigad_encoder: PatternEncoder::new(RAIGAD_LOG_LINE_PATTERN_COLORED),
            keyword: RAIGAD_KEYWORD,
        }
    }
}

impl Encode for RaigadEncoder {
    fn encode(&self, w: &mut dyn Write, record: &log::Record) -> anyhow::Result<()> {
        if record.target() == self.keyword {
            ...
            self.raigad_encoder.encode(w, &record)?;
