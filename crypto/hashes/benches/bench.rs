...
fn bench_heavy_hash(c: &mut Criterion) {
    ...
    c.bench_function("sydarX", |b| {
        b.iter(|| {
            black_box(sydarX::hash(&in_hash.as_bytes()));
        })
    });

    let hasher = CShake256::from_core(CShake256Core::new(b"sydarX"));
    c.bench_function("generic sydarX without init", |b| {
        b.iter(|| {
    ...
    c.bench_function("generic sydarX with init", |b| {
        b.iter(|| {
            let hasher = CShake256::from_core(CShake256Core::new(b"sydarX")).chain(black_box(in_hash.as_bytes()));
            let mut hash = [0u8; 32];
            hasher.finalize_xof().read(&mut hash);
            black_box(hash);
        })
