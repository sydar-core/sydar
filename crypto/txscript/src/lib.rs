...
use sydar_consensus_core::hashing::sighash::{SigHashReusedValues, SigHashReusedValuesUnsync, calc_signature_hash};
use sydar_consensus_core::hashing::sighash_type::SigHashType;
use sydar_consensus_core::tx::{ScriptPublicKey, TransactionInput, UtxoEntry, VerifiableTransaction};
use sydar_txscript_errors::TxScriptError;
...
impl<'a, T: VerifiableTransaction, Reused: SigHashReusedValues> TxScriptEngine<'a, T, Reused> {
    ...
    fn execute_opcode(&mut self, opcode: DynOpcodeImplementation<T, Reused>) -> Result<(), TxScriptError> {
        // Different from sydard: Illegal and disabled opcode are checked on execute instead
        // Note that this includes OP_RESERVED which counts as a push operation.
        ...
        match self.script_source {
            ScriptSource::TxInput { tx, idx, .. } => {
                ...
                match self.sig_cache.get(&sig_cache_key) {
                    Some(valid) => Ok(valid),
                    None => {
                        let sig = sydar_dilithium::DilithiumSignature::from_slice(sig_bytes);
                        let valid = sydar_dilithium::DilithiumKeyPair::verify(
                            pk_bytes,
                            &sig,
                            &msg_bytes,
                            b"",
                            sydar_dilithium::sydar_MODE,
                        );
...
mod tests {
    ...
    use sydar_consensus_core::hashing::sighash_type::SIG_HASH_ALL;
...
mod bitcoind_tests {
    ...
    use sydar_consensus_core::constants::MAX_TX_IN_SEQUENCE_NUM;
    use sydar_consensus_core::hashing::sighash::SigHashReusedValuesUnsync;
    use sydar_consensus_core::tx::{
