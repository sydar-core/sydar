...
pub use sydard::{sydard, sydardConfig, sydardCtl};
...
pub static LOCATIONS: &[&str] = &[
    ...
    "../target/release",
    "../target/debug",
    "../../sydar-cpu-miner/target/debug",
    "../../sydar-cpu-miner/target/release",
    "bin/windows-x64",
    "bin/linux-ia32",
    "bin/linux-x64",
    "bin/linux-arm64",
...
#[derive(Default)]
pub struct Daemons {
    pub sydard: Option<Arc<dyn sydardCtl + Send + Sync + 'static>>,
    // pub sydard_automute : Arc<
    pub cpu_miner: Option<Arc<dyn CpuMinerCtl + Send + Sync + 'static>>,
}

impl Daemons {
    pub fn new() -> Self {
        Self { sydard: None, cpu_miner: None }
    }

    pub fn with_sydard(mut self, sydard: Arc<dyn sydardCtl + Send + Sync + 'static>) -> Self {
        self.sydard = Some(sydard);
        self
    }
    ...
    pub fn sydard(&self) -> Arc<dyn sydardCtl + Send + Sync + 'static> {
        self.sydard.as_ref().expect("accessing Daemons::sydard while sydard option is None").clone()
    }

    pub fn try_sydard(&self) -> Option<Arc<dyn sydardCtl + Send + Sync + 'static>> {
        self.sydard.clone()
    }
