...
impl TryFrom<sydardConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: sydardConfig) -> Result<Vec<String>> {
        let mut argv = Vec::new();

        if args.path.is_none() {
            return Err(Error::Custom("no sydard path is specified".to_string()));
        }
...
impl sydard {
    pub fn new(args: sydardConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: sydardConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process
            && process.is_running()
        {
            return Err(Error::Custom("sydar node is already running.".to_string()));
        }
        ...
        // let options = sydardOptions::new(path,network)?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("sydard binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}
...
pub trait sydardCtl {
    ...
    async fn configure(&self, config: sydardConfig) -> Result<()>;
