...
pub fn get_sydar_tempdir() -> TempDir {
    let global_tempdir = std::env::temp_dir();
    let sydar_tempdir = global_tempdir.join("sydar");
    std::fs::create_dir_all(sydar_tempdir.as_path()).unwrap();
    tempfile::tempdir_in(sydar_tempdir.as_path()).unwrap()
}

/// Creates a DB within a temp directory under `<OS SPECIFIC TEMP DIR>/sydar`
/// Callers must keep the `TempDbLifetime` guard for as long as they wish the DB to exist.
#[macro_export]
macro_rules! create_temp_db {
    ($conn_builder: expr) => {{
        let db_tempdir = $crate::utils::get_sydar_tempdir();
