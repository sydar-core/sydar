# Running sydar Archive Nodes

This guide explains how to run a sydar archive node with HDD-optimized RocksDB configuration.
...
Normal sydar nodes are **pruned** and only keep recent blocks (determined by pruning depth).
...
sydard provides two RocksDB configuration presets optimized for different storage types:

### Default Preset (SSD/NVMe)
```bash
sydard --archival
# or explicitly:
sydard --archival --rocksdb-preset=default
```
...
### HDD Preset
```bash
sydard --archival --rocksdb-preset=hdd
```
...
```bash
# /etc/sysctl.d/90-sydard-archive.conf
vm.dirty_ratio = 40
...
Apply with: `sudo sysctl -p /etc/sysctl.d/90-sydard-archive.conf`
...
sydard --archival --rocksdb-preset=hdd --ram-scale=0.2

# Normal RAM (8GB system)
sydard --archival --rocksdb-preset=hdd --ram-scale=0.5

# High RAM (16GB+ system)
sydard --archival --rocksdb-preset=hdd --ram-scale=1.0
```
...
sydard --archival --rocksdb-preset=hdd --rocksdb-cache-size=2048  # 2GB cache
```
...
```bash
# Using sydar-cli (if installed)
sydar-cli getinfo

# Check logs
journalctl -u sydard -f

# Check disk usage
du -sh ~/.sydar/sydar-mainnet/datadir/
```

### Performance Metrics
```bash
# Enable performance metrics
sydard --archival --rocksdb-preset=hdd --perf-metrics --perf-metrics-interval-sec=60
```
...
services:
  sydard-archive:
    image: sydarnet/sydard:latest
    container_name: sydard-archive
    ...
    command:
      ...
      - --utxoindex
    volumes:
      - /mnt/hdd/sydar-archive:/app/data
    ports:
      - "16111:16111"  # P2P
...
docker run -d \
  --name sydard-archive \
  --restart unless-stopped \
  -v /mnt/hdd/sydar-archive:/app/data \
  -p 16111:16111 \
  -p 17110:17110 \
  -p 18110:18110 \
  sydarnet/sydard:latest \
    --archival \
    --rocksdb-preset=hdd \
    --ram-scale=1.0 \
    --rpclisten-borsh=0.0.0.0:17110 \
...
**/etc/systemd/system/sydard-archive.service:**
```ini
[Unit]
Description=sydar Archive Node (HDD-optimized)
...
User=sydar
Group=sydar
ExecStart=/usr/local/bin/sydard \
  --archival \
  --rocksdb-preset=hdd \
  --ram-scale=1.0 \
  --appdir=/mnt/hdd/sydar-archive \
  --rpclisten-borsh=0.0.0.0:17110 \
  --rpclisten-json=0.0.0.0:18110 \
  --utxoindex
Restart=always
...
sudo systemctl enable sydard-archive
sudo systemctl start sydard-archive
sudo systemctl status sydard-archive
```
...
1. Verify HDD preset is active:
   ```bash
   journalctl -u sydard-archive | grep "RocksDB preset"
...
2. Reduce `--ram-scale` and cache size:
   ...
   sydard --archival --rocksdb-preset=hdd --rocksdb-cache-size=64
   ```
...
systemctl cat sydard-archive | grep rocksdb-preset

# Check startup logs
journalctl -u sydard-archive -n 100 | grep -i rocksdb
