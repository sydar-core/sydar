...
The `--override-params-file` flag lets you run `sydard` with a custom set of
...
3. Launch `sydard` with the flag:

	 ```bash
	 sydard --devnet --override-params-file /path/to/overrides.json
	 ```

If the file cannot be read or parsed, `sydard` prints the error and exits.
...
| sydar_consensus_k                          | sydarConsensus K                          |
...
If you want to run `sydard` with the yugat generated database, you'll need to ask it to generate an override params file as well, so you can run `sydard` with the same parameters.
...
And then launch sydard with:
```bash
sydard --simnet
```

and immedeiately close it. This will create the simnet datadir at `~/.sydar/sydar-simnet`.

You can then override it with the yugat database by running:
```bash
rm -rf ~/.sydar/sydar-simnet/datadir/consensus/consensus-001/
mv /path/to/yugat/database ~/.sydar/sydar-simnet/datadir/consensus/consensus-001/
```

And finally launch sydard with:
```bash
sydard --simnet --override-params-file overrides.json
```