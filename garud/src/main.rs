...
use sydar_consensus_core::{
    ...
    constants::{KANA_PER_sydar, TX_VERSION},
    network::NetworkType,
    sign::sign,
    subnets::SUBNETWORK_ID_NATIVE,
    tx::{MutableTransaction, Transaction, TransactionInput, TransactionOutpoint, TransactionOutput, UtxoEntry},
};
use sydar_core::{info, sydard_env::version, time::unix_now, warn};
use sydar_dilithium::{DilithiumKeyPair, generate_keypair, generate_keypair_from_seed};
use sydar_grpc_client::{ClientPool, GrpcClient};
use sydar_notify::subscription::context::SubscriptionContext;
use sydar_rpc_core::{RpcUtxoEntry, api::rpc::RpcApi, notify::mode::NotificationMode};
use sydar_txscript::pay_to_address_script;
use sha2::{Digest, Sha256};
use tokio::time::{Instant, MissedTickBehavior, interval};

const DEFAULT_SEND_AMOUNT: u64 = 10 * KANA_PER_sydar;
...
#[tokio::main]
async fn main() {
    sydar_core::log::init_logger(None, "");
    let args = Args::parse();
    ...
    let sydar_to_addr =
        args.addr.as_ref().map_or_else(|| sydar_addr.clone(), |addr_str| Address::try_from(addr_str.clone()).unwrap());
    ...
    let mut log_message = format!(
        ...
        \tfrom address: {}",
        args.network,
        sydar_dilithium::pubkey_to_hex(&dilithium_keypair),
        String::from(&sydar_addr)
    );
    if args.addr.is_some() {
        log_message.push_str(&format!("\n\tto address: {}", String::from(&sydar_to_addr)));
    }
    ...
    let info = rpc_client.get_sydar_dag_info().await.expect("Failed to get block dag info.");
...
\tMedian time: {}, \n\tDAA score: {}, \n\tPruning point: {}, \n\tTips: {}, \n\t{} virtual parents: ...{}, \n\tCoinbase maturity: {}",
    ...
    let mut utxos = refresh_utxos(&rpc_client, sydar_addr.clone(), &mut pending, coinbase_maturity).await;
    ...
    loop {
        ...
        if !has_funds || now - last_refresh > 60_000 {
            ...
            utxos = refresh_utxos(&rpc_client, sydar_addr.clone(), &mut pending, coinbase_maturity).await;
...
async fn refresh_utxos(
    ...
    populate_pending_outpoints_from_mempool(rpc_client, sydar_addr.clone(), pending).await;
    fetch_spendable_utxos(rpc_client, sydar_addr, coinbase_maturity, pending).await
}
...
async fn populate_pending_outpoints_from_mempool(
    ...
    let entries = rpc_client.get_mempool_entries_by_addresses(vec![sydar_addr], true, false).await.unwrap();
    let now = Instant::now();
...
async fn fetch_spendable_utxos(
    ...
    let resp = rpc_client.get_utxos_by_addresses(vec![sydar_addr]).await.unwrap();
    let dag_info = rpc_client.get_sydar_dag_info().await.unwrap();
...
fn generate_tx(
    ...
    let script_public_key = pay_to_address_script(sydar_addr);
    let inputs = utxos
        .iter()
