use rand::Rng;
use sydar_core::{time::Stopwatch, trace};
...
mod tests {
    ...
    use sydar_txscript::{pay_to_script_hash_signature_script, test_helpers::op_true_script};
    ...
    fn test_reject_transaction() {
        ...
        let transactions = (0..TX_INITIAL_COUNT).map(|i| create_transaction(KANA_PER_sydar * (i + 1) as u64)).collect_vec();
