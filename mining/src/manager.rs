...
use sydar_consensusmanager::{ConsensusProxy, spawn_blocking};
use sydar_core::{debug, error, info, time::Stopwatch, warn};
use sydar_mining_errors::{manager::MiningManagerError, mempool::RuleError};
use std::sync::Arc;
...
impl MiningManager {
                ...
                Err(BuilderError::ConsensusError(BlockRuleError::InvalidTransactionsInNewBlock(invalid_transactions))) => {
                    ...
                    invalid_transactions.iter().for_each(|(x, err)| {
                        ...
                        if let Err(err) = removal_result {
                            ...
                            // https://github.com/sydarnet/sydard/issues/1553
    ...
    pub(crate) fn get_realtime_feerate_estimations_verbose(
        ...
        {
            let script_public_key = sydar_txscript::pay_to_address_script(&sydar_addresses::Address::new(
                prefix,
                sydar_addresses::Version::PubKey,
                &[0u8; 32],
            ));
            let miner_data: MinerData = MinerData::new(script_public_key, vec![]);

            let BlockTemplate { block: sydar_consensus_core::block::MutableBlock { transactions, .. }, calculated_fees, .. } =
...
mod tests {
    use super::*;
    use sydar_consensus_core::subnets;
