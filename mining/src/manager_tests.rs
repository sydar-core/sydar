...
mod tests {
    ...
    use sydar_consensus_core::{
        ...
        coinbase::MinerData,
        constants::{KANA_PER_sydar, MAX_TX_IN_SEQUENCE_NUM, TX_VERSION},
    ...
    use sydar_mining_errors::mempool::RuleResult;
    use sydar_txscript::{
    ...
    fn test_replace_by_fee_in_mempool() {
        ...
        impl TxOp {
            fn change(&self) -> Option<u64> {
                self.change.then_some(900 * KANA_PER_sydar)
            }
        }
    ...
    fn test_revalidate_high_priority_transactions() {
        ...
        let (parent_tx, child_tx_1) = create_parent_and_children_transactions(&consensus, vec![3000 * KANA_PER_sydar]);
    ...
    fn generate_new_coinbase(address_prefix: Prefix, op: OpType) -> MinerData {
        match op {
            OpType::Usual => get_miner_data(address_prefix), // TODO: use lib_sydar_wallet.CreateKeyPair, util.NewAddressPublicKeyECDSA equivalents
            OpType::Edcsa => get_miner_data(address_prefix), // TODO: use lib_sydar_wallet.CreateKeyPair, util.NewAddressPublicKey equivalents
            OpType::True => {
    ...
    fn create_transaction_with_utxo_entry(i: u32, block_daa_score: u64) -> MutableTransaction {
        ...
        let entry = UtxoEntry::new(KANA_PER_sydar, script_public_key.clone(), block_daa_score, true);
        let output = TransactionOutput::new(KANA_PER_sydar - DEFAULT_MINIMUM_RELAY_TRANSACTION_FEE, script_public_key);
    ...
    fn create_and_add_funding_transactions(consensus: &Arc<ConsensusMock>, count: usize) -> Vec<Transaction> {
        ...
        (0..count)
            .map(|i| {
                let funding_tx =
                    create_transaction_without_input(vec![1_000 * KANA_PER_sydar, 2_500 * KANA_PER_sydar + i as u64]);
    ...
    fn create_arrays_of_parent_and_children_transactions(
        ...
        (0..count)
            .map(|i| {
                create_parent_and_children_transactions(consensus, vec![500 * KANA_PER_sydar, 3_000 * KANA_PER_sydar + i as u64])
            })
            .unzip()
    }
    ...
    fn create_child_and_parent_txs_and_add_parent_to_consensus(consensus: &Arc<ConsensusMock>) -> Transaction {
        let parent_tx = create_transaction_without_input(vec![500 * KANA_PER_sydar]);
    ...
    fn get_miner_data(prefix: Prefix) -> MinerData {
        let kp = sydar_dilithium::generate_keypair().unwrap();
