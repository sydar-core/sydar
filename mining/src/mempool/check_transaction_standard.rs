...
use sydar_consensus_core::{hashing::sighash::SigHashReusedValuesUnsync, mass::NonContextualMasses};
use sydar_txscript::{get_sig_op_count_upper_bound, is_unspendable, script_class::ScriptClass};
...
impl Mempool {
    pub(crate) fn check_transaction_standard_in_isolation(&self, transaction: &MutableTransaction) -> NonStandardResult<()> {
        ...
        // However, in a later version of sydar the consensus-valid transaction version range might diverge from the
...
mod tests {
    ...
    use sydar_consensus_core::constants::MAX_KANA;
    use sydar_consensus_core::{
    ...
    fn test_check_transaction_standard_in_isolation() {
        ...
        let dummy_prev_out = TransactionOutpoint::new(sydar_hashes::Hash::from_u64_word(1), 1);
        ...
        let dummy_script_public_key = sydar_txscript::pay_to_address_script(&addr);
        let dummy_tx_out = TransactionOutput::new(KANA_PER_sydar, dummy_script_public_key);
