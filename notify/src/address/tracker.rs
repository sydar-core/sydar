...
///
/// Each [`Address`] is stored internally as a [`ScriptPubKey`](sydar_consensus_core::tx::ScriptPublicKey).
...
mod tests {
    ...
    fn create_addresses(start: usize, count: usize) -> Vec<Address> {
        (start..start + count)
            .map(|i| Address::new(Prefix::Mainnet, sydar_addresses::Version::PubKey, &Uint256::from_u64(i as u64).to_le_bytes()))
            .collect()
    }
