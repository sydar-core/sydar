...
use sydar_connectionmanager::ConnectionManager;
use sydar_consensus_core::api::{BlockValidationFuture, BlockValidationFutures};
use sydar_consensus_core::block::Block;
use sydar_consensus_core::config::Config;
use sydar_consensus_core::errors::block::RuleError;
use sydar_consensus_core::tx::{Transaction, TransactionId};
use sydar_consensus_notify::{
    notification::{Notification, PruningPointUtxoSetOverrideNotification},
    root::ConsensusNotificationRoot,
};
use sydar_consensusmanager::{BlockProcessingBatch, ConsensusInstance, ConsensusManager, ConsensusProxy, ConsensusSessionOwned};
use sydar_core::{
    debug, info,
    sydard_env::{name, version},
    task::tick::TickService,
};
use sydar_core::{time::unix_now, warn};
use sydar_hashes::Hash;
use sydar_mining::mempool::tx::{Orphan, Priority};
use sydar_mining::{manager::MiningManagerProxy, mempool::tx::RbfPolicy};
use sydar_notify::notifier::Notify;
use sydar_p2p_lib::{
    ConnectionInitializer, Hub, PeerKey, PeerProperties, Router, sydardHandshake,
    common::ProtocolError,
    convert::model::version::Version,
    make_message,
    pb::{InvRelayBlockMessage, sydard_message::Payload},
};
...
impl FlowContext {
    pub fn new(
        ...
        let max_orphans =
            (2u64.pow(orphan_resolution_range) as usize * config.sydar_consensus_k() as usize).min(MAX_ORPHANS_UPPER_BOUND);
        Self {
    ...
    ///
    /// _GO-sydarD: OnNewBlock + broadcastTransactionsAfterBlockAdded_
