...
use sydar_consensusmanager::{ConsensusProxy, StagingConsensus, spawn_blocking};
use sydar_core::{debug, info, time::unix_now, warn};
...
use sydar_p2p_lib::{
    ...
    pb::{
        ...
        sydard_message::Payload,
    },
};
use sydar_utils::channel::JobReceiver;
use std::{
    sync::Arc,
...
impl IbdFlow {
    ...
    async fn sync_and_validate_pruning_proof(&mut self, staging: &ConsensusProxy, relay_block: &Block) -> Result<Hash, ProtocolError> {
        ...
        // blocks headers, and sydar_consensus data, which are required to verify the pruning
        ...
        // (including the PP itself), alongside indexing denoting the respective metadata headers or sydar_consensus data
        ...
        debug!(
            "received trusted data with {} daa entries and {} sydar_consensus entries",
            pkg.daa_window.len(),
            pkg.sydar_consensus_window.len()
        );
    ...
    async fn validate_staging_timestamps(
        ...
        // https://github.com/sydarnet/research/issues/3#issuecomment-895243792
    ...
    async fn sync_missing_trusted_bodies_no_headers(
        ...
        for chunk in iter {
            ...
            for &hash in chunk.iter() {
                ...
                // TODO (relaxed): sending sydar_consensus data may be redundant, especially when the headers were already verified.
                // Consider sending empty sydar_consensus data, simplifying a great deal. The result should be the same -
                ...
                jobs.push(
                    consensus
                        .validate_and_insert_trusted_block(TrustedBlock::new(
                            block,
                            consensus.async_get_sydar_consensus_data(hash).await?,
                        ))
                        .virtual_state_task,
                );
            }
