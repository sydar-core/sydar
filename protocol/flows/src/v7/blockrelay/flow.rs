...
use sydar_consensus_core::{api::BlockValidationFutures, block::Block, blockstatus::BlockStatus, errors::block::RuleError};
use sydar_consensusmanager::{BlockProcessingBatch, ConsensusProxy};
...
use sydar_p2p_lib::{
    ...
    pb::{InvRelayBlockMessage, RequestBlockLocatorMessage, RequestRelayBlocksMessage, sydard_message::Payload},
};
use sydar_utils::channel::{JobSender, JobTrySendError as TrySendError};
...
impl HandleRelayInvsFlow {
    ...
    async fn check_orphan_resolution_range(
        ...
        // with current syncer-side implementations (in both go-sydar and this codebase) we could query only the last one,
