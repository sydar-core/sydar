//!
//! In v6 of the P2P protocol we dropped the filling of DAA and sydar_CONSENSUS indices for each trusted entry
//! since the syncee no longer uses them in the sydar design where the full sub-DAG is sent
//!
                ...
                .enqueue(make_response!(
                    ...
                    TrustedDataMessage {
                        daa_window: trusted_data
                            ...
                            .collect_vec(),
                        sydar_consensus_data: trusted_data.sydar_consensus_blocks.iter().map(|gd| gd.into()).collect_vec()
                    },
                    request_id
                ))
                .await?;
