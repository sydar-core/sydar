...
use sydar_consensus_core::{
    ...
    trusted::{ExternalsydarConsensusData, TrustedHeader, TrustedsydarConsensusData},
};
...
impl From<&ExternalsydarConsensusData> for protowire::sydarConsensusData {
    fn from(item: &ExternalsydarConsensusData) -> Self {
        Self {
...
impl TryFrom<protowire::sydarConsensusData> for ExternalsydarConsensusData {
    type Error = ConversionError;
    fn try_from(item: protowire::sydarConsensusData) -> Result<Self, Self::Error> {
...
impl TryFrom<protowire::BlocksydarConsensusDataHashPair> for TrustedsydarConsensusData {
    type Error = ConversionError;
    fn try_from(pair: protowire::BlocksydarConsensusDataHashPair) -> Result<Self, Self::Error> {
        Ok(Self::new(pair.hash.try_into_ex()?, pair.sydar_consensus_data.try_into_ex()?))
    }
}
...
impl TryFrom<Versioned<protowire::DaaBlockV4>> for TrustedHeader {
    ...
    fn try_from(value: Versioned<protowire::DaaBlockV4>) -> Result<Self, Self::Error> {
        ...
        Ok(TrustedHeader::new(Versioned(header_format, header).try_into().map(Arc::new)?, b.sydar_consensus_data.try_into_ex()?))
    }
}
