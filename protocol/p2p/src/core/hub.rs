use crate::{ConnectionInitializer, Peer, Router, common::ProtocolError, pb::sydardMessage};
use parking_lot::RwLock;
use sydar_core::{debug, info, warn};
use std::{
...
impl Hub {
    ...
    pub async fn send(&self, peer_key: PeerKey, msg: sydardMessage) -> Result<bool, ProtocolError> {
    ...
    pub async fn broadcast(&self, msg: sydardMessage, filter_peer: Option<PeerKey>) {
        let peers = self
            .peers
            .read()
            .values()
    ...
    pub async fn broadcast_to_some_peers(&self, msg: sydardMessage, num_peers: usize) {
        assert!(num_peers > 0);
    ...
    pub async fn broadcast_many(&self, msgs: Vec<sydardMessage>, filter_peer: Option<PeerKey>) {
        if msgs.is_empty() {
            return;
        }
        let peers = self
