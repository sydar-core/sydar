...
use crate::pb::{ReadyMessage, VerackMessage, VersionMessage, sydard_message::Payload};
use crate::{IncomingRoute, Router, sydardMessagePayloadType};
use crate::{common::ProtocolError, dequeue_with_timeout, make_message};
use sydar_core::debug;

/// Implements the sydar peer-to-peer handshake protocol
pub struct sydardHandshake<'a> {
    router: &'a Router,
...
impl<'a> sydardHandshake<'a> {
    /// Builds the handshake object and subscribes to handshake messages
    pub fn new(router: &'a Router) -> Self {
        Self {
            router,
            version_receiver: router.subscribe(vec![sydardMessagePayloadType::Version]),
            verack_receiver: router.subscribe(vec![sydardMessagePayloadType::Verack]),
            ready_receiver: router.subscribe(vec![sydardMessagePayloadType::Ready]),
        }
    }
