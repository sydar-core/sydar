//!
//! The main [`RpcApi`] trait that defines all RPC methods available in the sydar p2p node.
//!
...
use sydar_notify::{listener::ListenerId, scope::Scope, subscription::Command};
use std::sync::Arc;
...
pub trait RpcApi: Sync + Send + AnySync {
    ...
    /// Requests the list of known sydard addresses in the current network (mainnet, testnet, etc.)
    ...
    async fn get_sydar_dag_info(&self) -> RpcResult<GetsydarDagInfoResponse> {
        self.get_sydar_dag_info_call(None, GetsydarDagInfoRequest {}).await
    }
    async fn get_sydar_dag_info_call(
        &self,
        connection: Option<&DynRpcConnection>,
        request: GetsydarDagInfoRequest,
