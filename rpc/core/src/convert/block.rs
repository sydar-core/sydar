...
use sydar_consensus_core::block::{Block, MutableBlock};
...
impl From<&Block> for RpcBlock {
    fn from(item: &Block) -> Self {
        Self {
            ...
            // TODO: Implement a populating process inspired from sydard\app\rpc\rpccontext\verbosedata.go
            verbose_data: None,
        }
    }
}
    ...
    fn try_from(item: RpcBlock) -> RpcResult<Self> {
        Ok(Self {
            ...
            transactions: Arc::new(
                item.transactions
                    .into_iter()
                    .map(sydar_consensus_core::tx::Transaction::try_from)
                    .collect::<RpcResult<Vec<sydar_consensus_core::tx::Transaction>>>()?,
            ),
        })
    }
}
