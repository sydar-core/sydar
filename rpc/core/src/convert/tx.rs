...
use sydar_consensus_core::tx::{Transaction, TransactionInput, TransactionOutput};
...
impl TryFrom<RpcTransaction> for Transaction {
    ...
    fn try_from(item: RpcTransaction) -> RpcResult<Self> {
        let transaction = Transaction::new(
            ...
            item.inputs
                .into_iter()
                .map(sydar_consensus_core::tx::TransactionInput::try_from)
                .collect::<RpcResult<Vec<sydar_consensus_core::tx::TransactionInput>>>()?,
            item.outputs
                .into_iter()
                .map(sydar_consensus_core::tx::TransactionOutput::try_from)
                .collect::<RpcResult<Vec<sydar_consensus_core::tx::TransactionOutput>>>()?,
            item.lock_time,
