...
use sydar_consensus_core::api::stats::BlockCount;
use sydar_core::debug;
use sydar_notify::subscription::{Command, context::SubscriptionContext, single::UtxosChangedSubscription};
...
pub struct GetBlockTemplateRequest {
    /// Which sydar address should the coinbase block reward transaction pay into
...
pub struct GetBlockTemplateResponse {
    ...
    /// Whether sydard thinks that it's synced.
    /// Callers are discouraged (but not forbidden) from solving blocks when sydard is not synced.
    /// That is because when sydard isn't in sync with the rest of the network there's a high
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetsydarDagInfoRequest {}

impl Serializer for GetsydarDagInfoRequest {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?;
        Ok(())
    }
...
impl Deserializer for GetsydarDagInfoRequest {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let _version = load!(u16, reader)?;
        Ok(Self {})
    }
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetsydarDagInfoResponse {
...
impl Serializer for GetsydarDagInfoResponse {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?;
...
impl Deserializer for GetsydarDagInfoResponse {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let _version = load!(u16, reader)?;
...
//
// This call is only available when this sydard was started with `--utxoindex`
//
