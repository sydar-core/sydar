...
impl From<sydar_consensus_client::TransactionOutpoint> for RpcOptionalTransactionOutpoint {
    fn from(outpoint: sydar_consensus_client::TransactionOutpoint) -> Self {
        TransactionOutpoint::from(outpoint).into()
    }
}

impl TryFrom<RpcOptionalTransactionOutpoint> for sydar_consensus_client::TransactionOutpoint {
    type Error = RpcError;
