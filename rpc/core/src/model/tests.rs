...
mod mockery {
    ...
    use sydar_consensus_core::api::BlockCount;
    ...
    use sydar_math::Uint192;
    use sydar_notify::subscription::Command;
    use sydar_rpc_macros::test_wrpc_serializer as test;
    use sydar_txscript::script_class::ScriptClass;
    use sydar_utils::networking::{ContextualNetAddress, IpAddress, NetAddress};
    ...
    impl Mock for GetsydarDagInfoRequest {
        fn mock() -> Self {
            GetsydarDagInfoRequest {}
        }
    }

    test!(GetsydarDagInfoRequest);

    impl Mock for GetsydarDagInfoResponse {
        fn mock() -> Self {
            GetsydarDagInfoResponse {
