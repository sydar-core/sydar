...
use sydar_addresses::AddressOrStringArrayT;
use sydar_consensus_client::UtxoEntryReference;
use sydar_consensus_client::{OptionalHeader, Transaction};
use sydar_consensus_core::tx as cctx;
...
declare! {
    IGetsydarDagInfoRequest,
    r#"
    /**
     * @category Node RPC
     */
    export interface IGetsydarDagInfoRequest { }
    "#,
}

try_from! ( args: IGetsydarDagInfoRequest, GetsydarDagInfoRequest, {
    Ok(from_value(args.into())?)
});

declare! {
    IGetsydarDagInfoResponse,
    r#"
    /**
     * @category Node RPC
     */
    export interface IGetsydarDagInfoResponse {
        network: string;
        blockCount: bigint;
        headerCount: bigint;
        tipHashes: HexString[];
...
try_from! ( args: GetsydarDagInfoResponse, IGetsydarDagInfoResponse, {
    Ok(to_value(&args)?.into())
});
