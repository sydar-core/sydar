...
use regex::Regex;
use sydar_core::{debug, error, trace};
use sydar_grpc_core::{
    RPC_MAX_MESSAGE_SIZE,
    channel::NotificationChannel,
    ops::sydardPayloadOps,
    protowire::{GetInfoRequestMessage, sydardRequest, sydardResponse, rpc_client::RpcClient, sydard_request},
};
use sydar_notify::{
...
use sydar_utils::{channel::Channel, triggers::DuplexTrigger};
use sydar_utils_tower::{
...
impl RpcApi for GrpcClient {
    ...
    //     self.inner.call(sydardPayloadOps::SubmitBlock, request).await?.as_ref().try_into()
    // }
    ...
    route!(get_sydar_dag_info_call, GetsydarDagInfo);
    ...
    async fn submit_account_transaction(
        ...
        Err(sydar_rpc_core::RpcError::NotImplemented)
    }
...
type sydardRequestSender = async_channel::Sender<sydardRequest>;
type sydardRequestReceiver = async_channel::Receiver<sydardRequest>;
...
/// Data flow:
/// ```
/// //   sydardRequest -> request_send -> stream -> sydardResponse
/// ```
///
/// Execution flow:
/// ```
...
struct Inner {
    ...
    // Sending to server
    request_sender: sydardRequestSender,
    request_receiver: sydardRequestReceiver,
...
impl Inner {
    ...
    async fn try_connect(
        ...
        request_timeout: u64,
        counters: Arc<TowerConnectionCounters>,
    ) -> Result<(Streaming<sydardResponse>, ServerFeatures)> {
        // gRPC endpoint
        ...
        // Actual sydardRequest to sydardResponse stream
        let mut stream: Streaming<sydardResponse> = client.message_stream(request_stream).await?.into_inner();
    ...
    async fn call(&self, op: sydardPayloadOps, request: impl Into<sydardRequest>) -> Result<sydardResponse> {
        // Calls are only allowed if the client is connected to the server
        if self.is_connected() {
            let id = u64::from_le_bytes(rand::random::<[u8; 8]>());
            let mut request: sydardRequest = request.into();
            request.id = id;
    ...
    fn spawn_response_receiver_task(self: Arc<Self>, mut stream: Streaming<sydardResponse>) {
        // Note: self is a cloned Arc here so that it can be used in the spawned task.
    ...
    async fn start_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        let request = sydard_request::Payload::from_notification_type(&scope, Command::Start);
    ...
    async fn stop_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        if self.handle_stop_notify() {
            let request = sydard_request::Payload::from_notification_type(&scope, Command::Stop);
            self.call((&request).into(), request).await?;
        }
        Ok(())
    }
