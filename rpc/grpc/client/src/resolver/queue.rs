use crate::{
    error::{Error, Result},
    resolver::{Resolver, sydardResponseReceiver, sydardResponseSender, matcher::Matcher},
};
...
struct Pending {
    ...
    sender: sydardResponseSender,
}

impl Pending {
    fn new(op: sydardPayloadOps, request: sydardRequest, sender: sydardResponseSender) -> Self {
        Self { timestamp: Instant::now(), op, request, sender }
    }

    fn is_matching(&self, response: &sydardResponse, response_op: sydardPayloadOps) -> bool {
...
impl Resolver for QueueResolver {
    fn register_request(&self, op: sydardPayloadOps, request: &sydardRequest) -> sydardResponseReceiver {
        let (sender, receiver) = oneshot::channel::<Result<sydardResponse>>();
        {
    ...
    fn handle_response(&self, response: sydardResponse) {
        let response_op: sydardPayloadOps = response.payload.as_ref().unwrap().try_into().expect("response is not a notification");
