...
use sydar_rpc_core::{FromRpcHex, RpcError, RpcHash, RpcResult, ToRpcHex};
use std::str::FromStr;
...
from!(item: &sydar_rpc_core::RpcHeader, protowire::RpcBlockHeader, {
    Self {
...
from!(item: &sydar_rpc_core::RpcRawHeader, protowire::RpcBlockHeader, {
    Self {
...
try_from!(item: &protowire::RpcBlockHeader, sydar_rpc_core::RpcHeader, {
    ...
    let header = Header::new_finalized(
        ...
        item.nonce,
        item.daa_score,
        sydar_rpc_core::RpcBlueWorkType::from_rpc_hex(&item.blue_work)?,
        item.blue_score,
...
try_from!(item: &protowire::RpcBlockHeader, sydar_rpc_core::RpcRawHeader, {
    Self {
        ...
        nonce: item.nonce,
        daa_score: item.daa_score,
        blue_work: sydar_rpc_core::RpcBlueWorkType::from_rpc_hex(&item.blue_work)?,
        blue_score: item.blue_score,
        pruning_point: RpcHash::from_str(&item.pruning_point)?,
    }
});

try_from!(item: &protowire::RpcBlockHeader, sydar_rpc_core::RpcOptionalHeader, {
    // We re-hash the block to remain as most trustless as possible
    let header = Header::new_finalized(
        item.version.try_into()?,
        item.parents.iter().map(Vec::<RpcHash>::try_from).collect::<RpcResult<Vec<Vec<RpcHash>>>>()?.try_into()?,
    ...
    sydar_rpc_core::RpcOptionalHeader::from(header)
});
...
mod tests {
    ...
    use sydar_consensus_core::{block::Block, header::Header};
    use sydar_rpc_core::{RpcBlock, RpcHash, RpcHeader};
