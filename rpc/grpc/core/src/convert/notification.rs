use crate::protowire::{
    BlockAddedNotificationMessage, NewBlockTemplateNotificationMessage, RpcNotifyCommand, sydardResponse,
    sydard_response::Payload,
};
use crate::protowire::{
...
use sydar_rpc_core::{Notification, RpcError, RpcHash};
use std::str::FromStr;
use std::sync::Arc;
...
from!(item: &sydar_rpc_core::Notification, sydardResponse, { Self { id: 0, payload: Some(item.into()) } });

from!(item: &sydar_rpc_core::Notification, Payload, {
    match item {
...
from!(item: &sydar_rpc_core::BlockAddedNotification, BlockAddedNotificationMessage, { Self { block: Some((&*item.block).into()) } });

from!(&sydar_rpc_core::NewBlockTemplateNotification, NewBlockTemplateNotificationMessage);

from!(item: &sydar_rpc_core::VirtualChainChangedNotification, VirtualChainChangedNotificationMessage, {
    Self {
...
from!(item: &sydar_rpc_core::FinalityConflictNotification, FinalityConflictNotificationMessage, {
    Self { violating_block_hash: item.violating_block_hash.to_string() }
});

from!(item: &sydar_rpc_core::FinalityConflictResolvedNotification, FinalityConflictResolvedNotificationMessage, {
    Self { finality_block_hash: item.finality_block_hash.to_string() }
});

from!(item: &sydar_rpc_core::UtxosChangedNotification, UtxosChangedNotificationMessage, {
    Self {
...
from!(item: &sydar_rpc_core::SinkBlueScoreChangedNotification, SinkBlueScoreChangedNotificationMessage, {
    Self { sink_blue_score: item.sink_blue_score }
});

from!(item: &sydar_rpc_core::VirtualDaaScoreChangedNotification, VirtualDaaScoreChangedNotificationMessage, {
    Self { virtual_daa_score: item.virtual_daa_score }
});

from!(&sydar_rpc_core::PruningPointUtxoSetOverrideNotification, PruningPointUtxoSetOverrideNotificationMessage);
...
try_from!(item: &sydardResponse, sydar_rpc_core::Notification, {
    item.payload
        .as_ref()
        .ok_or_else(|| RpcError::MissingRpcFieldError("sydardResponse".to_string(), "payload".to_string()))?
        .try_into()?
});

try_from!(item: &Payload, sydar_rpc_core::Notification, {
    match item {
...
try_from!(item: &BlockAddedNotificationMessage, sydar_rpc_core::BlockAddedNotification, {
    Self {
        block: Arc::new(
            item.block
                .as_ref()
...
try_from!(&NewBlockTemplateNotificationMessage, sydar_rpc_core::NewBlockTemplateNotification);

try_from!(item: &VirtualChainChangedNotificationMessage, sydar_rpc_core::VirtualChainChangedNotification, {
    Self {
...
try_from!(item: &FinalityConflictNotificationMessage, sydar_rpc_core::FinalityConflictNotification, {
    Self { violating_block_hash: RpcHash::from_str(&item.violating_block_hash)? }
});

try_from!(item: &FinalityConflictResolvedNotificationMessage, sydar_rpc_core::FinalityConflictResolvedNotification, {
    Self { finality_block_hash: RpcHash::from_str(&item.finality_block_hash)? }
});

try_from!(item: &UtxosChangedNotificationMessage, sydar_rpc_core::UtxosChangedNotification, {
    Self {
...
try_from!(item: &SinkBlueScoreChangedNotificationMessage, sydar_rpc_core::SinkBlueScoreChangedNotification, {
    Self { sink_blue_score: item.sink_blue_score }
});

try_from!(item: &VirtualDaaScoreChangedNotificationMessage, sydar_rpc_core::VirtualDaaScoreChangedNotification, {
    Self { virtual_daa_score: item.virtual_daa_score }
});

try_from!(&PruningPointUtxoSetOverrideNotificationMessage, sydar_rpc_core::PruningPointUtxoSetOverrideNotification);
