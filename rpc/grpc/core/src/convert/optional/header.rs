...
fn compressed_parents_to_protowire(parents: &sydar_rpc_core::RpcCompressedParents) -> Vec<protowire::RpcBlockLevelRun> {
    parents
        .raw()
        .iter()
...
fn compressed_parents_from_protowire(runs: &[protowire::RpcBlockLevelRun]) -> RpcResult<sydar_rpc_core::RpcCompressedParents> {
...
from!(item: &sydar_rpc_core::RpcOptionalHeader, protowire::RpcOptionalHeader, {
    Self {
...
try_from!(item: &protowire::RpcOptionalHeader, sydar_rpc_core::RpcOptionalHeader, {
    Self {
        ...
        nonce: item.nonce,
        daa_score: item.daa_score,
        blue_work: item.blue_work.as_ref().map(|x| sydar_rpc_core::RpcBlueWorkType::from_rpc_hex(x)).transpose()?,
        blue_score: item.blue_score,
