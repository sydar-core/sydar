...
pub enum GrpcServerError {
    ...
    RpcApiError(#[from] sydar_rpc_core::error::RpcError),
    ...
    ClosedHandler(sydardPayloadOps),
...
impl From<GrpcServerError> for sydar_rpc_core::error::RpcError {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => err,
            GrpcServerError::NotificationError(err) => err.into(),
            _ => sydar_rpc_core::error::RpcError::General(err.to_string()),
        }
    }
}

impl From<GrpcServerError> for sydar_notify::error::Error {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => sydar_notify::error::Error::General(err.to_string()),
            GrpcServerError::NotificationError(err) => err,
            _ => sydar_notify::error::Error::General(err.to_string()),
        }
    }
}
...
impl<T> From<TrySendError<T>> for GrpcServerError {
    fn from(_: TrySendError<T>) -> Self {
        sydar_notify::error::Error::ChannelSendError.into()
    }
}
