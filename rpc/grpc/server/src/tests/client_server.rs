...
use sydar_notify::scope::{NewBlockTemplateScope, Scope};
use sydar_rpc_core::api::rpc::RpcApi;
use sydar_utils::networking::{ContextualNetAddress, NetAddress};
use std::sync::Arc;

#[tokio::test]
async fn test_client_server_sanity_check() {
    sydar_core::log::try_init_logger("info, sydar_grpc_core=trace, sydar_grpc_server=trace, sydar_grpc_client=trace");
