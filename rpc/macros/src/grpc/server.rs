...
struct RpcTable {
    ...
    sydard_request_type: Expr,
    sydard_response_type: Expr,
    payload_ops: Expr,
    handlers: ExprArray,
}
    ...
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if parsed.len() != 7 {
            return Err(Error::new_spanned(parsed,
                "usage: build_grpc_server_interface!(server_context, ServerContextType, ConnectionType, sydardRequestType, sydardResponseType, sydardPayloadOps, [GetInfo, ..])".to_string()));
        }
        ...
        let sydard_request_type = iter.next().unwrap().clone();
        let sydard_response_type = iter.next().unwrap().clone();
...
impl ToTokens for RpcTable {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let sydard_request_type = &self.sydard_request_type;
        let sydard_response_type = &self.sydard_response_type;
        ...
        for handler in self.handlers.elems.iter() {
            ...
            match is_subscription {
                false => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #sydard_request_type, #sydard_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, _: #connection_ctx_type, request: #sydard_request_type| {
                                Box::pin(async move {
                                    let mut response: #sydard_response_type = match request.payload {
                ...
                true => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #sydard_request_type, #sydard_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, connection: #connection_ctx_type, request: #sydard_request_type| {
                                Box::pin(async move {
                                    let mut response: #sydard_response_type = match request.payload {
                                        Some(Payload::#request_type(ref request)) => {
                                            match sydar_rpc_core::#fallback_request_type::try_from(request) {
                                                Ok(request) => {
