...
use sydar_addresses::{Address, AddressError};
...
use sydar_consensus_notify::notification::{self as consensus_notify, Notification as ConsensusNotification};
use sydar_consensusmanager::{ConsensusManager, ConsensusProxy};
use sydar_hashes::Hash;
use sydar_math::Uint256;
use sydar_mining::model::{TransactionIdSet, owner_txs::OwnerTransactions};
use sydar_notify::converter::Converter;
use sydar_rpc_core::{
...
use sydar_txscript::{extract_script_pub_key_address, script_class::ScriptClass};
...
impl ConsensusConverter {
    ...
    ///
    /// _GO-sydarD: PopulateBlockWithVerboseData_
    pub async fn get_block(
        ...
        let hash = block.hash();
        let sydar_consensus_data = consensus.async_get_sydar_consensus_data(hash).await?;
        ...
        let verbose_data = Some(RpcBlockVerboseData {
            ...
            selected_parent_hash: sydar_consensus_data.selected_parent,
            transaction_ids: block.transactions.iter().map(|x| x.id()).collect(),
            is_header_only: block_status.is_header_only(),
            blue_score: sydar_consensus_data.blue_score,
            children_hashes: children,
            merge_set_blues_hashes: sydar_consensus_data.mergeset_blues,
            merge_set_reds_hashes: sydar_consensus_data.mergeset_reds,
            is_chain_block,
        });
    ...
    ///
    /// _GO-sydarD: PopulateTransactionWithVerboseData
