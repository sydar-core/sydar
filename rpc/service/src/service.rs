...
use sydar_consensus_core::api::counters::ProcessingCounters;
use sydar_consensus_core::daa_score_timestamp::DaaScoreTimestamp;
use sydar_consensus_core::errors::block::RuleError;
use sydar_consensus_core::tx::{TransactionQueryResult, TransactionType};
use sydar_consensus_core::utxo::utxo_inquirer::UtxoInquirerError;
...
use sydar_mining::feerate::FeeEstimateVerbose;
use sydar_mining::manager::MiningManagerProxy;
use sydar_mining::mempool::tx::{Orphan, Priority, RbfPolicy};
use sydar_mining::model::tx_query::TransactionQuery;
use sydar_notify::listener::ListenerLifespan;
use sydar_notify::subscription::context::SubscriptionContext;
use sydar_notify::subscription::{MutationPolicies, UtxosChangedMutationPolicy};
use sydar_notify::{
...
use sydar_p2p_flows::flow_context::FlowContext;
use sydar_p2p_lib::common::ProtocolError;
use sydar_p2p_mining::rule_engine::MiningRuleEngine;
use sydar_perf_monitor::{Monitor as PerfMonitor, counters::CountersSnapshot};
use sydar_rpc_core::{
...
use sydar_txscript::{extract_script_pub_key_address, pay_to_address_script};
use sydar_utils::expiring_cache::ExpiringCache;
use sydar_utils::sysinfo::SystemInfo;
use sydar_utils::{channel::Channel, triggers::SingleTrigger};
use sydar_utils_tower::counters::TowerConnectionCounters;
use sydar_utxoindex::api::UtxoIndexProxy;
...
/// A service implementing the Rpc API at sydar_rpc_core level.
///
...
pub struct RpcCoreService {
    ...
    fee_estimate_verbose_cache: ExpiringCache<sydar_mining::errors::MiningManagerResult<GetFeeEstimateExperimentalResponse>>,
...
impl RpcCoreService {
    ...
    async fn submit_account_transaction_call(
        ...
        let tx = sydar_consensus_core::tx::Transaction::new(
            0,
            vec![],
            vec![sydar_consensus_core::tx::TransactionOutput::new(
                request.amount,
                sydar_consensus_core::tx::ScriptPublicKey::new(0, receiver_bytes.into()),
            )],
            0,
            sydar_consensus_core::subnets::SubnetworkId::from_bytes([0; 20]),
            0,
            payload,
        );
...
NOTE: This error usually indicates an RPC conversion error between the node and the miner. This is likely to reflect using a NON-SUPPORTED miner.",
    ...
    async fn get_block_template_call(
        ...
        if request.pay_address.prefix != self.config.prefix() {
            return Err(sydar_addresses::AddressError::InvalidPrefix(request.pay_address.prefix.to_string()))?;
        }
        ...
        let script_public_key = sydar_txscript::pay_to_address_script(&request.pay_address);
    ...
    async fn get_blocks_call(
        ...
        let low_hash = match request.low_hash {
            Some(low_hash) => {
                ...
                session.async_get_sydar_consensus_data(low_hash).await?;
                low_hash
            }
    ...
    async fn get_sink_blue_score_call(
        ...
        let session = self.consensus_manager.consensus().unguarded_session();
        Ok(GetSinkBlueScoreResponse::new(session.async_get_sydar_consensus_data(session.async_get_sink().await).await?.blue_score))
    }
    ...
    async fn get_utxos_by_addresses_call(
        ...
        for address in request.addresses {
            ...
            if balance > 0 {
                let script_public_key = sydar_txscript::pay_to_address_script(&address);

                let outpoint = sydar_rpc_core::RpcTransactionOutpoint { transaction_id: Default::default(), index: 0 };

                let utxo_entry =
                    sydar_rpc_core::RpcUtxoEntry { amount: balance, script_public_key, block_daa_score: 0, is_coinbase: false };

                entries.push(sydar_rpc_core::RpcUtxosByAddressesEntry { address: Some(address.clone()), outpoint, utxo_entry });
            }
        }
    ...
    async fn get_sydar_dag_info_call(
        &self,
        _connection: Option<&DynRpcConnection>,
        _: GetsydarDagInfoRequest,
    ) -> RpcResult<GetsydarDagInfoResponse> {
        let session = self.consensus_manager.consensus().unguarded_session();
        ...
        Ok(GetsydarDagInfoResponse::new(
            self.config.net,
    ...
    async fn estimate_network_hashes_per_second_call(
        ...
        const LEGACY_VIRTUAL: sydar_hashes::Hash = sydar_hashes::Hash::from_bytes([0xff; sydar_hashes::HASH_SIZE]);
