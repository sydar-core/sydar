//! sydar wRPC client implementation.
...
pub use sydar_rpc_macros::build_wrpc_client_interface;
use std::fmt::Debug;
...
impl Inner {
    pub fn new(encoding: Encoding, url: Option<&str>, resolver: Option<Resolver>, network_id: Option<NetworkId>) -> Result<Inner> {
        // log_trace!("sydar wRPC::{encoding} connecting to: {url}");
        ...
        .for_each(|notification_op| {
            ...
            interface.notification(
                notification_op,
                workflow_rpc::client::Notification::new(move |notification: Serializable<sydar_rpc_core::Notification>| {
                    ...
                    Box::pin(async move {
                        ...
                        if notification_sender.receiver_count() > 1 {
                            ...
                            log_warn!("WARNING: sydar RPC notification is not consumed by user: {:?}", notification.into_inner());
                        }
                        Ok(())
                    })
                }),
...
/// # [`sydarRpcClient`] connects to sydar wRPC endpoint via binary Borsh or JSON protocols.
///
...
///
#[derive(Clone)]
pub struct sydarRpcClient {
    inner: Arc<Inner>,
}

impl Debug for sydarRpcClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("sydarRpcClient").field("url", &self.url()).field("connected", &self.is_connected()).finish()
    }
}

impl sydarRpcClient {
    /// Create a new `sydarRpcClient` with the given Encoding, and an optional url or a Resolver.
    /// Please note that if you pass the url to the constructor, it will force the sydarRpcClient
    ...
    ) -> Result<sydarRpcClient> {
        Self::new_with_args(encoding, url, resolver, network_id, subscription_context)
        // FIXME
        // pub fn new(encoding: Encoding, url: &str, ) -> Result<sydarRpcClient> {
    ...
    pub fn new_with_args(
        ...
        let client = sydarRpcClient { inner };
        ...
        // let client = sydarRpcClient { inner, notifier, notification_mode };
            ...
            loop {
                select! {
                    ...
                    msg = notification_relay_channel.receiver.recv().fuse() => {
                        if let Ok(msg) = msg {
                            // inner.rpc_ctl.notify(msg).await.expect("(sydarRpcClient) rpc_ctl.notify() error");
                    ...
                    msg = wrpc_ctl_channel.receiver.recv().fuse() => {
                        if let Ok(msg) = msg {
                            match msg {
                                WrpcCtl::Connect => {
                                    inner.rpc_ctl.signal_open().await.expect("(sydarRpcClient) rpc_ctl.signal_open() error");
                                }
                                WrpcCtl::Disconnect => {
                                    inner.rpc_ctl.signal_close().await.expect("(sydarRpcClient) rpc_ctl.signal_close() error");
                                }
                            }
                        } else {
...
#[async_trait]
impl RpcApi for sydarRpcClient {
    //
    ...
    async fn submit_account_transaction(
        &self,
        _request: sydar_rpc_core::SubmitAccountTransactionRequest,
    ) -> sydar_rpc_core::RpcResult<sydar_rpc_core::SubmitAccountTransactionResponse> {
