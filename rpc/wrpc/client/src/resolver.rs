//!
//! Module implementing [`Resolver`] client for obtaining public sydar wRPC endpoints.
//!
...
///
/// # Resolver - a client for obtaining public sydar wRPC endpoints.
///
/// This client operates against [sydar Resolver](https://github.com/aspectron/sydar-resolver) service
/// that provides load-balancing and failover capabilities for sydar wRPC endpoints. The default
/// configuration allows access to public sydar nodes, while custom configurations can be supplied
/// if you are running your own custom sydar node cluster.
///
#[derive(Debug, Clone)]
pub struct Resolver {
    inner: Arc<Inner>,
...
impl Resolver {
    ...
    fn make_url(&self, url: &str, encoding: Encoding, network_id: NetworkId) -> String {
        ...
        format!("{url}/v{CURRENT_VERSION}/sydar/{network_id}/{tls}/wrpc/{encoding}")
    }
    ...
    /// Obtain a sydar p2p [`NodeDescriptor`] from the resolver based on the supplied [`Encoding`] and [`NetworkId`].
    ...
    /// Returns a sydar wRPC URL from the resolver based on the supplied [`Encoding`] and [`NetworkId`].
