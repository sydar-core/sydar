// Basic example of a sydar wRPC client that connects to a node
...
// sydar RPC primitives
use sydar_wrpc_client::prelude::*;
// reuse wRPC Result type for convenience
use sydar_wrpc_client::result::Result;
...
impl Listener {
    pub fn try_new(network_id: NetworkId, url: Option<String>) -> Result<Self> {
        ...
        // Create a basic sydar RPC client instance using Borsh encoding.
        let client =
            Arc::new(sydarRpcClient::new_with_args(WrpcEncoding::Borsh, url.as_deref(), resolver, Some(network_id), None)?);
