...
use sydar_rpc_core::{RpcDataVerbosityLevel, RpcHash, RpcOptionalTransaction, api::rpc::RpcApi};
use sydar_wrpc_client::{
    sydarRpcClient, WrpcEncoding,
...
async fn get_vcc_v2() -> Result<()> {
    ...
    let dag_info = client.get_sydar_dag_info().await?;
