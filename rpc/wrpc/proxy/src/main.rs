...
use sydar_wrpc_server::{
    connection::Connection,
    router::Router,
    server::Server,
    service::{Options, sydarRpcHandler},
};
use std::sync::Arc;
use workflow_log::*;
...
async fn main() -> Result<()> {
    ...
    let sydard_port = network_type.default_rpc_port();
    ...
    let options = Arc::new(Options {
        ...
        grpc_proxy_address: Some(grpc_proxy_address.unwrap_or_else(|| format!("grpc://127.0.0.1:{sydard_port}"))),
        verbose,
        // ..Options::default()
    });
    log_info!("");
    ...
    let rpc_handler = Arc::new(sydarRpcHandler::new(tasks, encoding, None, options.clone()));
    ...
    log_info!("sydar wRPC server is listening on {}", options.listen_address);
