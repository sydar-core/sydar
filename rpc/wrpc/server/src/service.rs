...
use sydar_rpc_core::api::ops::RpcApiOps;
...
/// ### sydarRpcHandler
///
/// [`sydarRpcHandler`] is a handler struct that implements the [`RpcHandler`] trait
...
///
pub struct sydarRpcHandler {
    pub server: Server,
    pub options: Arc<Options>,
}

impl sydarRpcHandler {
    pub fn new(
        tasks: usize,
        ...
        sydarRpcHandler { server: Server::new(tasks, encoding, core_service, options.clone()), options }
    }
}

#[async_trait]
impl RpcHandler for sydarRpcHandler {
...
pub struct WrpcService {
    ...
    options: Arc<Options>,
    server: RpcServer,
    rpc_handler: Arc<sydarRpcHandler>,
...
impl WrpcService {
    ...
    pub fn new(
        ...
        let rpc_handler = Arc::new(sydarRpcHandler::new(tasks, *encoding, core_service, options.clone()));
