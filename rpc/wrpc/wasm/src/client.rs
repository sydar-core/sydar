//!
//! # WASM bindings for the [sydar p2p Node RPC client](sydarRpcClient).
//!
//! This module provides a WASM interface for the sydar p2p Node RPC client
//! - [`RpcClient`].
//!
...
use sydar_addresses::{Address, AddressOrStringArrayT};
use sydar_consensus_client::UtxoEntryReference;
use sydar_consensus_core::network::{NetworkType, NetworkTypeT};
use sydar_notify::connection::ChannelType;
use sydar_notify::events::EventType;
use sydar_notify::listener;
use sydar_notify::notification::Notification as NotificationT;
use sydar_rpc_core::api::ctl;
pub use sydar_rpc_core::wasm::message::*;
pub use sydar_rpc_macros::{
    build_wrpc_wasm_bindgen_interface, build_wrpc_wasm_bindgen_subscriptions, declare_typescript_wasm_interface as declare,
};
use sydar_wasm_core::events::{Sink, get_event_targets};
...
pub(crate) struct Inner {
    ...
    notification_channel: Channel<sydar_rpc_core::Notification>,
}
...
///
///
/// sydar RPC client uses ([wRPC](https://github.com/workflow-rs/workflow-rs/tree/master/rpc))
/// interface to connect directly with sydar Node. wRPC supports
/// two types of encodings: `borsh` (binary, default) and `json`.
///
/// There are two ways to connect: Directly to any sydar Node or to a
...
/// ```
///
/// **Connecting to a sydar Node directly**
///
/// ```javascript
/// let rpc = new RpcClient({
...
/// to re-register event listeners, but your have to re-subscribe for sydar node
/// notifications:
///
/// ```typescript
/// rpc.addEventListener("connect", async (event) => {
...
impl RpcClient {
    pub fn new(config: Option<RpcConfig>) -> Result<RpcClient> {
        ...
        let client = Arc::new(
            sydarRpcClient::new(encoding, url.as_deref(), resolver.clone().map(Into::into), network_id, None)
...
impl RpcClient {
    ...
    /// Connect to the sydar RPC server. This function starts a background
    ...
    /// Disconnect from the sydar RPC server.
    ...
    /// however, you have to re-subscribe for sydar node notifications. As such, it is recommended
...
impl RpcClient {
    pub fn new_with_rpc_client(client: Arc<sydarRpcClient>) -> RpcClient {
    ...
    pub fn client(&self) -> &Arc<sydarRpcClient> {
        &self.inner.client
    }
                ...
                select_biased! {
                    ...
                    msg = notification_receiver.recv().fuse() => {
                        if let Ok(notification) = &msg {
                            match &notification {
                                sydar_rpc_core::Notification::UtxosChanged(utxos_changed_notification) => {
...
impl RpcClient {
    ...
    pub fn parse_url(url: &str, encoding: Encoding, network: NetworkId) -> Result<String> {
        let url_ = sydarRpcClient::parse_url(url.to_string(), encoding, network.into())?;
        Ok(url_)
    }
}
...
impl RpcClient {
    ...
    /// Difficulty Adjustment Algorithm (DAA) score changes in the sydar sydarDAG.
    ...
    /// sydar sydarDAG. The event notification will be scoped to the
    ...
    /// chain changes in the sydar sydarDAG.
...
build_wrpc_wasm_bindgen_subscriptions!([
    ...
    /// block is added to the sydar sydarDAG.
    BlockAdded,
    ...
    /// conflict occurs in the sydar sydarDAG.
    FinalityConflict,
    ...
    /// conflict in the sydar sydarDAG is resolved.
    ...
    /// score of the sink block changes in the sydar sydarDAG.
    ...
    /// UTXO set override for the pruning point changes in the sydar sydarDAG.
    ...
    /// template is generated for mining in the sydar sydarDAG.
    NewBlockTemplate,
]);
...
build_wrpc_wasm_bindgen_interface!(
    [
        ...
        /// Retrieves the current number of blocks in the sydar sydarDAG.
        ...
        /// structure of the sydar sydarDAG.
        ...
        GetsydarDagInfo,
        /// Returns the total current coin supply of sydar network.
        /// Returned information: Total coin supply.
        GetCoinSupply,
        /// Retrieves information about the peers connected to the sydar node.
        /// Returned information: Peer ID, IP address and port, connection
        /// status, protocol version.
        GetConnectedPeerInfo,
        /// Retrieves general information about the sydar node.
        /// Returned information: Version of the sydar node, protocol
        /// version, network identifier.
        /// This call is primarily used by gRPC clients.
        /// For wRPC clients, use {@link RpcClient.getServerInfo}.
        GetInfo,
        /// Provides a list of addresses of known peers in the sydar
        ...
        /// performance and status of the sydar node.
        ...
        /// the highest cumulative difficulty in the sydar sydarDAG.
        ...
        GetSinkBlueScore,
        /// Tests the connection and responsiveness of a sydar node.
        /// Returned information: None.
        Ping,
        /// Gracefully shuts down the sydar node.
        /// Returned information: None.
        Shutdown,
        /// Retrieves information about the sydar server.
        /// Returned information: Version of the sydar server, protocol
        /// version, network identifier.
        GetServerInfo,
        /// Obtains basic information about the synchronization status of the sydar node.
    ...
    [
        ...
        /// Adds a peer to the sydar node's list of known peers.
        /// Returned information: None.
        AddPeer,
        /// Bans a peer from connecting to the sydar node for a specified duration.
        /// Returned information: None.
        Ban,
        ...
        /// Retrieves the balance of a specific address in the sydar sydarDAG.
        /// Returned information: Balance of the address.
        GetBalanceByAddress,
        /// Retrieves balances for multiple addresses in the sydar sydarDAG.
        /// Returned information: Balances of the addresses.
        GetBalancesByAddresses,
        /// Retrieves a specific block from the sydar sydarDAG.
        /// Returned information: Block information.
        GetBlock,
        /// Retrieves multiple blocks from the sydar sydarDAG.
        ...
        /// Retrieves block headers from the sydar sydarDAG.
        /// Returned information: List of block headers.
        GetHeaders,
        /// Retrieves mempool entries from the sydar node's mempool.
        ...
        GetMempoolEntry,
        /// Retrieves information about a subnetwork in the sydar sydarDAG.
        ...
        /// Resolves a finality conflict in the sydar sydarDAG.
        /// Returned information: None.
        ResolveFinalityConflict,
        /// Submits a block to the sydar network.
        /// Returned information: None.
        SubmitBlock,
        /// Submits a transaction to the sydar network.
        /// Returned information: Submitted Transaction Id.
        SubmitTransaction,
        /// Submits an RBF transaction to the sydar network.
