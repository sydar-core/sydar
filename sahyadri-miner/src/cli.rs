...
pub struct Opt {
    ...
    /// The sydar address for the miner reward
    pub mining_address: String,
    #[clap(short = 's', long = "sydard-address", default_value = "127.0.0.1", display_order = 1)]
    /// The IP of the sydard instance
    pub sydard_address: String,

    #[clap(long = "devfund", display_order = 6)]
    /// Mine a percentage of the blocks to the sydar devfund [default: Off]
    ...
    /// sydard port [default: Mainnet = 26110, Testnet = 16210]
    port: Option<u16>,
    ...
    /// Mine even when sydard says it is not synced, only useful when passing `--allow-submit-block-when-not-synced` to sydard  [default: false]
    ...
    /// Output logs in alternative format (same as sydard)
    pub altlogs: bool,
}
...
impl Opt {
    pub fn process(&mut self) -> Result<(), Error> {
        if self.sydard_address.is_empty() {
            self.sydard_address = "127.0.0.1".to_string();
        }

        if !self.sydard_address.starts_with("grpc://") {
            IpAddr::from_str(&self.sydard_address)?;
            let port = self.port();
            self.sydard_address = format!("grpc://{}:{}", self.sydard_address, port);
        }
        log::info!("sydard address: {}", self.sydard_address);
