use crate::{
    miner::MinerManager,
    proto::{
        sydard_message::Payload, rpc_client::RpcClient, GetBlockTemplateRequestMessage, GetInfoRequestMessage,
        sydardMessage,
    },
...
#[allow(dead_code)]
pub struct sydardHandler {
    client: RpcClient<TonicChannel>,
    pub send_channel: Sender<sydardMessage>,
    stream: Streaming<sydardMessage>,
...
impl sydardHandler {
    ...
    pub async fn client_send(&self, msg: impl Into<sydardMessage>) -> Result<(), SendError<sydardMessage>> {
        self.send_channel.send(msg.into()).await
    }

    pub async fn client_get_block_template(&mut self) -> Result<(), SendError<sydardMessage>> {
    ...
    pub async fn listen(&mut self, miner: &mut MinerManager, shutdown: ShutdownHandler) -> Result<(), Error> {
        while let Some(msg) = self.stream.message().await? {
            ...
            match msg.payload {
                ...
                None => warn!("sydard message payload is empty"),
            }
        }
        Ok(())
    }
    ...
    async fn handle_message(&mut self, msg: Payload, miner: &mut MinerManager) -> Result<(), Error> {
        match msg {
            ...
            Payload::GetInfoResponse(info) => info!("sydard version: {}", info.server_version),
