...
use crate::{
    cli::Opt, client::sydardHandler, miner::MinerManager, proto::NotifyNewBlockTemplateRequestMessage, target::Uint256,
};
...
async fn main() -> Result<(), Error> {
    ...
    while !shutdown.is_shutdown() {
        let mut client =
            sydardHandler::connect(opt.sydard_address.clone(), opt.mining_address.clone(), opt.mine_when_not_synced)
                .await?;
        ...
        warn!("Disconnected from sydard, retrying");
    }
    Ok(())
}
