...
use sydar_core::sydard_env::version;
use sydar_notify::address::tracker::Tracker;
use sydar_utils::networking::ContextualNetAddress;
use sydar_wrpc_server::address::WrpcNetAddress;
...
#[cfg(feature = "devnet-prealloc")]
use sydar_consensus_core::tx::{TransactionOutpoint, UtxoEntry};
#[cfg(feature = "devnet-prealloc")]
use sydar_txscript::pay_to_address_script;
#[cfg(feature = "devnet-prealloc")]
use std::sync::Arc;
...
impl Args {
    ...
    #[cfg(feature = "devnet-prealloc")]
    pub fn generate_prealloc_utxos(&self, num_prealloc_utxos: u64) -> sydar_consensus_core::utxo::utxo_collection::UtxoCollection {
...
pub fn cli() -> Command {
    ...
    let cmd = Command::new("sydard")
        .about(format!("{} (sydar) v{}", env!("CARGO_PKG_DESCRIPTION"), version()))
        .version(env!("CARGO_PKG_VERSION"))
        .arg(arg!(-C --configfile <CONFIG_FILE> "Path of config file.").env("sydarD_CONFIGFILE"))
        .arg(arg!(-b --appdir <DATA_DIR> "Directory to store data.").env("sydarD_APPDIR"))
        .arg(arg!(--logdir <LOG_DIR> "Directory to log output.").env("sydarD_LOGDIR"))
        .arg(arg!(--nologfiles "Disable logging to files.").env("sydarD_NOLOGFILES"))
        .arg(
            Arg::new("async_threads")
                .short('t')
                .long("async-threads")
                .env("sydarD_ASYNC_THREADS")
        ...
        .arg(arg!(--unsaferpc "Enable RPC commands which affect the state of the node").env("sydarD_UNSAFERPC"))
        .arg(
            Arg::new("connect-peers")
                .long("connect")
                .env("sydarD_CONNECTPEERS")
                .value_name("IP[:PORT]")
        ...
        .arg(
            Arg::new("rpcmaxclients")
                .long("rpcmaxclients")
                .env("sydarD_RPCMAXCLIENTS")
                .value_name("rpcmaxclients")
                .require_equals(true)
                .value_parser(clap::value_parser!(usize))
                .help("Max number of RPC clients for standard connections (default: 128)."),
        )
        .arg(arg!(--"reset-db" "Reset database before starting node. It's needed when switching between subnetworks.").env("sydarD_RESET_DB"))
        .arg(arg!(--"enable-unsynced-mining" "Allow the node to accept blocks from RPC while not synced (this flag is mainly used for testing)").env("sydarD_ENABLE_UNSYNCED_MINING"))
        .arg(
            Arg::new("enable-mainnet-mining")
                .long("enable-mainnet-mining")
                .env("sydarD_ENABLE_MAINNET_MINING")
                .action(ArgAction::SetTrue)
                .hide(true)
                .help("Allow mainnet mining (currently enabled by default while the flag is kept for backwards compatibility)"),
        )
        .arg(arg!(--utxoindex "Enable the UTXO index").env("sydarD_UTXOINDEX"))
        .arg(
            Arg::new("max-tracked-addresses")
                .long("max-tracked-addresses")
                .env("sydarD_MAX_TRACKED_ADDRESSES")
                .require_equals(true)
                .value_parser(clap::value_parser!(usize))
...
0, Tracker::MAX_ADDRESS_UPPER_BOUND, Tracker::DEFAULT_MAX_ADDRESSES)),
        ...
        .arg(arg!(--testnet "Use the test network").env("sydarD_TESTNET"))
        .arg(
            Arg::new("netsuffix")
                .long("netsuffix")
                .env("sydarD_NETSUFFIX")
                .value_name("netsuffix")
                .require_equals(true)
                .value_parser(clap::value_parser!(u32))
                .help("Testnet network suffix number"),
        )
        .arg(arg!(--devnet "Use the development test network").env("sydarD_DEVNET"))
        .arg(arg!(--simnet "Use the simulation test network").env("sydarD_SIMNET"))
        .arg(arg!(--archival "Run as an archival node: avoids deleting old block data when moving the pruning point (Warning: heavy disk usage)").env("sydarD_ARCHIVAL"))
        .arg(arg!(--sanity "Enable various sanity checks which might be compute-intensive (mostly performed during pruning)").env("sydarD_SANITY"))
        .arg(arg!(--yes "Answer yes to all interactive console questions").env("sydarD_NONINTERACTIVE"))
        .arg(
            Arg::new("user_agent_comments")
                .long("uacomment")
                .env("sydarD_USER_AGENT_COMMENTS")
                .action(ArgAction::Append)
                .require_equals(true)
        ...
        .arg(arg!(--"perf-metrics" "Enable performance metrics: cpu, memory, disk io usage").env("sydarD_PERF_METRICS"))
        .arg(
            Arg::new("perf-metrics-interval-sec")
                .long("perf-metrics-interval-sec")
                .env("sydarD_PERF_METRICS_INTERVAL_SEC")
                .require_equals(true)
                .value_parser(clap::value_parser!(u64))
                .help("Interval in seconds for performance metrics collection."),
        )
        .arg(arg!(--"disable-upnp" "Disable upnp").env("sydarD_DISABLE_UPNP"))
        .arg(arg!(--"nodnsseed" "Disable DNS seeding for peers").env("sydarD_NODNSSEED"))
        .arg(arg!(--"nogrpc" "Disable gRPC server").env("sydarD_NOGRPC"))
        .arg(
            Arg::new("ram-scale")
                .long("ram-scale")
                .env("sydarD_RAM_SCALE")
                .require_equals(true)
...
/*
  ...
  -a, --addpeer=                            Add a peer to connect with at startup
      ...
      --rpccert=                            File containing the certificate file (default:
                                            /Users/aspect/Library/Application Support/sydard/rpc.cert)
      --rpckey=                             File containing the certificate key (default:
                                            /Users/aspect/Library/Application Support/sydard/rpc.key)
