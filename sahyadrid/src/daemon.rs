...
use sydar_consensus_notify::{root::ConsensusNotificationRoot, service::NotifyService};
use sydar_core::{core::Core, debug, info};
use sydar_core::{sydard_env::version, task::tick::TickService};
use sydar_database::{
    prelude::{CachePolicy, DbWriter, DirectDbWriter, RocksDbPreset},
    registry::DatabaseStorePrefixes,
};
use sydar_grpc_server::service::GrpcService;
use sydar_notify::{address::tracker::Tracker, subscription::context::SubscriptionContext};
use sydar_p2p_lib::Hub;
use sydar_p2p_mining::rule_engine::MiningRuleEngine;
use sydar_rpc_service::service::RpcCoreService;
...
use sydar_addressmanager::AddressManager;
use sydar_consensus::{
...
use sydar_p2p_flows::{flow_context::FlowContext, service::P2pService};

use sydar_perf_monitor::{builder::Builder as PerfMonitorBuilder, counters::CountersSnapshot};
use sydar_utxoindex::{UtxoIndex, api::UtxoIndexProxy};
use sydar_wrpc_server::service::{
...
/// for the sydard process.
pub const DESIRED_DAEMON_SOFT_FD_LIMIT: u64 = 8 * 1024;
/// Minimum acceptable soft FD limit for the sydard
/// process. (sydar will operate with the minimal
...
pub fn get_app_dir() -> PathBuf {
    #[cfg(target_os = "windows")]
    return get_home_dir().join("sydar");
    #[cfg(not(target_os = "windows"))]
    return get_home_dir().join(".sydar");
}
...
fn request_database_deletion_approval(approve: bool) -> bool {
    let msg =
        "Node database is from a different sydard *DB* version and needs to be fully deleted, do you confirm the delete? (y/n)";
    get_user_approval_or_exit(msg, approve);
    info!("Deleting databases from previous sydard version");
...
/// the application folder that contains sydard logs and the database.
    ...
    pub fn from_args(args: &Args) -> Self {
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "semaphore-trace")] {
                sydar_core::log::init_logger(log_dir.as_deref(), &format!("{},{}=debug", args.log_level, sydar_utils::sync::semaphore_module_path()));
            } else {
                sydar_core::log::init_logger(log_dir.as_deref(), &args.log_level);
            }
        };
...
do you confirm? (answer y/n or pass --yes to the sydard command line to confirm all interactive questions)";
    ...
    let mut meta_db = sydar_database::prelude::ConnBuilder::default()
    ...
    if !is_db_reset_needed && (args.testnet || args.devnet || args.simnet) {
        ...
        match active_consensus_dir_name {
            Some(dir_name) => {
                let consensus_db = sydar_database::prelude::ConnBuilder::default()
    ...
    // Reset Condition: Need to reset if we're upgrading from sydard DB version
...
Do you confirm? (y/n)";
    ...
    if is_db_reset_needed && !args.reset_db {
        ...
        // Reopen the DB
        meta_db = sydar_database::prelude::ConnBuilder::default()
    ...
    let perf_monitor = if args.perf_metrics {
        let cb = move |counters: CountersSnapshot| {
            debug!("[{}] {}", sydar_perf_monitor::SERVICE_NAME, counters.to_process_metrics_display());
            debug!("[{}] {}", sydar_perf_monitor::SERVICE_NAME, counters.to_io_metrics_display());
            #[cfg(feature = "heap")]
            debug!("[{}] heap stats: {:?}", sydar_perf_monitor::SERVICE_NAME, dhat::HeapStats::get());
        };
    ...
    let index_service: Option<Arc<IndexService>> = if args.utxoindex {
        ...
        let utxoindex_db = sydar_database::prelude::ConnBuilder::default()
