extern crate sydar_consensus;
extern crate sydar_core;
extern crate sydar_hashes;

use std::sync::Arc;

use sydar_alloc::init_allocator_with_default_settings;
use sydar_core::{info, signals::Signals};
...
pub fn main() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("sydard-heap.json").build();
    ...
    match fd_budget::try_set_fd_limit(DESIRED_DAEMON_SOFT_FD_LIMIT) {
        Ok(limit) => {
            if limit < MINIMUM_DAEMON_SOFT_FD_LIMIT {
                ...
                println!("The sydard node requires a setting of at least {DESIRED_DAEMON_SOFT_FD_LIMIT} to operate properly.");
