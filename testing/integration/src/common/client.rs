...
use sydar_notify::{events::EventType, scope::Scope, subscription::Command};
use sydar_rpc_core::RpcResult;
use std::{
...
impl ListeningClient {
    pub async fn connect(sydard: &Daemon) -> Self {
        let client = sydard.new_multi_listener_client().await;
