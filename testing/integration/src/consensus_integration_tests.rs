...
use sydar_alloc::init_allocator_with_default_settings;
use sydar_consensus::config::{Config, ConfigBuilder};
use sydar_consensus::consensus::factory::Factory as ConsensusFactory;
use sydar_consensus::consensus::test_consensus::{TestConsensus, TestConsensusFactory};
use sydar_consensus::model::stores::block_transactions::{
    BlockTransactionsStore, BlockTransactionsStoreReader, DbBlockTransactionsStore,
};
use sydar_consensus::model::stores::headers::HeaderStoreReader;
use sydar_consensus::model::stores::reachability::DbReachabilityStore;
use sydar_consensus::model::stores::relations::DbRelationsStore;
use sydar_consensus::model::stores::sydar_consensus::{KType as sydarConsensusKType, sydarConsensusStoreReader};
use sydar_consensus::model::stores::selected_chain::SelectedChainStoreReader;
use sydar_consensus::params::{DEVNET_PARAMS, ForkActivation, MAINNET_PARAMS, OverrideParams};
use sydar_consensus::pipeline::ProcessingCounters;
use sydar_consensus::pipeline::monitor::ConsensusMonitor;
use sydar_consensus::processes::reachability::tests::{DagBlock, DagBuilder, StoreValidationExtensions};
use sydar_consensus::processes::window::{WindowManager, WindowType};
use sydar_consensus_core::api::args::TransactionValidationArgs;
use sydar_consensus_core::api::{BlockValidationFutures, ConsensusApi};
use sydar_consensus_core::block::Block;
use sydar_consensus_core::blockhash::{self, new_unique};
use sydar_consensus_core::blockstatus::BlockStatus;
use sydar_consensus_core::coinbase::MinerData;
use sydar_consensus_core::constants::{BLOCK_VERSION, KANA_PER_sydar, TRANSIENT_BYTE_TO_MASS_FACTOR};
use sydar_consensus_core::errors::block::{BlockProcessResult, RuleError};
use sydar_consensus_core::hashing;
use sydar_consensus_core::header::Header;
use sydar_consensus_core::mining_rules::MiningRules;
use sydar_consensus_core::subnets::SubnetworkId;
...
use sydar_consensus_notify::root::ConsensusNotificationRoot;
use sydar_consensus_notify::service::NotifyService;
use sydar_consensusmanager::ConsensusManager;
use sydar_core::task::tick::TickService;
use sydar_core::time::unix_now;
use sydar_database::utils::get_sydar_tempdir;
use sydar_hashes::Hash;
use sydar_rpc_core::RpcHeader;
...
// use sydar_consensus_core::hashing::sighash::calc_Dilithium3_signature_hash;
use sydar_consensus_core::muhash::MuHashExtensions;
use sydar_core::core::Core;
use sydar_core::signals::Shutdown;
use sydar_core::task::runtime::AsyncRuntime;
use sydar_core::{assert_match, info};
use sydar_database::create_temp_db;
use sydar_database::prelude::{CachePolicy, ConnBuilder};
use sydar_index_processor::service::IndexService;
use sydar_math::Uint256;
use sydar_muhash::MuHash;
use sydar_notify::subscription::context::SubscriptionContext;
use sydar_txscript::caches::TxScriptCacheCounters;
use sydar_txscript::opcodes::codes::OpTrue;
// use sydar_txscript::script_builder::ScriptBuilderResult;
use sydar_utxoindex::UtxoIndex;
use sydar_utxoindex::api::{UtxoIndexApi, UtxoIndexProxy};
...
#[derive(Serialize, Deserialize, Debug)]
struct sydarConsensusTestDag {
    #[serde(rename = "K")]
    k: sydarConsensusKType,

    #[serde(rename = "GenesisID")]
    genesis_id: String,

    #[serde(rename = "Blocks")]
    blocks: Vec<sydarConsensusTestBlock>,
}

#[derive(Serialize, Deserialize, Debug)]
struct sydarConsensusTestBlock {
    #[serde(rename = "ID")]
    id: String,
...
async fn sydar_consensus_test() {
    ...
    for path_str in path_strings.iter() {
        ...
        let test: sydarConsensusTestDag = serde_json::from_reader(reader).unwrap();

        let config = ConfigBuilder::new(MAINNET_PARAMS)
            .skip_proof_of_work()
            .edit_consensus_params(|p| {
                p.genesis.hash = string_to_hash(&test.genesis_id);
                p.sydar_consensus_k = test.k;
        ...
        let sydar_consensus_store =
            consensus.sydar_consensus_store().clone_with_new_cache(CachePolicy::Count(10_000), CachePolicy::Count(10_000));

        // Assert sydar_CONSENSUS output data
        for block in test.blocks {
            info!("Asserting block {}", block.id);
            let block_id = string_to_hash(&block.id);
            let output_sydar_consensus_data = sydar_consensus_store.get_data(block_id).unwrap();

            assert_eq!(
                output_sydar_consensus_data.selected_parent,
            ...
            assert_eq!(
                output_sydar_consensus_data.mergeset_reds.to_vec(),
            ...
            assert_eq!(
                output_sydar_consensus_data.mergeset_blues.to_vec(),
            ...
            assert_eq!(output_sydar_consensus_data.blue_score, block.score, "blue score assertion failed for {}", block.id,);
        }
...
async fn block_window_test() {
    ...
    for test_block in test_blocks {
        ...
        let window = consensus
            .window_manager()
            .block_window(&consensus.sydar_consensus_store().get_data(block_id).unwrap(), WindowType::VaryingWindow(10))
            .unwrap()
            .blocks
            .clone();
...
async fn bounded_merge_depth_test() {
    ...
    let config = ConfigBuilder::new(DEVNET_PARAMS)
        .skip_proof_of_work()
        .edit_consensus_params(|p| {
            p.sydar_consensus_k = 5;
            p.merge_depth = 7;
        })
        .build();

    assert!((config.sydar_consensus_k() as u64) < config.merge_depth(), "K must be smaller than merge depth for this test to run");
    ...
    for i in 0..config.sydar_consensus_k() {
...
async fn difficulty_test() {
    ...
    async fn add_block(consensus: &TestConsensus, block_time: Option<u64>, parents: Vec<Hash>) -> Header {
        let selected_parent = consensus.sydar_consensus_manager().find_selected_parent(parents.iter().copied());
    ...
    fn past_median_time(consensus: &TestConsensus, parents: &[Hash]) -> u64 {
        let sydar_consensus_data = consensus.sydar_consensus_manager().sydar_consensus(parents);
        consensus.window_manager().calc_past_median_time(&sydar_consensus_data).unwrap().0
    }
    ...
    fn full_window_bits(consensus: &TestConsensus, hash: Hash) -> u32 {
        ...
        let sydar_consensus_data = &consensus.sydar_consensus_store().get_data(hash).unwrap();
        let window = consensus.window_manager().block_window(sydar_consensus_data, WindowType::VaryingWindow(window_size)).unwrap();
        assert_eq!(window.blocks.len(), window_size);
        let daa_window = consensus.window_manager().calc_daa_window(sydar_consensus_data, window);
        consensus.window_manager().calculate_difficulty_bits(sydar_consensus_data, &daa_window)
    }
...
async fn selected_chain_test() {
    ...
    assert_eq!(consensus.selected_chain_store.read().get_by_index(0).unwrap(), config.genesis.hash);
    assert_eq!(consensus.selected_chain_store.read().get_by_index(1).unwrap(), 22.into()); // We expect 23's selected parent to be 22 because of sydar_CONSENSUS tie-breaking rules.
...
async fn staging_consensus_test() {
    ...
    let db_tempdir = get_sydar_tempdir();
    let db_path = db_tempdir.path().to_owned();
    let consensus_db_dir = db_path.join("consensus");
    let meta_db_dir = db_path.join("meta");

    let meta_db = sydar_database::prelude::ConnBuilder::default().with_db_path(meta_db_dir).with_files_limit(5).build().unwrap();
    ...
    let consensus_factory = Arc::new(ConsensusFactory::new(
        ...
        sydar_database::prelude::RocksDbPreset::Default,
        None,
        None,
    ));
...
async fn kip10_test() {
    ...
    use sydar_txscript::opcodes::codes::{Op0, OpTxInputSpk};
    use sydar_txscript::pay_to_script_hash_script;
    use sydar_txscript::script_builder::ScriptBuilder;
    ...
    let initial_utxo_collection = [(
        ...
        UtxoEntry { amount: KANA_PER_sydar, script_public_key: spk.clone(), block_daa_score: 0, is_coinbase: false },
    )];
...
//     };
//     use sydar_txscript::{opcodes::codes::*, script_builder::ScriptBuilder};
//
//     init_allocator_with_default_settings();
//
...
//     .unwrap();
//
//     let script_pub_key = sydar_txscript::pay_to_script_hash_script(&redeem_script);
//
//     // Set up initial UTXO with P2SH script
//     let initial_utxo_collection = [(
//         TransactionOutpoint::new(1.into(), 0),
//         UtxoEntry { amount: KANA_PER_sydar, script_public_key: script_pub_key.clone(), block_daa_score: 0, is_coinbase: false },
//     )];
//
//     let config = ConfigBuilder::new(DEVNET_PARAMS)
//         .skip_proof_of_work()
...
//     use sydar_consensus_core::hashing::sighash::SigHashReusedValuesUnsync;
//     use sydar_consensus_core::hashing::sighash_type::{
//         SIG_HASH_ALL, SIG_HASH_ANY_ONE_CAN_PAY, SIG_HASH_NONE, SIG_HASH_SINGLE, SigHashType,
//     };
//     use sydar_consensus_core::subnets::SUBNETWORK_ID_NATIVE;
//     use sydar_txscript::opcodes::codes::*;
//     use sydar_txscript::pay_to_script_hash_script;
//     use sydar_txscript::script_builder::ScriptBuilder;
...
//             UtxoEntry { amount: KANA_PER_sydar / 10, script_public_key: p2sh_script.clone(), block_daa_score: 0, is_coinbase: false },
//         ));
//     }
//     for i in 0..3 {
//         initial_utxo_collection.push((
//             TransactionOutpoint::new((i + 7).into(), 0),
//             UtxoEntry { amount: KANA_PER_sydar / 20, script_public_key: op_true_spk.clone(), block_daa_score: 0, is_coinbase: false },
//         ));
//     }
//
//     let config = ConfigBuilder::new(DEVNET_PARAMS)
