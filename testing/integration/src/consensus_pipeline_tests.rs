...
use sydar_consensus_core::{api::ConsensusApi, blockhash};
...
async fn test_concurrent_pipeline() {
    ...
    for (hash, parents) in blocks {
        ...
        let b: sydar_consensus_core::block::Block = consensus.build_header_only_block_with_parents(hash, parents).to_immutable();
        let results = join!(
