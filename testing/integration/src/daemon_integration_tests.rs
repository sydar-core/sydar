...
use sydar_consensus::params::SIMNET_PARAMS;
use sydar_consensus_core::header::Header;
use sydar_consensusmanager::ConsensusManager;
use sydar_core::{task::runtime::AsyncRuntime, trace};
use sydar_grpc_client::GrpcClient;
use sydar_notify::scope::{BlockAddedScope, UtxosChangedScope, VirtualDaaScoreChangedScope};
use sydar_rpc_core::{Notification, RpcTransactionId, api::rpc::RpcApi};
...
async fn daemon_sanity_test() {
    ...
    // let total_fd_limit =  sydar_utils::fd_budget::get_limit() / 2 - 128;
    let total_fd_limit = 10;
    let mut sydard1 = Daemon::new_random(total_fd_limit);
    let rpc_client1 = sydard1.start().await;
    assert!(rpc_client1.handle_message_id() && rpc_client1.handle_stop_notify(), "the client failed to collect server features");

    let mut sydard2 = Daemon::new_random(total_fd_limit);
    let rpc_client2 = sydard2.start().await;
...
async fn daemon_mining_test() {
    ...
    let mut sydard1 = Daemon::new_random_with_args(args.clone(), total_fd_limit);
    let mut sydard2 = Daemon::new_random_with_args(args, total_fd_limit);
    let rpc_client1 = sydard1.start().await;
    let rpc_client2 = sydard2.start().await;

    rpc_client2.add_peer(format!("127.0.0.1:{}", sydard1.p2p_port).try_into().unwrap(), true).await.unwrap();
    ...
    for i in 0..10 {
        let template = rpc_client1
            .get_block_template(Address::new(sydard1.network.into(), sydar_addresses::Version::PubKey, &[0; 32]), vec![])
            .await
            .unwrap();
    ...
    let dag_info = rpc_client2.get_sydar_dag_info().await.unwrap();
    ...
    let vc = rpc_client2
        .get_virtual_chain_from_block(
            sydar_consensus::params::SIMNET_GENESIS.hash, //
            true,
            None,
        )
        .await
...
/// `cargo test --release --package sydar-testing-integration --lib -- daemon_integration_tests::daemon_utxos_propagation_test`
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
#[ignore]
async fn daemon_utxos_propagation_test() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("sydar-testing-integration-heap.json").build();

    sydar_core::log::try_init_logger(
        "INFO,sydar_testing_integration=trace,sydar_notify=debug,sydar_rpc_core=debug,sydar_grpc_client=debug",
    );
    ...
    let miner_kp = sydar_dilithium::generate_keypair().expect("generate miner keypair");
    let miner_address = Address::new(
        sydard1.network.into(),
    ...
    let user_kp = sydar_dilithium::generate_keypair().expect("generate user keypair");
    ...
    let blank_address = Address::new(sydard1.network.into(), sydar_addresses::Version::PubKeyDilithium, &[0; 20]);
    ...
    // Check that acceptance data contains the expected coinbase tx ids
    let vc = rpc_client2.get_virtual_chain_from_block(sydar_consensus::params::SIMNET_GENESIS.hash, true, None).await.unwrap();
    assert_eq!(vc.removed_chain_block_hashes.len(), 0);
    ...
    let mut clients = vec![ListeningClient::connect(&sydard2).await, ListeningClient::connect(&sydard1).await];
...
async fn daemon_cleaning_test() {
    init_allocator_with_default_settings();
    sydar_core::log::try_init_logger(
        "info,sydar_grpc_core=trace,sydar_grpc_server=trace,sydar_grpc_client=trace,sydar_core=trace",
    );
    ...
    {
        let total_fd_limit = 10;
        let mut sydard1 = Daemon::new_random_with_args(args, total_fd_limit);
        let dyn_consensus_manager = sydard1.core.find(ConsensusManager::IDENT).unwrap();
        let dyn_async_runtime = sydard1.core.find(AsyncRuntime::IDENT).unwrap();
        ...
        core = Arc::downgrade(&sydard1.core);
