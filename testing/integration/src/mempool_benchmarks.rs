...
use sydar_consensus_core::{constants::KANA_PER_sydar, network::NetworkType, tx::Transaction};
use sydar_core::{debug, info};
use sydar_notify::{
    listener::ListenerId,
    scope::{NewBlockTemplateScope, Scope},
};
use sydar_rpc_core::{Notification, RpcError, api::rpc::RpcApi};
...
/// `cargo test --release --package sydar-testing-integration --lib --features devnet-prealloc -- mempool_benchmarks::bench_bbt_latency --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
async fn bench_bbt_latency() {
    sydar_core::log::try_init_logger("info,sydar_core::time=debug,sydar_mining::monitor=debug");
    ...
    */
    ...
    let args = Args {
        ...
        prealloc_amount: 500 * KANA_PER_sydar,
    ...
    let pay_kp = sydar_dilithium::generate_keypair().expect("generate pay keypair");
    ...
    let submit_block_pool_tasks = submit_block_pool.start(|c, block| async move {
        let _sw = sydar_core::time::Stopwatch::<500>::with_threshold("sb");
        let response = c.submit_block(block, false).await.unwrap();
        assert_eq!(response.report, sydar_rpc_core::SubmitBlockReport::Success);
        false
    });
    ...
    let submit_tx_pool_tasks = submit_tx_pool.start(|c, (i, tx)| async move {
        match c.submit_transaction(tx.as_ref().into(), false).await {
            ...
            Err(RpcError::General(msg)) if msg.contains("orphan") => {
                sydar_core::warn!("\n\n\n{msg}\n\n");
                sydar_core::warn!("Submitted {} transactions, exiting tx submit loop", i);
                return true;
            }
            Err(e) => panic!("{e}"),
        }
        ...
        while let Ok(notification) = notification_rx.recv().await {
            match notification {
                Notification::NewBlockTemplate(_) => {
                    ...
                    // let _sw = sydar_core::time::Stopwatch::<500>::with_threshold("bbt");
                    *current_template.lock() = cc.get_block_template(pac.clone(), vec![]).await.unwrap();
                }
                _ => panic!(),
            }
            if !exec.load(Ordering::Relaxed) {
                sydar_core::warn!("Test is over, stopping miner receiver loop");
                break;
            }
        }
        sydar_core::warn!("Miner receiver loop task exited");
    });
    ...
    let miner_loop_task = tokio::spawn(async move {
        for i in 0..BLOCK_COUNT {
            ...
            if !exec.load(Ordering::Relaxed) {
                sydar_core::warn!("Test is over, stopping miner loop");
                break;
            }
        }
        ...
        sydar_core::warn!("Miner loop task exited");
    });
    ...
    let tx_sender_task = tokio::spawn(async move {
        ...
        sydar_core::warn!("Tx sender task, waiting for mempool to drain..");
        loop {
            if !exec.load(Ordering::Relaxed) {
                break;
            }
        ...
        exec.store(false, Ordering::Relaxed);
        sydar_core::warn!("Tx sender task exited");
    });
...
/// Run this benchmark with the following command line:
/// `cargo test --release --package sydar-testing-integration --lib --features devnet-prealloc -- mempool_benchmarks::bench_bbt_latency_2 --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
