...
use sydar_addresses::{Address, Prefix, Version};
use sydar_consensus::params::SIMNET_GENESIS;
use sydar_consensus_core::{constants::MAX_KANA, header::Header, subnets::SubnetworkId, tx::Transaction};
use sydar_core::{assert_match, info};
use sydar_grpc_core::ops::sydardPayloadOps;
...
use sydar_rpc_core::{Notification, api::rpc::RpcApi, model::*};
use sydar_utils::{fd_budget, networking::ContextualNetAddress};
use sydard_lib::args::Args;
...
/// `cargo test --release --package sydar-testing-integration --lib -- rpc_tests::sanity_test`
#[tokio::test]
async fn sanity_test() {
    sydar_core::log::try_init_logger("info");
    // As we log the panic, we want to set it up after the logger
    sydar_core::panic::configure_panic();
    ...
    for op in sydardPayloadOps::iter() {
        ...
        let task: JoinHandle<()> = match op {
            ...
            sydardPayloadOps::GetBlockTemplate => {
                tst!(op, "see SubmitBlock")
            }

            sydardPayloadOps::GetCurrentBlockColor => {
                tst!(op, "see SubmitBlock")
            }

            sydardPayloadOps::GetCurrentNetwork => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            sydardPayloadOps::GetInfo => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client.get_info_call(None, GetInfoRequest {}).await.unwrap();
                    assert_eq!(response.server_version, sydar_core::sydard_env::version().to_string());
            ...
            sydardPayloadOps::GetMempoolEntries => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
            ...
            sydardPayloadOps::GetConnectedPeerInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            sydardPayloadOps::Unban => {
                tst!(op, "see Ban")
            }

            sydardPayloadOps::SubmitTransaction => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            sydardPayloadOps::SubmitTransactionReplacement => {
                let rpc_client = client.clone();
                tst!(op, {
                    // Build an erroneous transaction...
                    let transaction = Transaction::new(0, vec![], vec![], 0, SubnetworkId::default(), 0, vec![]);
            ...
            sydardPayloadOps::GetSubnetwork => {
                let rpc_client = client.clone();
                tst!(op, {
                    let result =
            ...
            sydardPayloadOps::GetVirtualChainFromBlock => {
                tst!(op, "see SubmitBlock")
            }

            sydardPayloadOps::GetBlockCount => {
                tst!(op, "see SubmitBlock")
            }

            sydardPayloadOps::GetsydarDagInfo => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client.get_sydar_dag_info_call(None, GetsydarDagInfoRequest {}).await.unwrap();
                    assert_eq!(response.network, network_id);
                })
            }

            sydardPayloadOps::ResolveFinalityConflict => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response_result = rpc_client
            ...
            sydardPayloadOps::GetUtxosByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            sydardPayloadOps::GetBalanceByAddress => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
            ...
            sydardPayloadOps::GetBalancesByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            sydardPayloadOps::GetSinkBlueScore => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client.get_sink_blue_score_call(None, GetSinkBlueScoreRequest {}).await.unwrap();
            ...
            sydardPayloadOps::EstimateNetworkHashesPerSecond => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response_result = rpc_client
            ...
            sydardPayloadOps::GetMempoolEntriesByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
                    let addresses = vec![Address::new(Prefix::Simnet, Version::PubKey, &[0u8; 32])];
                    let response = rpc_client
            ...
            sydardPayloadOps::GetCoinSupply => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            sydardPayloadOps::GetConnections => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            sydardPayloadOps::GetSystemInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            sydardPayloadOps::GetServerInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            sydardPayloadOps::GetSyncStatus => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            sydardPayloadOps::GetDaaScoreTimestampEstimate => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            sydardPayloadOps::GetFeeEstimate => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            sydardPayloadOps::GetFeeEstimateExperimental => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            sydardPayloadOps::GetUtxoReturnAddress => {
                let rpc_client = client.clone();
                tst!(op, {
                    ...
                    assert!(results.is_err_and(|err| {
                        match err {
                            sydar_rpc_core::RpcError::General(msg) => {
            ...
            sydardPayloadOps::GetVirtualChainFromBlockV2 => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
            ...
            sydardPayloadOps::NotifyBlockAdded => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            sydardPayloadOps::NotifyNewBlockTemplate => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            sydardPayloadOps::NotifyFinalityConflict => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            sydardPayloadOps::NotifyUtxosChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            sydardPayloadOps::NotifySinkBlueScoreChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            sydardPayloadOps::NotifyPruningPointUtxoSetOverride => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            sydardPayloadOps::NotifyVirtualDaaScoreChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            sydardPayloadOps::NotifyVirtualChainChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
                    rpc_client
            ...
            sydardPayloadOps::StopNotifyingPruningPointUtxoSetOverride => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            sydardPayloadOps::StopNotifyingUtxosChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            sydardPayloadOps::SubmitAccountTransaction => {
                tokio::spawn(async move {
