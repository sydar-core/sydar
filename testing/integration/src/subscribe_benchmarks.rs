...
use sydar_core::{info, task::tick::TickService, trace};
use sydar_math::Uint256;
use sydar_notify::scope::VirtualDaaScoreChangedScope;
use sydar_rpc_core::api::rpc::RpcApi;
...
/// `cargo test --package sydar-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::utxos_changed_subscriptions_sanity_check --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
async fn utxos_changed_subscriptions_sanity_check() {
    init_allocator_with_default_settings();
    sydar_core::log::try_init_logger(
        "INFO, sydar_core::time=debug, sydar_rpc_core=debug, sydar_grpc_client=debug, sydar_notify=info, sydar_notify::address::tracker=debug, sydar_notify::listener=debug, sydar_notify::subscription::single=debug, sydar_mining::monitor=debug, sydar_testing_integration::subscribe_benchmarks=trace",
    );
    ...
    // Fold-up
    //
    sydar_core::info!("Signal the daemon to shutdown");
    client.shutdown().await.unwrap();
    sydar_core::warn!("Disconnect the main client");
    client.disconnect().await.unwrap();
    drop(client);

    sydar_core::warn!("Waiting for the daemon to exit...");
    daemon_process.wait().await.expect("failed to wait for the daemon process");
}

/// `cargo test --package sydar-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_daemon --exact --nocapture --ignored -- --rpc=16610 --p2p=16611 --private-key=a2760251adb5b6e8d4514d23397f1631893e168c33f92ff8a7a24f397d355d62 --max-tracked-addresses=1000000 --utxoindex`
///
...
async fn bench_utxos_changed_subscriptions_daemon() {
    init_allocator_with_default_settings();
    sydar_core::log::try_init_logger(
        "INFO, sydar_core::core=trace, sydar_core::time=debug, sydar_rpc_core=debug, sydar_grpc_client=debug, sydar_notify=info, sydar_notify::address::tracker=debug, sydar_notify::listener=debug, sydar_notify::subscription::single=debug, sydar_mining::monitor=debug, sydar_testing_integration::subscribe_benchmarks=trace",
    );
    // As we log the panic, we want to set it up after the logger
    sydar_core::panic::configure_panic();
...
async fn utxos_changed_subscriptions_client(address_cycle_seconds: u64, address_max_cycles: usize) {
    ...
    let prealloc_kp = sydar_dilithium::generate_keypair_from_seed(&seed);
...
/// `cargo test --package sydar-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_a --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package sydar-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_b --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package sydar-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_c --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package sydar-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_d --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package sydar-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_e --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
