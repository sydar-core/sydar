...
use sydar_core::{trace, warn};
use sydar_dilithium;
use sydar_utils::{fd_budget, triggers::SingleTrigger};
...
impl DaemonArgs {
    ...
    pub fn to_command_args(&self, test_name: &str) -> Vec<String> {
        let mut args = vec![
            "test".to_owned(),
            "--package".to_owned(),
            "sydar-testing-integration".to_owned(),
            "--lib".to_owned(),
            "--features".to_owned(),
    ...
    pub fn prealloc_address(&self) -> Address {
        ...
        let kp = sydar_dilithium::generate_keypair_from_seed(&seed);
        Address::new(
