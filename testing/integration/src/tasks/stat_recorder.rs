...
impl Task for StatRecorderTask {
    fn start(&self, stop_signal: SingleTrigger) -> Vec<JoinHandle<()>> {
        ...
        let task = tokio::spawn(async move {
            sydar_core::warn!("Stat recorder task starting...");
            std::fs::create_dir_all(PathBuf::from(&folder)).unwrap();
            {
                let file_path = PathBuf::from(&folder).join(file_name).into_os_string();
                sydar_core::warn!("Recording memory metrics into file {}", file_path.to_str().unwrap());
                ...
                loop {
                    tokio::select! {
                        ...
                        _ = stop_signal.listener.clone() => {
                            sydar_core::trace!("Leaving stat recorder loop");
                            break;
                        }
            ...
            sydar_core::warn!("Stat recorder task exited");
        });
        vec![task]
    }
}
