...
After starting the wallet shell (native or WASM) and starting a local sydar sydard node (with `--testnet` and `--utxoindex`), you should perform the following actions:
```
network testnet
server localhost
connect
