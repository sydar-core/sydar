...
//! (Partial Signed sydar Transaction Bundles).
//!
...
use futures::stream;
use sydar_bip32::{DerivationPath, KeyFingerprint, PrivateKey};
use sydar_consensus_client::UtxoEntry as ClientUTXO;
use sydar_consensus_core::hashing::sighash::{SigHashReusedValuesUnsync, calc_signature_hash};
use sydar_consensus_core::tx::VerifiableTransaction;
use sydar_consensus_core::tx::{TransactionInput, UtxoEntry};
use sydar_dilithium::{generate_keypair_from_seed, sign_bytes};
use sydar_txscript::extract_script_pub_key_address;
use sydar_txscript::opcodes::codes::OpData65;
use sydar_txscript::script_builder::ScriptBuilder;
use sydar_wallet_core::tx::{Generator, GeneratorSettings, PaymentDestination, PendingTransaction};
pub use sydar_wallet_pskt::bundle::Bundle;
use sydar_wallet_pskt::bundle::{script_sig_to_address, unlock_utxo_outputs_as_batch_transaction_pskb};
use sydar_wallet_pskt::prelude::KeySource;
use sydar_wallet_pskt::prelude::lock_script_sig_templating_bytes;
use sydar_wallet_pskt::prelude::{Finalizer, Inner, SignInputOk, Signature, Signer};
pub use sydar_wallet_pskt::pskt::{Creator, PSKT};
use std::iter;
                ...
                .pass_signature_sync(|tx, sighash| -> Result<Vec<SignInputOk>, String> {
                    tx.tx
                        ...
                        .map(|(input_idx, _input)| {
                            ...
                            let pub_key = sydar_bip32::DilithiumPkHash(pk_arr);
...
pub fn pskt_to_pending_transaction(
    ...
    let output: &Vec<sydar_consensus_core::tx::TransactionOutput> = &signed_tx.outputs;
    if output.is_empty() {
    ...
    let pending_tx = PendingTransaction::try_new(
        ...
        0,
        0,
        sydar_wallet_core::tx::DataKind::Final,
    )?;
