...
pub const BIP32_WATCH_ACCOUNT_KIND: &str = "sydar-bip32-watch-standard";
...
impl Factory for Ctor {
    ...
    fn description(&self) -> String {
        "sydar Core bip32-watch Account".to_string()
    }
...
impl Bip32Watch {
    pub async fn try_new(wallet: &Arc<Wallet>, name: Option<String>, xpub_keys: ExtendedPublicKeys, ecdsa: bool) -> Result<Self> {
        ...
        let (id, storage_key) =
            make_account_hashes(from_bip32_watch(&sydar_wallet_keys::prelude::PublicKey { bytes: public_key.0.to_vec() }));
