...
use sydar_wallet_keys::prelude::PublicKey;

pub const KEYPAIR_ACCOUNT_KIND: &str = "sydar-keypair-standard";
...
pub struct Payload {
    pub public_key: sydar_wallet_keys::prelude::PublicKey,
    pub ecdsa: bool,
}

impl Payload {
    pub fn new(public_key: sydar_wallet_keys::prelude::PublicKey, ecdsa: bool) -> Self {
...
impl BorshDeserialize for Payload {
    fn deserialize_reader<R: std::io::Read>(reader: &mut R) -> IoResult<Self> {
        ...
        let public_key = sydar_wallet_keys::prelude::PublicKey::from(public_key_bytes);
...
impl Keypair {
    pub async fn try_new(
        wallet: &Arc<Wallet>,
        name: Option<String>,
        public_key: sydar_wallet_keys::prelude::PublicKey,
        prv_key_data_id: PrvKeyDataId,
        ecdsa: bool,
    ) -> Result<Self> {
...
impl Account for Keypair {
    ...
    fn create_address_private_keys<'l>(
        ...
        addresses: &[&'l Address],
    ) -> Result<Vec<(&'l Address, sydar_bip32::DilithiumSeed)>> {
        let private_key =
