//!
//! API trait for interfacing with the sydar wallet subsystem.
//!
...
pub trait WalletApi: Send + Sync + AnySync {
    ...
    /// If the `filename` is `None`, the wallet opens the default wallet named `sydar`.
    ///
    ...
    /// applications such as KDX and sydarnet.io web wallet using a deprecated derivation path
