...
use sydar_bip32::{ExtendedPrivateKey, Language, Mnemonic, Prefix, SecretKey};
...
// ---
// {"type":"sydar-wallet","encryption":"default","version":1,"generator":"pwa","wallet":{"mnemonic":"hex"}}
...
fn legacy_v0_keydata_location() -> Result<(PathBuf, Options)> {
    let filename = if runtime::is_windows() {
        ...
        fs::resolve_path(&format!("{appdata}/sydar/sydar.kpk"))?
    } else if runtime::is_macos() {
        fs::resolve_path("~/Library/Application Support/sydar/sydar.kpk")?
    } else {
        fs::resolve_path("~/.sydar/sydar.kpk")?
    };

    let options = workflow_store::fs::Options::with_local_storage_key("sydar-wallet");
