...
use sydar_bip32::Error as BIP32Error;
use sydar_consensus_core::sign::Error as CoreSignError;
use sydar_rpc_core::RpcError as sydarRpcError;
use sydar_wrpc_client::error::Error as sydarWorkflowRpcError;
...
pub enum Error {
    ...
    #[error(transparent)]
    WalletKeys(#[from] sydar_wallet_keys::error::Error),

    #[error("please select an account")]
    AccountSelection,

    #[error("{0}")]
    sydarRpcClientResult(#[from] sydarRpcError),

    #[error("wRPC -> {0}")]
    RpcError(#[from] RpcError),

    #[error("Wallet wRPC -> {0}")]
    sydarWorkflowRpcError(#[from] sydarWorkflowRpcError),
    ...
    #[error("{0}")]
    NetworkType(#[from] sydar_consensus_core::network::NetworkTypeError),

    #[error("{0}")]
    NetworkId(#[from] sydar_consensus_core::network::NetworkIdError),
    ...
    #[error(transparent)]
    Address(#[from] sydar_addresses::AddressError),
    ...
    #[error(transparent)]
    ScriptBuilderError(#[from] sydar_txscript::script_builder::ScriptBuilderError),
    ...
    #[error(transparent)]
    ConsensusClient(#[from] sydar_consensus_client::error::Error),

    #[error(transparent)]
    ConsensusWasm(#[from] sydar_consensus_wasm::error::Error),
    ...
    #[error(transparent)]
    MultisigCreateError(#[from] sydar_txscript::MultisigCreateError),

    #[error(transparent)]
    TxScriptError(#[from] sydar_txscript_errors::TxScriptError),
    ...
    #[error(transparent)]
    Metrics(#[from] sydar_metrics_core::error::Error),

    #[error("Connected node is not synced")]
    NotSynced,
    #[error(transparent)]
    Pskt(#[from] sydar_wallet_pskt::error::Error),
