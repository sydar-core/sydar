...
use crate::imports::*;
use sydar_bip32::{ExtendedPrivateKey, ExtendedPublicKey, Language, Mnemonic};
use sydar_utils::hex::ToHex;
use sydar_wallet_keys::prelude::PrivateKey as SecretKey;
...
impl PrvKeyDataPayload {
    ...
    pub fn get_xprv(&self, payment_secret: Option<&Secret>) -> Result<ExtendedPrivateKey<sydar_bip32::DilithiumSeed>> {
        ...
        match &self.prv_key_variant {
            PrvKeyDataVariant::Mnemonic(mnemonic) => {
                ...
                let xkey =
                    ExtendedPrivateKey::<sydar_bip32::DilithiumSeed>::new(mnemonic.to_seed(payment_secret.unwrap_or_default()))?;
                Ok(xkey)
            }
            PrvKeyDataVariant::Bip39Seed(seed) => {
                let seed = Zeroizing::new(Vec::from_hex(seed.as_ref())?);
                let xkey = ExtendedPrivateKey::<sydar_bip32::DilithiumSeed>::new(seed)?;
                Ok(xkey)
            }
            PrvKeyDataVariant::ExtendedPrivateKey(extended_private_key) => {
                let xkey: ExtendedPrivateKey<sydar_bip32::DilithiumSeed> = extended_private_key.parse()?;
                Ok(xkey)
            }
    ...
    pub fn as_secret_key(&self) -> Result<Option<sydar_bip32::DilithiumSeed>> {
        match &self.prv_key_variant {
            PrvKeyDataVariant::SecretKey(private_key) => {
                let bytes: Vec<u8> = sydar_utils::hex::FromHex::from_hex(private_key.as_str())
                    .map_err(|e| crate::error::Error::Custom(format!("hex decode: {}", e)))?;
                let mut arr = [0u8; 32];
                arr.copy_from_slice(&bytes);
                Ok(Some(sydar_bip32::DilithiumSeed(arr)))
            }
            _ => Ok(None),
        }
    }
...
impl PrvKeyData {
    pub async fn create_xpub(
        ...
        account_index: u64,
    ) -> Result<ExtendedPublicKey<sydar_bip32::DilithiumPkHash>> {
    ...
    pub fn as_secret_key(&self, payment_secret: Option<&Secret>) -> Result<Option<sydar_bip32::DilithiumSeed>> {
        let payload = self.payload.decrypt(payment_secret)?;
        payload.as_secret_key()
    }

    pub fn try_from_secret_key(
        secret_key: sydar_wallet_keys::prelude::PrivateKey,
        payment_secret: Option<&Secret>,
        encryption_kind: EncryptionKind,
        name: Option<String>,
    ) -> Result<Self> {
