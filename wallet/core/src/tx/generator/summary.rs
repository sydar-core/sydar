...
use sydar_consensus_core::tx::TransactionId;
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if let Some(final_transaction_amount) = self.final_transaction_amount {
            ...
            write!(
                ...
                kana_to_sydar_string_with_suffix(final_transaction_amount, &self.network_id),
                kana_to_sydar_string_with_suffix(self.aggregate_fees, &self.network_id),
                kana_to_sydar_string_with_suffix(total, &self.network_id),
                self.aggregated_utxos,
                transactions
            )?;
        } else {
