...
use crate::{tx::PaymentOutputs, utils::sydar_to_kana};
use rand::prelude::*;
use sydar_addresses::Address;
use sydar_consensus_core::config::params::Params;
use sydar_consensus_core::mass::UtxoCell;
...
impl From<sydar> for Kana {
    fn from(sydar: sydar) -> Self {
        Kana(sydar_to_kana(sydar.0))
    }
}
    ...
    F: FnOnce(NetworkType) -> Address,
{
    ...
    let utxo_entries: Vec<UtxoEntryReference> = values.into_iter().map(sydar_to_kana).map(UtxoEntryReference::simulated).collect();
...
pub(crate) fn change_address(network_type: NetworkType) -> Address {
    match network_type {
        NetworkType::Mainnet => Address::try_from("sydar:qpauqsvk7yf9unexwmxsnmg547mhyga37csh0kj53q6xxgl24ydxjsgzthw5j").unwrap(),
        NetworkType::Testnet => {
            Address::try_from("sydartest:qqz22l98sf8jun72rwh5rqe2tm8lhwtdxdmynrz4ypwak427qed5juktjt7ju").unwrap()
        }
...
pub(crate) fn output_address(network_type: NetworkType) -> Address {
    match network_type {
        NetworkType::Mainnet => Address::try_from("sydar:qrd9efkvg3pg34sgp6ztwyv3r569qlc43wa5w8nfs302532dzj47knu04aftm").unwrap(),
        NetworkType::Testnet => {
            Address::try_from("sydartest:qqrewmx4gpuekvk8grenkvj2hp7xt0c35rxgq383f6gy223c4ud5s58ptm6er").unwrap()
        }
        _ => unreachable!("network type not supported"),
    }
}
...
fn test_generator_sweep_two_utxos() -> Result<()> {
    make_generator(test_network_id(), &[10.0, 10.0], &[], None, Fees::None, change_address, PaymentDestination::Change)
        ...
        .fetch(&Expected {
            is_final: true,
            input_count: 2,
            aggregate_input_value: sydar(20.0),
            output_count: 1,
...
fn test_generator_compound_200k_10kas_transactions() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(sydar(5.0)),
        [(output_address, sydar(190_000.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .validate()
...
fn test_generator_compound_100k_random_transactions() -> Result<()> {
    ...
    let outputs = [(output_address, sydar(total - 10.0))];
    generator(test_network_id(), &inputs, &[], None, Fees::sender(sydar(5.0)), outputs.as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_random_outputs() -> Result<()> {
    ...
    let outputs: Vec<_> = outputs.into_iter().map(|v| (output_address, sydar(v))).collect();

    generator(test_network_id(), &[total + 100.0], &[], None, Fees::sender(sydar(5.0)), outputs.as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_dust_1_1() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(sydar(5.0)),
        [(output_address, sydar(1.0)), (output_address, sydar(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: true,
        input_count: 4,
        aggregate_input_value: sydar(40.0),
        output_count: 3,
        priority_fees: FeesExpected::sender(sydar(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_2_outputs_2_fees_exclude() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(sydar(5.0)),
        [(output_address, sydar(10.0)), (output_address, sydar(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
...
fn test_generator_inputs_100_outputs_1_fees_exclude_success() -> Result<()> {
    // generator(test_network_id(), &[10.0; 100], &[], Fees::sender(sydar(5.0)), [(output_address, sydar(990.0))].as_slice())
    generator(test_network_id(), &[10.0; 100], &[], None, Fees::sender(sydar(0.0)), [(output_address, sydar(990.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
            input_count: 88,
            aggregate_input_value: sydar(880.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            is_final: false,
            input_count: 12,
            aggregate_input_value: sydar(120.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            ...
            output_count: 2,
            // priority_fees: FeesExpected::sender(sydar(5.0)),
            priority_fees: FeesExpected::sender(sydar(0.0)),
        })
        .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_include_success() -> Result<()> {
    generator(
        ...
        &[],
        None,
        Fees::receiver(sydar(5.0)),
        // [(output_address, sydar(100.0))].as_slice(),
        [(output_address, sydar(100.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: false,
        input_count: 88,
        aggregate_input_value: sydar(88.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        is_final: false,
        input_count: 12,
        aggregate_input_value: sydar(12.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        ...
        output_count: 1,
        priority_fees: FeesExpected::receiver(sydar(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_exclude_insufficient_funds() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(sydar(5.0)),
        [(output_address, sydar(1000.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
...
fn test_generator_inputs_1k_outputs_2_fees_exclude() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(sydar(5.0)),
        [(output_address, sydar(9_000.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .drain(
    ...
    .fetch(&Expected {
        is_final: false,
        input_count: 21,
        aggregate_input_value: sydar(210.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
...
fn test_generator_inputs_32k_outputs_2_fees_exclude() -> Result<()> {
    ...
    generator(
        ...
        &[],
        None,
        Fees::sender(sydar(10_000.0)),
        [(output_address, sydar(f * 32_747.0 - 10_001.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .accumulate(379)
...
fn test_generator_fan_out_1() -> Result<()> {
    use sydar_consensus_core::mass::calc_storage_mass;
    ...
    //     1.00000000,
    //     87.23579967,
    // ], &[], None, Fees::sender(sydar(1.0)), [
    //     (output_address, sydar(0.20000000)),
    //     (output_address, sydar(0.25000000)),
    //     (output_address, sydar(0.21000000)),
    // ].as_slice())
    //     .unwrap()
    //     .harness()
    //     // .accumulate(1)
    ...
    //         is_final: true,
    //         input_count: 2,
    //         aggregate_input_value: sydar(1.00000000 + 87.23579967),
    //         output_count: 4,
    //         priority_fees: FeesExpected::receiver(sydar(1.0)),
    //         // priority_fees: FeesExpected::None,
    //     })
    //     .finalize();
