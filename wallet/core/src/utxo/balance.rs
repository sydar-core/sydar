...
export interface IBalance {
    /**
     * Total amount of sydar (in KANA) available for 
     * spending.
     */
    mature: bigint;
    /**
     * Total amount of sydar (in KANA) that has been 
     * received and is pending confirmation.
     */
    pending: bigint;
    /**
     * Total amount of sydar (in KANA) currently 
...
impl From<(Option<&Balance>, &NetworkType, Option<usize>)> for BalanceStrings {
    fn from((balance, network_type, padding): (Option<&Balance>, &NetworkType, Option<usize>)) -> Self {
        let suffix = utils::sydar_suffix(network_type);
        if let Some(balance) = balance {
            let mut mature = utils::kana_to_sydar_string(balance.mature);
            let mut pending = if balance.pending > 0 { Some(utils::kana_to_sydar_string(balance.pending)) } else { None };
