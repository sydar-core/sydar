...
use sydar_wrpc_client::sydarRpcClient;
...
impl UtxoProcessor {
    ...
    pub fn rpc_client(&self) -> Option<Arc<sydarRpcClient>> {
        self.rpc_api().clone().downcast_arc::<sydarRpcClient>().ok()
    }
    ...
    pub async fn init_state_from_server(&self) -> Result<bool> {
        ...
        log_trace!(
            "Connected to sydard: '{server_version}' on '{server_network_id}';  SYNC: {is_synced}  DAA: {virtual_daa_score}"
        );
