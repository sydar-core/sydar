//!
//! # sydar wallet runtime implementation.
//!
//! This module contains a Rust implementation of the sydar wallet that
...
use sydar_bip32::{ExtendedKey, Language, Mnemonic, Prefix as KeyPrefix, WordCount};
use sydar_notify::{
    listener::ListenerId,
    scope::{Scope, VirtualDaaScoreChangedScope},
};
use sydar_wallet_keys::xpub::NetworkTaggedXpub;
use sydar_wrpc_client::{Resolver, sydarRpcClient, WrpcEncoding};
...
impl Wallet {
    ...
    pub fn try_with_wrpc(store: Arc<dyn Interface>, resolver: Option<Resolver>, network_id: Option<NetworkId>) -> Result<Wallet> {
        let rpc_client = Arc::new(sydarRpcClient::new_with_args(
            WrpcEncoding::Borsh,
    ...
    pub fn try_wrpc_client(&self) -> Option<Arc<sydarRpcClient>> {
        self.try_rpc_api().and_then(|api| api.clone().downcast_arc::<sydarRpcClient>().ok())
    }

    pub fn wrpc_client(&self) -> Arc<sydarRpcClient> {
        self.try_rpc_api().and_then(|api| api.clone().downcast_arc::<sydarRpcClient>().ok()).unwrap()
    }
    ...
    pub fn address_prefix(&self) -> Result<sydar_addresses::Prefix> {
    ...
    pub async fn create_account_keypair(
        ...
        let public_key = {
            let kp = sydar_dilithium::generate_keypair_from_seed(&secret_key.0);
            sydar_wallet_keys::prelude::PublicKey::from(kp.public_key().to_vec())
        };
    ...
    pub async fn create_prv_key_data(
        ...
        let prv_key_data = match kind {
            ...
            PrvKeyDataVariantKind::SecretKey => {
                let secret_key = sydar_wallet_keys::prelude::PrivateKey::try_from_slice(secret.as_ref())?;
    ...
    pub async fn import_sydarwallet_golang_single_v1<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        ...
        let prefix = file.xpublic_key.split_at(sydar_bip32::Prefix::LENGTH).0;
        let prefix = sydar_bip32::Prefix::try_from(prefix)?;
    ...
    pub async fn import_sydarwallet_golang_single_v0<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
    ...
    pub async fn import_sydarwallet_golang_multisig_v0<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
        ...
        let prefix = first_pub_key.split_at(sydar_bip32::Prefix::LENGTH).0;
        let prefix = sydar_bip32::Prefix::try_from(prefix)?;
    ...
    pub async fn import_sydarwallet_golang_multisig_v1<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
        ...
        all_pub_keys.sort_unstable_by(|left, right| {
            left.split_at(sydar_bip32::Prefix::LENGTH).1.cmp(right.split_at(sydar_bip32::Prefix::LENGTH).1)
        });
    ...
    pub async fn import_multisig_with_mnemonic(
        ...
        let mut additional_xpub_keys = additional_xpub_keys
            ...
            .collect::<Result<Vec<_>, sydar_bip32::Error>>()?;
...
mod test {
    ...
    use sydar_addresses::{Prefix, Version};
    use sydar_bip32::{ChildNumber, ExtendedPrivateKey, SecretKey};
    use sydar_consensus_core::subnets::SUBNETWORK_ID_NATIVE;
    use sydar_consensus_wasm::{sign_transaction, SignableTransaction, Transaction, TransactionInput, TransactionOutput};
    ...
    async fn wallet_test() -> Result<()> {
        ...
        let info = rpc_api.get_sydar_dag_info().await?;
        ...
        let address = Address::try_from("sydartest:qz7ulu4c25dh7fzec9zjyrmlhnkzrg4wmf89q7gzr3gfrsj3uz6xjceef60sd")?;
        ...
        let to_address = Address::try_from("sydartest:qpakxqlesqywgkq7rg4wyhjd93kmw7trkl3gpa3vd5flyt59a43yyn8vu0w8c")?;
        ...
        let derivation_path =
            gen1::WalletDerivationManager::build_derivate_path(false, 0, None, Some(sydar_bip32::AddressType::Receive))?;
