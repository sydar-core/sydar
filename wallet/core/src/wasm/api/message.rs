 ...
 * Wallet API for interfacing with sydar Wallet implementation.
 */
"#;
...
declare! {
     ...
     * with the default filename `sydar`.
     * 
     * @category Wallet API
     */
...
try_from! (args: IAccountsCreateRequest, AccountsCreateRequest, {
    ...
    let account_create_args = match kind.as_str() {
        ...
        _ => {
            return Err(Error::custom("only BIP32/sydar-keypair-standard accounts are currently supported"));
        }
    };
