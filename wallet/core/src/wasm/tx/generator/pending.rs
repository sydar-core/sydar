...
use sydar_consensus_client::{Transaction, TransactionT};
use sydar_consensus_client::{numeric, string};
    ...
    pub fn sign(&self, js_value: PrivateKeyArrayT, check_fully_signed: Option<bool>) -> Result<()> {
        if let Ok(keys) = js_value.dyn_into::<Array>() {
            let keys = keys
                ...
                .collect::<std::result::Result<Vec<_>, sydar_wallet_keys::error::Error>>()?;
