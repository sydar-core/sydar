...
use sydar_addresses::{Address, Prefix as AddressPrefix, Version as AddressVersion};
use sydar_bip32::types::{ChainCode, HmacSha512, KEY_SIZE, KeyFingerprint, PublicKeyBytes};
use sydar_bip32::{
...
impl PubkeyDerivationManager {
    ...
    pub fn derive_pubkey(&self, index: u32) -> Result<DilithiumPkHash> {
        ...
        let kp = sydar_dilithium::generate_keypair_from_seed(&child_seed);
...
impl From<&PubkeyDerivationManager> for ExtendedPublicKey<DilithiumPkHash> {
    fn from(inner: &PubkeyDerivationManager) -> ExtendedPublicKey<DilithiumPkHash> {
        let kp = sydar_dilithium::generate_keypair_from_seed(&inner.seed.to_bytes());
...
impl WalletDerivationManager {
    ...
    pub fn derive_child_pubkey_manager_from_seed(
        ...
        let mut hmac = HmacSha512::new_from_slice(&branch_attrs.chain_code).map_err(sydar_bip32::Error::Hmac)?;
    ...
    pub fn derive_public_key(
        ...
        let mut hmac = HmacSha512::new_from_slice(&attrs.chain_code).map_err(sydar_bip32::Error::Hmac)?;
        ...
        let depth = attrs.depth.checked_add(1).ok_or(sydar_bip32::Error::Depth)?;
...
impl WalletDerivationManagerTrait for WalletDerivationManager {
    ...
    fn from_master_xprv(xprv: &str, is_multisig: bool, account_index: u64, _cosigner_index: Option<u32>) -> Result<Self> {
        ...
        let (account_seed, attrs) =
            Self::create_extended_key(xprv_key.private_key().clone(), attrs.clone(), is_multisig, account_index)?;
        let kp = sydar_dilithium::generate_keypair_from_seed(&account_seed.to_bytes());
        let hash = Sha256::digest(kp.public_key());
        let mut pk_bytes = [0u8; 32];
        pk_bytes.copy_from_slice(&hash);
...
mod tests {
    ...
    use sydar_addresses::Prefix;

    fn gen1_receive_addresses() -> Vec<&'static str> {
        vec![
            "sydar:qz7ulu4c25dh7fzec9zjyrmlhnkzrg4wmf89q7gzr3gfrsj3uz6xjellj43pf",
            "sydar:qzn3qjzf2nzyd3zj303nk4sgv0aae42v3ufutk5xsxckfels57dxjjed4qvlx",
            "sydar:qpakxqlesqywgkq7rg4wyhjd93kmw7trkl3gpa3vd5flyt59a43yyjp28qsku",
            "sydar:qz0skffpert8cav6h2c9nfndmhzzfhvkrjexclmwgjjwt0sutysnw6lp55ak0",
            "sydar:qrmzemw6sm67svltul0qsk3974ema4auhrja3k68f4sfhxe4mxjwx0cj353df",
            "sydar:qpe4apax5dquy600py9rprmukhq8fqyqv9qu072twkvgse0glhqa74ynxmvfr",
            "sydar:qrptdge6ykdq672xqjd4rv2cedwdcz030jngsr2xhaxrn5l8pfhc294x9c7x6",
            "sydar:qqnys5nyennjkvyl77vwneq5j2vmjss57zerd88ptzaeqhm998smxw28uth8l",
            "sydar:qztckuvk02885rdazvj9w079qujg5qpxcdnmsvxqx0q8z7l483prkszjqwwff",
            "sydar:qrp53krck4m0x6n0dxs7vzf5mg0x6we8e06xjpmu8xr8p4du6f89khqdzw6uw",
            "sydar:qr4l3mahqe0jeeu6c474q5tywz08mudhddgtdneeq46unv0qx0j77kdtr52uu",
            "sydar:qzatdsueklx7pkfzanh9u0pwr47sd3a25gfm8wypsevdejhhpj8ck3v74v54j",
            "sydar:qqk3g5l6ymdkjfmzezx4zrv9fhr5rh0d8tm07udkqxq79n6t60tzu3fa7lnqg",
            "sydar:qqasa6d590u6875hsese68fa9f8mnedzesn2udehp0s73ggt5cklw2ge393eq",
            "sydar:qpuzq5jc757uxue9fradme33jd6egxr9fdznd8qysqcc5xy8k7alqpjgpdgrn",
            "sydar:qqygznwmkl56vprrnvyvnta9qql43yv52m3qz2462vxskn32axl0xccnpsqx9",
            "sydar:qqk974yml6uuustenwu57hn8n7d202luvn4dum0txvzjgg60g2jzsknngheak",
            "sydar:qpxqat995cxnjla8nm0dwnneesqnk5enc6hqrua7jztels0eqjg8vsm032lww",
            "sydar:qpyzkjs2a6k8ljx2qt4pwscj6jccr6k7pmru9k7r2t25teajjuzaz7zkesu0e",
            "sydar:qzf5mxtvk8wgp8gr3dcj3dkzdu6w4dgpvp2f0gm9pepv9vazxrhy577fy87rt",
            "sydar:qz44rhjkrddak9vf5z4swlmenxtfhmqc47d0lyf0j7ednyjln0u824ue33gvr",
        ]
    }

    fn gen1_change_addresses() -> Vec<&'static str> {
        vec![
            "sydar:qrqrnyzdwh9ec2q05guzy3vv33f86nvdyw52qwlmk0mewzx3dgdss3pmcd692",
            "sydar:qqx8jlz0hh0wun5ru4glt9za3v8wj3jn7v3w55a0lyud74ppetqfqny4yhw87",
            "sydar:qzpa69mrh2nj6xk6gq38vcnzu64necp0jwaxxyusr9xcy5udhu2m7uvql8rnd",
            "sydar:qqxddf76hr39dc7k7lpdzg065ajtvrhlm5p3edm4gyen0waneryss2c0la85t",
            "sydar:qps4qh9dtskwvf923yl9utl74r8sdm9h2wv3mftuxcfc2cshwswc6txj0k2kl",
            "sydar:qrds58d6nw9uz7z93ds4l6x9cgw3rquqzr69dtch6n4d8fxum8c65f7nqmhzx",
            "sydar:qrajjrpj0krqkww7rymwuwzcd36grjr6688ynvna649q26zukhcq6eqf4jmnx",
            "sydar:qrumkgz7hlsa748tnzvpztmf6wu9zsgqh6rppw4gzw2mvyq4ccj0y3ms9ju5l",
            "sydar:qz2g3cj3jcklk4w95djwnm9dffcwg75aqct2pefsxujrldgs08wac99rz70rc",
            "sydar:qznmzsvk0srfkur8l9pf55st0hnh3x8tmdyskjl9570w99lxsgs7cwrhxap2r",
            "sydar:qptamza95k7tchmukulldps4kl6wk853dnwa52t4azzm76h588qjufmnu3rn7",
            "sydar:qqt9h5cjqu9an68cn9k9jc2ywqmqu6kswjzeu09tqulswxkuccaxg6wz45f5r",
            "sydar:qphr6uy46ad3ca7rerzkx7kkzfzsvfe0xanh4u5mrh538cexs4yjkww0pa4dh",
            "sydar:qzv3qlh5q4fpy6eu5s4wj080l64del4lvg986z5uh0c3g7wf6n8pvsgm3c9e0",
            "sydar:qp2dd6y4szgyhcendh7ncxws0qvx8k3s92tg7lvy8eel5npg4pd2ks0ctx4hl",
            "sydar:qpkqvnkler4rwlpt720unepf3q8cayv0shx0vzydrae7a6u7ryy8zdvnmncyc",
            "sydar:qr4v33jupxv9h6juqads0znrnw6g7an2ajuzusthnjqujquz66rewtjekhz4l",
            "sydar:qz5pq2yzpz8ce5avrsa4uzzwrlr5a86rvs74afd6qdm3h649v08nk0qxhrl9n",
            "sydar:qrajmn035raezl6rcvd0wvnfmdnc0qzwr686ccsrn3z5x8aqnpt8qa0e954jk",
            "sydar:qrqg7r05nk7syxjh8rdz8wanzmyh8sdts9uexxnnwkq8fplrjammvcnrdggw0",
        ]
    }
    ...
    async fn wallet_from_mnemonic() {
        ...
        let mnemonic = sydar_bip32::Mnemonic::new(mnemonic, sydar_bip32::Language::English).unwrap();
        let xprv = sydar_bip32::ExtendedPrivateKey::<sydar_bip32::SecretKey>::new(mnemonic.to_seed("")).unwrap();
        let xprv_str = xprv.to_string(sydar_bip32::Prefix::KPRV).to_string();
        assert_eq!(
            xprv_str,
        ...
        let xpub_str = wallet.to_string(Some(sydar_bip32::Prefix::KPUB)).to_string();
        assert_eq!(
            xpub_str,
    ...
    async fn address_test_by_ktrv() {
        ...
        let mnemonic = sydar_bip32::Mnemonic::new(mnemonic, sydar_bip32::Language::English).unwrap();
        let xprv = sydar_bip32::ExtendedPrivateKey::<sydar_bip32::SecretKey>::new(mnemonic.to_seed("")).unwrap();
        let ktrv_str = xprv.to_string(sydar_bip32::Prefix::KTRV).to_string();
        assert_eq!(
            ktrv_str,
        ...
        let ktub_str = wallet.to_string(Some(sydar_bip32::Prefix::KTUB)).to_string();
        assert_eq!(
            ktub_str,
        ...
        assert_eq!(address, "sydartest:qrc2959g0pqda53glnfd238cdnmk24zxzkj8n5x83rkktx4h73dkc4ave6wyg")
    }
    ...
    #[tokio::test]
    #[ignore]
    async fn generate_sydartest_addresses() {
        let receive_addresses = [
            "sydartest:qz7ulu4c25dh7fzec9zjyrmlhnkzrg4wmf89q7gzr3gfrsj3uz6xjceef60sd",
            "sydartest:qzn3qjzf2nzyd3zj303nk4sgv0aae42v3ufutk5xsxckfels57dxjnltw0jwz",
            "sydartest:qpakxqlesqywgkq7rg4wyhjd93kmw7trkl3gpa3vd5flyt59a43yyn8vu0w8c",
            "sydartest:qz0skffpert8cav6h2c9nfndmhzzfhvkrjexclmwgjjwt0sutysnwme80mr8t",
            "sydartest:qrmzemw6sm67svltul0qsk3974ema4auhrja3k68f4sfhxe4mxjwxw752m0ud",
            "sydartest:qpe4apax5dquy600py9rprmukhq8fqyqv9qu072twkvgse0glhqa75z4a5jc8",
            "sydartest:qrptdge6ykdq672xqjd4rv2cedwdcz030jngsr2xhaxrn5l8pfhc2ynq7hqh7",
            "sydartest:qqnys5nyennjkvyl77vwneq5j2vmjss57zerd88ptzaeqhm998smx0vp8yfkm",
            "sydartest:qztckuvk02885rdazvj9w079qujg5qpxcdnmsvxqx0q8z7l483prk3y5mpscd",
            "sydartest:qrp53krck4m0x6n0dxs7vzf5mg0x6we8e06xjpmu8xr8p4du6f89kkxtepyd2",
            "sydartest:qr4l3mahqe0jeeu6c474q5tywz08mudhddgtdneeq46unv0qx0j77htdcm5dc",
            "sydartest:qzatdsueklx7pkfzanh9u0pwr47sd3a25gfm8wypsevdejhhpj8cks2cwr2yk",
            "sydartest:qqk3g5l6ymdkjfmzezx4zrv9fhr5rh0d8tm07udkqxq79n6t60tzus0m9sd3v",
            "sydartest:qqasa6d590u6875hsese68fa9f8mnedzesn2udehp0s73ggt5cklwtwl220gy",
            "sydartest:qpuzq5jc757uxue9fradme33jd6egxr9fdznd8qysqcc5xy8k7alqq5w6zkjh",
            "sydartest:qqygznwmkl56vprrnvyvnta9qql43yv52m3qz2462vxskn32axl0xe746l7hp",
            "sydartest:qqk974yml6uuustenwu57hn8n7d202luvn4dum0txvzjgg60g2jzsh44nc8vj",
            "sydartest:qpxqat995cxnjla8nm0dwnneesqnk5enc6hqrua7jztels0eqjg8v3af29pl2",
            "sydartest:qpyzkjs2a6k8ljx2qt4pwscj6jccr6k7pmru9k7r2t25teajjuzazlyszlz7a",
            "sydartest:qzf5mxtvk8wgp8gr3dcj3dkzdu6w4dgpvp2f0gm9pepv9vazxrhy5lc0lgqj0",
        ];
        ...
        for index in 0..20 {
            ...
            //let address = Address::new(Prefix::Testnet, sydar_addresses::Version::PubKey, key.to_bytes());
