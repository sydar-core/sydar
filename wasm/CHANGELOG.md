Latest online documentation available at: https://sydar.aspectron.org/docs/
...
- Merge with sydar master (0.13.6)
...
- Rename `sydar-beacon` app to `sydar-resolver`
...
- Introduced `Beacon` class that provides connectivity to the community-operated public node infrastructure (backed by `sydar-beacon` load balancer & node status monitor)
...
- Changed documentation structure to use `typedoc` available as a part of redistributables or online at https://sydar.aspectron.org/docs/
