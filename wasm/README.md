...
- [**Rust** documentation](https://docs.rs/sydar-wasm/latest/sydar_wasm/index.html)
- [**TypeScript** documentation](https://sydar.aspectron.org/docs/)
...
- **RPC API** — RPC API for the sydar node using WebSockets.
- **Wallet SDK** — Bindings for primitives related to key management and transactions.
- **Wallet API** — API for the sydar Wallet framework.
...
- `web/sydar` - **full** sydar WASM32 SDK bindings for use in web browsers.
- `web/sydar-rpc` - only the RPC bindings for use in web browsers (reduced WASM binary size).
- `nodejs/sydar` - **full** sydar WASM32 SDK bindings for use with NodeJS.
- `docs` - sydar WASM32 SDK documentation.
...
- `examples/javascript/wallet` - Interfacing with the sydar Wallet framework.
...
Included documentation in the release can be accessed by loading the `docs/sydar/index.html` 
file in a web browser.

## Building from Source

To build the WASM32 SDK from source, you need to have the Rust environment installed. To do that,
follow instructions in the [sydar README](https://github.com/sydarnet/sydar).
...
serve them from the root of the SDK folder (`sydar-wasm32-sdk` if using a redistributable or
`sydar/wasm` if building from source). This is because examples use relative paths.
...
before loading the WASM32 library. The compatible WebSocket library is [WebSocket](https://www.npmjs.com/package/websocket) and is included in the `sydar` NPM package. `sydar` package is a wrapper around `sydar-wasm` that imports and installs this WebSocket shim in the `globalThis` object and then re-exports `sydar-wasm` exports.
...
<html>
    <head>
        <script type="module">
            import * as sydar from './sydar/sydar-wasm.js';
            (async () => {
                await sydar.default('./sydar/sydar-wasm_bg.wasm');
