...
const fs = require('fs');
const sydar = require('../../../../nodejs/sydar-dev');
const {
...
(async()=>{
    ...
    async function log_transactions(accountId){
        ...
        transactionsResult.transactions.forEach((tx)=>{
            ...
            list.push({
                Id: tx.id,
                Type: tx.data.type,
                Value: kanaTosydarString(value(tx)||0)
            });
    ...
    try {
                ...
                case "balance":
                    ...
                    Object.keys(balance).map(id=>{
                        ...
                        list.push({
                            ...
                            Mature: kanaTosydarString(b.mature),
                            Pending: kanaTosydarString(b.pending),
                            Outgoing: kanaTosydarString(b.outgoing),
        ...
        // Send sydar to address
        let sendResult = await wallet.accountsSend({
            walletSecret,
            // @ts-ignore
            accountId: firstAccount.accountId,
            priorityFeeKana: sydarToKana("0.001"),
            destination:[{
                // @ts-ignore
                address: firstAccount.changeAddress,
                amount: sydarToKana("1.567")
            }]
        });
        ...
        // Transfer sydar between accounts
        let transferResult = await wallet.accountsTransfer({
            ...
            transferAmountKana: sydarToKana("2.456"),
        });
