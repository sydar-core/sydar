...
use sydar_consensus::{
    ...
    model::stores::{
        ...
        sydar_consensus::{KType, sydarConsensusStoreReader},
    },
...
use sydar_consensus_core::{
    ...
    config::bps::calculate_sydar_consensus_k, errors::block::BlockProcessResult, mining_rules::MiningRules, tx::TransactionType,
};
...
use sydar_database::{create_temp_db, load_existing_db};
use sydar_hashes::Hash;
use sydar_perf_monitor::{builder::Builder, counters::CountersSnapshot};
use sydar_utils::fd_budget;
use simulator::network::sydarNetworkSimulator;
use std::{collections::VecDeque, sync::Arc, time::Duration};

mod blocks_json;
pub mod simulator;

// sydar Network Simulator
#[derive(Parser, Debug)]
...
fn main() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(feature = "semaphore-trace")] {
            sydar_core::log::init_logger(None, &format!("{},{}=debug", args.log_level, sydar_utils::sync::semaphore_module_path()));
        } else {
            sydar_core::log::init_logger(None, &args.log_level);
        }
    };
...
fn main_impl(mut args: Args) {
    ...
    let stop_perf_monitor = args.perf_metrics.then(|| {
        ...
        let cb = move |counters: CountersSnapshot| {
            trace!("[{}] {}", sydar_perf_monitor::SERVICE_NAME, counters.to_process_metrics_display());
            trace!("[{}] {}", sydar_perf_monitor::SERVICE_NAME, counters.to_io_metrics_display());
            #[cfg(feature = "heap")]
    ...
    let (consensus, _lifetime) = if let Some(input_dir) = args.input_dir {
        ...
        let mut sim = sydarNetworkSimulator::new(args.delay, args.bps, args.target_blocks, config.clone(), args.output_dir);
    ...
    if args.test_pruning {
        ...
        let num_txs = print_stats(&consensus, &hashes, args.delay, args.bps, config.sydar_consensus_k());
...
fn apply_args_to_consensus_params(args: &Args, params: &mut Params) {
    ...
    if args.testnet11 {
        info!(
            "Using sydar-testnet-11 configuration (sydar_CONSENSUS K={}, DAA window size={}, Median time window size={})",
            params.sydar_consensus_k(),
            params.difficulty_window_size,
            params.past_median_time_window_size,
        );
    } else {
        let max_delay = args.delay.max(NETWORK_DELAY_BOUND as f64);
        let k = u64::max(calculate_sydar_consensus_k(2.0 * max_delay * args.bps, 0.05), params.sydar_consensus_k() as u64);
        let k = u64::min(k, KType::MAX as u64) as KType; // Clamp to KType::MAX
        params.sydar_consensus_k = k;
        ...
        info!("2Dλ={}, sydar_CONSENSUS K={}, DAA window size={}", 2.0 * args.delay * args.bps, k, params.difficulty_window_size);
    }
    if args.test_pruning {
...
async fn validate(src_consensus: &Consensus, dst_consensus: &Consensus, params: &Params, delay: f64, bps: f64, header_only: bool) {
    ...
    let num_blocks = hashes.len();
    let num_txs = print_stats(src_consensus, &hashes, delay, bps, params.sydar_consensus_k());
    if header_only {
...
fn print_stats(src_consensus: &Consensus, hashes: &[Hash], delay: f64, bps: f64, k: KType) -> usize {
    let blues_mean =
        hashes.iter().map(|&h| src_consensus.sydar_consensus_store.get_data(h).unwrap().mergeset_blues.len()).sum::<usize>() as f64
            / hashes.len() as f64;
    let reds_mean =
        hashes.iter().map(|&h| src_consensus.sydar_consensus_store.get_data(h).unwrap().mergeset_reds.len()).sum::<usize>() as f64
            / hashes.len() as f64;
    ...
    info!("[DELAY={delay}, BPS={bps}, sydar_CONSENSUS K={k}]");
...
mod tests {
    ...
    fn test_pruning_via_yugat() {
        ...
        sydar_core::log::try_init_logger(&args.log_level);
        // As we log the panic, we want to set it up after the logger
        sydar_core::panic::configure_panic();
        main_impl(args);
    }
